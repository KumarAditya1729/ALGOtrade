...
def _fetch_yahoo_chart_quote(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "Mozilla/5.0 (compatible; CalculatedRisk/1.0)"},
            timeout=10,
        )
...
def fetch_local_stock_opportunity_prices(market: str, limit: int = 15, *, fast: bool = False) -> List[Dict[str, Any]]:
    ...
    m = str(market or "").strip()
    if m not in ("CNStock", "HKStock", "USStock", "INStock"):
        return []

    _FALLBACK_SYMBOLS = {
        "INStock": [
            {"symbol": "RELIANCE.NS", "name": "Reliance"}, {"symbol": "HDFCBANK.NS", "name": "HDFC Bank"},
            {"symbol": "TCS.NS", "name": "TCS"}, {"symbol": "INFY.NS", "name": "Infosys"},
            {"symbol": "ICICIBANK.NS", "name": "ICICI Bank"}, {"symbol": "SBIN.NS", "name": "SBI"},
            {"symbol": "BHARTIARTL.NS", "name": "Bharti Airtel"}, {"symbol": "ITC.NS", "name": "ITC"},
            {"symbol": "L&T.NS", "name": "Larsen & Toubro"}, {"symbol": "BAJFINANCE.NS", "name": "Bajaj Finance"},
        ],
        "USStock": [
    ...
    try:
        ...
        if m in ("USStock", "HKStock", "CNStock", "INStock") and len(items) > 1:
