...
def get_economic_calendar_payload() -> Dict[str, Any]:
    ...
    Provider priority:
    1. RapidAPI Ultimate Economic Calendar (if RAPIDAPI_KEY is set) — Indian + global events
    2. Trading Economics (if credentials configured)
    3. AkShare WallstreetCN fallback (no key needed)
    4. Finnhub (if FINNHUB_FREE_ONLY=false and key configured)
    """
    finnhub_free_only_override = os.getenv("FINNHUB_FREE_ONLY")
    finnhub_api_key_override = os.getenv("FINNHUB_API_KEY")

    # --- 1. RapidAPI Ultimate Economic Calendar ---
    rapidapi_key = os.getenv("RAPIDAPI_KEY", "").strip()
    if rapidapi_key:
        try:
            events = _fetch_rapidapi_calendar(rapidapi_key)
            if events:
                logger.info("Economic calendar loaded from RapidAPI: %d events", len(events))
                return {
                    "events": events,
                    "status": "ok",
            ...
            logger.warning("RapidAPI economic calendar returned no events, falling through")
        except Exception as exc:
            logger.warning("RapidAPI economic calendar failed: %s", exc)
...
def _fetch_rapidapi_calendar(api_key: str) -> List[Dict[str, Any]]:
    """Fetch economic calendar from RapidAPI Ultimate Economic Calendar.
    Prioritises Indian events (IN) + major global events.
    """
    ...
    host = os.getenv("RAPIDAPI_ECONOMIC_CALENDAR_HOST", "ultimate-economic-calendar.p.rapidapi.com")

    resp = requests.get(
        f"https://{host}/economic-events/tradingview",
        params={"from": date_from, "to": date_to, "countries": "IN,US,EU,GB,JP"},
        headers={
    ...
    for idx, row in enumerate(rows):
        if not isinstance(row, dict):
            continue
        event_en = str(
            row.get("title") or row.get("event") or row.get("name") or row.get("indicator") or ""
        ).strip()
        if not event_en:
            continue

        country = str(row.get("country") or row.get("region") or "INTL").strip().upper()
        # Build date/time from the row
        date_value = str(
            row.get("date") or row.get("datetime") or row.get("time") or ""
        ).strip()
        date_str = date_value[:10] if len(date_value) >= 10 else today.isoformat()
        time_str = date_value[11:16] if len(date_value) >= 16 else "--:--"

        # Map importance
        importance_raw = row.get("importance") or row.get("impact") or row.get("priority") or ""
        importance = _map_importance(importance_raw)

        actual = _format_value(_none_if_nan(row.get("actual")), "")
        forecast = _format_value(_none_if_nan(row.get("forecast") or row.get("estimate")), "")
        previous = _format_value(_none_if_nan(row.get("previous") or row.get("prev")), "")

        impact_if_above, impact_if_below = _impact_rules(event_en)
        actual_impact = None
        if actual and forecast:
            actual_impact = _compare_impact(actual, forecast, event_en, impact_if_above, impact_if_below)
        is_released = actual is not None

        dedupe_key = f"{date_str}|{time_str}|{event_en.lower()}|{country}"
        if dedupe_key in seen_keys:
            continue
        seen_keys.add(dedupe_key)

        events.append(_with_ai_insight({
            ...
            "impact_desc": "",
            "impact_desc_en": "",
            "expected_impact": actual_impact or impact_if_above or "neutral",
            "actual_impact": actual_impact,
            "is_released": is_released,
            "source": "rapidapi_economic_calendar",
        }))

    # Sort: Indian events first, then by importance, then by date/time
    events.sort(key=lambda e: (
        0 if e.get("country") == "IN" else 1,
        _importance_rank(e.get("importance")),
        e["date"],
        e["time"],
    ))
    return events
