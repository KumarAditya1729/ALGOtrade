...
def _execute(kind: str, payload: dict, on_progress):
    ...
    if kind == "flow_strategy":
        from app.routes.agent_v1.backtests import _run_backtest
        from app.services.strategy_v2.flow_compiler import FlowCompiler
        
        # Compile flow to python code
        compiler = FlowCompiler(request_payload.get("flow", {}))
...
@celery_app.task(name="calculatedrisk.tasks.agent_job", acks_late=True)
