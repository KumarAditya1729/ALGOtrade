...
def require_live_trading_permission(f):
    """
    Decorator that checks if live trading is globally enabled.
    Must be used after @login_required.
    """
    @wraps(f)
    def decorated(*args, **kwargs):
        if not Config.LIVE_TRADING_ENABLED:
            return jsonify({
                'code': 403,
                'msg': 'Live trading is globally disabled on this node.',
                'data': None
            }), 403
            
        # We can also verify user-level live trading permissions here if needed.
        return f(*args, **kwargs)
    return decorated
