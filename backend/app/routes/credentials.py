...
def create_credential(data):
    ...
    try:
        ...
        # Known Indian Equity Brokers
        indian_brokers = {
            'fivepaisa', 'fivepaisaxts', 'aliceblue', 'angel', 'arrow',
            'compositedge', 'dhan', 'deltaexchange', 'indmoney', 'dhan_sandbox',
            'definedge', 'firstock', 'flattrade', 'motilal', 'fyers',
            'groww', 'hdfcsecurities', 'hdfcsky', 'ibulls', 'iifl',
            'iiflcapital', 'jainamxts', 'kotak', 'mstock', 'nubra',
            'paytm', 'pocketful', 'rmoney', 'samco', 'shoonya',
            'tradejini', 'tradesmart', 'upstox', 'wisdom', 'zebu', 'zerodha'
        }
        ...
        if exchange_id == 'alpaca':
        ...
        elif exchange_id in indian_brokers:
            # Indian Equity Brokers (SaaS model: UI passes api_key/api_secret)
            api_key = (data.get('api_key') or data.get('apiKey') or '').strip()
            secret_key = (data.get('secret_key') or data.get('secretKey') or '').strip()
            if not api_key:
                return jsonify({'code': 0, 'msg': 'Missing api_key', 'data': None}), 400
            ...
            # For Indian brokers, the API key acts as the Client ID for OAuth redirects,
            # so the frontend needs the full string.
            hint = api_key
        else:
