...
def _classify_agent_intent(message: str, attachments: list[dict], context: dict, language: str) -> dict:
    ...
    system_prompt = (
        "You are the CalculatedRisk Agent Intent Router. Classify the user's message into a "
...
def _build_system_prompt(language: str, context: dict, intent: str, has_image: bool, json_response: bool = True) -> str:
    ...
    base = (
        "You are CalculatedRisk Copilot, a trading system assistant for open-source quant users. "
        ...
        "If the user asks to write strategy code, stay inside CalculatedRisk native workflows. "
    ...
    if isinstance(research_context, dict) and research_context:
        base += (
            "\n[CalculatedRisk Research Context]\n"
...
def export_chat_report_pdf():
    ...
    filename = f"CalculatedRisk_{symbol or 'report'}_{_now_utc().strftime('%Y%m%d')}.pdf"
    return Response(
        pdf_bytes,
