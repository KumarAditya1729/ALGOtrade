...
def place_order(body):
    ...
    try:
        ...
        if not credential_id:
            return api_response(0, "Missing credential_id", None, 400)
        if not symbol:
            return api_response(0, "Missing symbol", None, 400)
        if side not in ("buy", "sell"):
            return api_response(0, "side must be 'buy' or 'sell'", None, 400)
        if usdt_amount <= 0:
            return api_response(0, "amount must be > 0", None, 400)
        if order_type == "limit" and price <= 0:
            return api_response(0, "price required for limit orders", None, 400)
        ...
        if market_type == "spot":
            ...
            if order_type == "market" and side == "buy":
                ...
                if quote_for_buy <= 0:
                    return api_response(0, "Order notional is below the exchange minimum. Increase the USDT amount.", None, 400)
            ...
            if base_qty <= 0 and quote_for_buy <= 0:
                ...
                return api_response(0, f"Order quantity is below the exchange minimum. Increase the amount or check symbol rules.{hint}", None, 400)
                ...
                if need_quote > 0:
                    try:
                        ...
                        if need_quote > avail + 1e-6:
                            return api_response(0, f"Insufficient spot USDT balance: need about {need_quote:.4f} USDT, available {avail:.4f} USDT.", {"error_hint": "quickTrade.errorHints.insufficientBalance"}, 400)
        ...
        if market_type != "spot" and order_type == "market":
            try:
                ...
                if avail > 0 and est_margin > avail:
                    return api_response(0, f"Insufficient derivatives margin: {notional_usdt:.2f} USDT notional at {lev}x needs about {est_margin:.2f} USDT margin, but only {avail:.2f} USDT is available. Reduce order size, adjust leverage, or transfer funds.", {"error_hint": "quickTrade.errorHints.insufficientBalance"}, 400)
            except Exception as pe:
                logger.warning("swap margin pre-check skipped: %s", pe)

        # ---- place order ----
        # Phase 6: Canonical live execution path. Disallow synchronous client.place_order bypasses.
        import time
        import uuid
        import json
        ...
        client_order_id = f"qt{timestamp_suffix}{uuid_suffix}"
        idempotency_key = f"quicktrade:{user_id}:{client_order_id}"
        ...
        execution_mode = "paper" if "paper" in exchange_id.lower() else "live"
        ...
        with get_db_connection() as db:
            ...
            row = cur.fetchone()
            if not row:
                cur.execute("SELECT id FROM pending_orders WHERE idempotency_key = %s", (idempotency_key,))
                row = cur.fetchone()
            db.commit()
            cur.close()
            
        pending_id = int(row["id"] if row else 0)
        
        # We also create a queued record in qd_quick_trades so the UI has immediate feedback
        status = "queued"
        exchange_order_id = f"pending_{pending_id}"
        filled = 0.0
        avg_fill = 0.0
        commission = 0.0
        commission_ccy = ""
        commission_quote = 0.0
        raw_record = {"_quick_trade": {"pending_order_id": pending_id}}
        ...
        return api_response(1, "Order placed successfully", {
            "trade_id": trade_id,
            "exchange_order_id": exchange_order_id,
            "filled": filled,
            "avg_price": avg_fill,
