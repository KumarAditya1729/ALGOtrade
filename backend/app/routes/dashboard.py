...
def summary():
    ...
    try:
        ...
        from app.routes.quick_trade import (
            ...
            _fetch_spot_holdings_raw
        )
        from app.services.quick_trade.balances import fetch_balance_raw
        ...
        try:
            with get_db_connection() as db:
                cur = db.cursor()
                cur.execute(
                    "SELECT id, exchange_id, encrypted_config FROM qd_exchange_credentials WHERE user_id = %s ORDER BY id DESC",
                    (user_id,)
                )
            ...
            if real_cred:
                ...
                # Fetch Real Balances
                parsed_funds = fetch_balance_raw(client, exchange_id=ex_id, market_type="spot", exchange_config=ex_cfg)
                
                real_available = 0.0
                real_equity = 0.0
                if isinstance(parsed_funds, dict):
                    real_available = float(parsed_funds.get("available") or 0.0)
                    real_equity = float(parsed_funds.get("total") or real_available)
