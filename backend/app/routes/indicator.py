...
`/addons/calculatedrisk/indicator/getIndicators`.
...
def ai_generate():
    ...
    # CalculatedRisk indicator IDE: chart render only; strategies are separate script assets.
    SYSTEM_PROMPT = """# Role

You write production-ready **CalculatedRisk** chart indicator scripts: Python that runs in the Indicator IDE and renders overlays/markers on the K-line chart. Indicators are **not executable strategies**: they must not open, close, size, backtest, or live trade. If a user wants trading logic, keep this file as a visual indicator and let the Strategy API V2 workflow generate executable strategy code separately.
...
""" + "\n\n" + INDICATOR_GENERATION_CONTRACT
    ...
    def _generate_code_via_llm() -> str:
        ...
        if existing:
            user_prompt = (
                "# Existing CalculatedRisk indicator code (migrate it to the chart-only indicator contract):\n\n```python\n"
                + existing.strip()
                + "\n```\n\n# Change request:\n\n"
                + prompt
                + context_text
    ...
    def _repair_code_via_llm(bad_code: str, validation: Dict[str, Any]) -> str:
        ...
        repair_prompt = (
            "You produced CalculatedRisk indicator code that failed automatic validation. "
