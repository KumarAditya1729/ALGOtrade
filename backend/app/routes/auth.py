...
@auth_blp.route('/broker-config', methods=['GET'])
def get_broker_config():
    """Return broker configuration for React SPA."""
    import re
    redirect_url = os.getenv("REDIRECT_URL", "http://localhost:5000/zerodha/callback")
    match = re.search(r"/([^/]+)/callback$", redirect_url)
    broker_name = match.group(1) if match else "zerodha"
...
@auth_blp.route('/session-status', methods=['GET'])
def get_session_status():
    """Return current session/auth status for React SPA.

    The AuthSync component polls this endpoint to sync Flask session state
    with the Zustand auth store. Returns a 200 JSON response indicating
    whether the user is authenticated and has connected a broker.
    """
    # Check if request carries a valid JWT
    ...
    try:
        ...
        username = payload.get('_verified_username') or payload.get('sub') or payload.get('username')
        return jsonify({
            "status": "success",
            "authenticated": True,
            "logged_in": False,   # broker OAuth not connected in JWT-only flow
            "user": username,
            "broker": None,
            "active_sessions": 0,
        }), 200
...
def login(data):
    ...
    try:
        ...
        if not username or not password:
            return api_response(400, 'Missing username/email or password', None, 400)
        ...
        if not turnstile_ok:
            return api_response(0, turnstile_msg, None, 400)
        
        # Step 2: Check rate limiting
        allowed, block_msg = security.check_login_allowed(username, ip_address)
        if not allowed:
            return api_response(0, block_msg, {'blocked': True}, 429)
        ...
        if not _is_single_user_mode():
            try:
                ...
                if user and user.get('_no_password'):
                    ...
                    return api_response(0, 'This account was created with email verification code and has no password set. Please use email code login or set a password first in your profile settings.', None, 401)
            except Exception as e:
        ...
        if not user:
            ...
            return api_response(0, 'Invalid credentials', None, 401)
        
        # Check user status
        if user.get('status') == 'disabled':
            security.log_security_event('login_blocked', user.get('id'), ip_address, user_agent,
                                       {'reason': 'account_disabled'})
            return api_response(0, 'Account is disabled', None, 403)
        
        if user.get('status') == 'pending':
            return api_response(0, 'Account is pending activation', None, 403)
        ...
        try:
            ...
            if need_mfa:
                ...
                return api_response(1, 'MFA verification required', {
                    'mfa_required': True,
                    **challenge
                })
        except Exception as e:
            logger.error(f"MFA check failed after password authentication: {e}")
            return api_response(0, 'MFA service unavailable', None, 503)

        token, userinfo = _issue_login_token(user, int(user_id), username)
        
        if not token:
            return api_response(500, 'Token generation error', None, 500)
        ...
        return api_response(0, str(e), None, 401)
    except Exception as e:
