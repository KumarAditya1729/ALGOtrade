...
def place_order():
    ...
    try:
        ...
        # Phase 6: Canonical live execution path. Disallow synchronous bypasses.
        import time
        import uuid
        import json
        ...
        user_id = int(getattr(g, "user_id", 1))
        
        timestamp_suffix = str(int(time.time()))[-6:]
        uuid_suffix = uuid.uuid4().hex[:8]
        client_order_id = f"al{timestamp_suffix}{uuid_suffix}"
        idempotency_key = f"alpaca:{user_id}:{client_order_id}"
