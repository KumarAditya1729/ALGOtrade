...
@portfolio_blp.route('/angel-one/holdings', methods=['GET'])
@login_required
def get_angel_holdings():
    """Fetch live holdings from Angel One SmartAPI and return in standard format."""
    try:
        from app.data_sources.india import IndiaDataSource
        import httpx

        creds = IndiaDataSource._get_user_credentials()
        if not creds:
            return jsonify({'code': 0, 'msg': 'No Angel One credentials found', 'data': []})

        jwt_token = creds.get("access_token", "")
        api_key = creds.get("api_key", "")
        
        if not jwt_token or not api_key:
            return jsonify({'code': 0, 'msg': 'Angel One credentials missing jwt or api_key', 'data': []})
        ...
        with httpx.Client() as client:
            res = client.get("https://apiconnect.angelbroking.com/rest/secure/angelbroking/portfolio/v1/getHolding", headers=headers, timeout=10.0)
            
        data = res.json()
        if not data.get("status"):
            logger.error(f"AngelOne Holdings API Error: {data.get('message')}")
            return jsonify({'code': 0, 'msg': data.get('message'), 'data': []})
        ...
        for h in holdings:
            ...
            standardized_holdings.append({
                "id": h.get("symboltoken"),
                "market": "India",
        ...
        return jsonify({'code': 1, 'msg': 'success', 'data': standardized_holdings})
    except Exception as e:
        logger.error(f"get_angel_holdings failed: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({'code': 0, 'msg': str(e), 'data': []}), 500


@portfolio_blp.route('/angel-one/positions', methods=['GET'])
@login_required
def get_angel_positions():
    """Fetch live positions from Angel One SmartAPI and return in standard format."""
    try:
        from app.data_sources.india import IndiaDataSource
        import httpx
        ...
        with httpx.Client() as client:
            res = client.get("https://apiconnect.angelbroking.com/rest/secure/angelbroking/order/v1/getPosition", headers=headers, timeout=10.0)
            
        data = res.json()
        if not data.get("status"):
            logger.error(f"AngelOne Positions API Error: {data.get('message')}")
            return jsonify({'code': 0, 'msg': data.get('message'), 'data': []})
        ...
        for p in positions:
            ...
            cost_value = abs(net_qty * avg_price)
            market_value = abs(net_qty * ltp)
            
            # For short positions (netqty < 0), logic reverses.
            if net_qty > 0:
        ...
        return jsonify({'code': 1, 'msg': 'success', 'data': standardized_positions})
    except Exception as e:
        logger.error(f"get_angel_positions failed: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({'code': 0, 'msg': str(e), 'data': []}), 500
