...
def search_symbols(market: str, keyword: str, limit: int = 20) -> List[Dict]:
    ...
    # Bundle Indian markets together
    is_india = market.upper() == 'INDIA'
    market_clause = "market IN ('NSE', 'BSE', 'NFO', 'BFO', 'MCX', 'CDS')" if is_india else "market = ?"
    query_params = (kw, max(limit, 0)) if is_india else (market, kw, max(limit, 0))
    ...
    try:
        with _get_db_connection() as db:
            ...
            cur.execute(
                f"""
                SELECT market, symbol, name, exchange, instrument_id FROM qd_market_symbols
                WHERE {market_clause} AND is_active = 1 AND UPPER(symbol) = UPPER(?)
                LIMIT ?
                """,
                query_params
            )
            ...
            query_params_like = (
                pattern, pattern, pattern, kw, prefix_pattern, pattern, prefix_pattern, kw, prefix_pattern, max(limit, 0)
            )
            if not is_india:
                query_params_like = (market,) + query_params_like

            cur.execute(
                f"""
                SELECT market, symbol, name, exchange, instrument_id FROM qd_market_symbols s
                WHERE {market_clause} AND is_active = 1
                  AND (
...
def _search_symbols_without_aliases(market: str, keyword: str, limit: int) -> List[Dict]:
    ...
    is_india = market.upper() == 'INDIA'
    market_clause = "market IN ('NSE', 'BSE', 'NFO', 'BFO', 'MCX', 'CDS')" if is_india else "market = ?"
    query_params_fb = (pattern, pattern, keyword, prefix_pattern, pattern, prefix_pattern, max(limit, 0))
    if not is_india:
        query_params_fb = (market,) + query_params_fb
