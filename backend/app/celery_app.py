...
celery_app = Celery("calculatedrisk", task_cls=FlaskContextTask)
celery_app.conf.update(
    ...
    task_routes={
        "calculatedrisk.tasks.fast_analysis": {"queue": "ai"},
        "calculatedrisk.tasks.agent_job": {"queue": "jobs"},
        "calculatedrisk.tasks.reflection": {"queue": "maintenance"},
        "calculatedrisk.tasks.ai_calibration": {"queue": "maintenance"},
        "calculatedrisk.tasks.market_catalog_sync": {"queue": "maintenance"},
        "calculatedrisk.tasks.worker_heartbeat": {"queue": "maintenance"},
        "calculatedrisk.tasks.cleanup_runtime_metadata": {"queue": "maintenance"},
    },
    beat_schedule={
        "reflection-cycle": {
            "task": "calculatedrisk.tasks.reflection",
            "schedule": max(300, int(os.getenv("REFLECTION_WORKER_INTERVAL_SEC", "86400"))),
        },
        "ai-calibration-cycle": {
            "task": "calculatedrisk.tasks.ai_calibration",
            "schedule": max(3600, int(os.getenv("AI_CALIBRATION_INTERVAL_SEC", "86400"))),
        },
        "market-catalog-sync": {
            "task": "calculatedrisk.tasks.market_catalog_sync",
            "schedule": max(900, int(os.getenv("MARKET_CATALOG_SYNC_INTERVAL_SEC", "86400"))),
        },
        "celery-worker-heartbeat": {
            "task": "calculatedrisk.tasks.worker_heartbeat",
            "schedule": 10.0,
        },
        "runtime-metadata-cleanup": {
            "task": "calculatedrisk.tasks.cleanup_runtime_metadata",
            "schedule": 86400.0,
        },
    },
)
