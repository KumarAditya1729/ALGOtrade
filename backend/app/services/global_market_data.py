...
def compute_trading_opportunities() -> List[Dict[str, Any]]:
    ...
    candidate_scanners = [
        ("INStock", lambda: analyze_opportunities_local_stocks(opportunities, "INStock")),
    ]
    scanners = [(label, fn) for label, fn in candidate_scanners if is_market_visible(label) or label == "INStock"]
