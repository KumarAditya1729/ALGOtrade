...
def execute_grid_market_order(
    ...
    *,
    ...
    try:
        ...
        # Phase 6: Canonical live execution path for grid market orders.
        ...
        with get_db_connection() as db:
            ...
            cur.execute(
                ...
                VALUES
                  (%s, 0, %s, %s, UNIX_TIMESTAMP(), %s, %s, %s, %s, %s, 'pending', 0,
                   0, 10, '', %s, 0, %s, %s, NOW(), NOW(), '')
        ...
        # We simulate the `res` object as a dict with exchange_order_id mapped to pending_id
        # Then wait_grid_market_fill will wait for the actual exchange fill? No, wait_grid_market_fill
        # polls the *exchange* directly.
        # But pending_orders worker will insert the fill into the exchange.
        # So we can just set ex_oid to something like "pending_{pending_id}" and let wait_grid_market_fill poll pending_orders.
        
        # Wait, wait_grid_market_fill expects an exchange_order_id.
        # If we change ex_oid here, we need to modify wait_grid_market_fill too, or we poll here instead.
        import time
        start_t = time.time()
        ...
        while time.time() - start_t < max_wait_sec:
            ...
            if rec:
                ...
                ex_oid = str(rec.get("exchange_order_id") or "")
                filled_qty = float(rec.get("filled") or 0.0)
                avg_price = float(rec.get("avg_price") or 0.0)
                if final_status in ("filled", "partially_filled", "failed", "rejected", "cancelled"):
                    break
            time.sleep(1.0)
