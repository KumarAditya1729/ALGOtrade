...
from app.services.live_trading.executors import (
    ...
    MarketOrderExecutor, OrderExecutionResult,
...
class PendingOrderWorker(PendingOrderPositionSyncMixin):
    ...
    def _fetch_pending_orders(self, limit: int = 50) -> List[Dict[str, Any]]:
        try:
            ...
            if stale_sec > 0:
                with get_db_connection() as db:
                    cur = db.cursor()
                    # 1. Dead-letter stale processing that exceeded max_attempts
                    cur.execute(
                        """
                        UPDATE pending_orders
                        SET status = 'failed',
                            updated_at = NOW(),
                            dispatch_note = 'dead_letter_max_attempts'
                    ...
                    # 2. Requeue stale processing that have remaining attempts
                    cur.execute(
                        """
                        UPDATE pending_orders
                        SET status = 'pending',
    ...
    def _dispatch_one(self, order_row: Dict[str, Any]) -> None:
        ...
        if mode == "paper":
            self._execute_paper_order(order_id=order_id, order_row=order_row, payload=payload)
            return
    ...
    def _execute_paper_order(self, *, order_id: int, order_row: Dict[str, Any], payload: Dict[str, Any]) -> None:
        ...
        from app.services.pending_orders.live_order_support import build_live_order_context, LiveOrderRejected
        from app.services.live_trading.contracts import OrderIntent
        ...
        if ctx.exchange_id != "paper":
            self._mark_failed(order_id=order_id, error="Execution mode invariant violated: paper order routed to live exchange config")
            return
            
        adapter = PaperBrokerAdapter()
        intent = OrderIntent(
            symbol=ctx.symbol,
            side="buy" if "long" in ctx.signal_type else "sell",
            quantity=ctx.amount,
            price=payload.get("limit_price") or payload.get("price") or 0.0,
            exchange_config=ctx.exchange_config,
            market_type=ctx.market_type,
            client_order_id=ctx.client_order_id or f"paper_{order_id}",
        )

        order_type = payload.get("order_type", "market")
        if order_type == "limit":
            order_res = adapter.place_limit_order(intent)
        else:
            order_res = adapter.place_market_order(intent)

        fill = adapter.wait_for_fill(intent, order_id=order_res.exchange_order_id)
        
        # Now we need to reconcile back to Postgres!
        # The worker handles durable state and reconciliation.
        from app.services.pending_orders.fill_records import persist_strategy_fill
        
        fee_ccy = next(iter(fill.fees_by_ccy.keys())) if fill.fees_by_ccy else "USD"
        fee_amt = fill.fees_by_ccy.get(fee_ccy, 0.0)

        profit, _ = persist_strategy_fill(
            ...
            market_type=ctx.market_type,
            order_id=order_id,
            fill_source="worker_paper_sim",
            commission=fee_amt,
            commission_ccy=fee_ccy,
            commission_quote=fill.fees_by_ccy.get("USD", 0.0),
            close_reason="agent_paper_trade",
            user_id=order_row.get("user_id"),
            strategy_run_id=order_row.get("strategy_run_id", 0),
            order_intent_id=order_row.get("order_intent_id", 0),
            exchange_id="paper",
            exchange_order_id=order_res.exchange_order_id,
            raw_fill=fill.raw,
        )

        status = "filled" if fill.status == "closed" else "partially_filled" if fill.filled_qty > 0 else "sent"
        ...
        with get_db_connection() as db:
            ...
            cur.execute(
                ...
                WHERE id = %s
                """,
                (status, fill.filled_qty, fill.avg_price, order_id)
            )
            db.commit()
            cur.close()
    ...
    def _execute_live_order(self, *, order_id: int, order_row: Dict[str, Any], payload: Dict[str, Any]) -> None:
        ...
        if exchange_id == "paper":
            error = "Execution mode invariant violated: live order routed to paper exchange config"
            self._mark_failed(order_id=order_id, error=error)
            _console_print(f"[worker] order rejected: {error}")
            _notify_live_best_effort(status="failed", error=error)
            append_strategy_log(strategy_id, "error", error)
            return
            
        
        # Phase 6: Enforce live trading kill switch at execution boundary
        import os
        is_live_enabled = os.getenv("AGENT_LIVE_TRADING_ENABLED", "false").lower() in ("1", "true", "yes")
        if not is_live_enabled:
            error = "agentLiveTradingDisabled: Live trading is globally disabled on this server."
            self._mark_failed(order_id=order_id, error=error)
            _console_print(f"[worker] order rejected: strategy_id={strategy_id} pending_id={order_id} {error}")
            _notify_live_best_effort(status="failed", error=error)
            append_strategy_log(strategy_id, "error", error)
            return
        ...
        try:
            ...
            if type(client).__name__ == "CalculatedRiskDummyClient":
                try:
                    user_id = int(order_row.get("user_id") or payload.get("user_id") or 0)
                    if user_id > 0:
                        broker_credentials = CredentialService.load_by_exchange(user_id, exchange_id)
                except Exception as exc:
                    logger.warning("Failed to load broker_credentials for %s (exc: %s)", exchange_id, exc)

            AdapterClass = CalculatedRiskAdapter if type(client).__name__ == "CalculatedRiskDummyClient" else LiveOrderPhaseAdapter
            ...
            if execution_algo == "limit":
                ...
                # Layer 3 Pre-Submit Reconciliation + Ledger
                reconciled = self._log_execution_attempt_start(order_id, intent.client_order_id)
                if reconciled:
                    _console_print(f"[worker] Layer 3 Reconciliation matched order: {intent.client_order_id}")
                    if hasattr(adapter, "find_order_by_client_id"):
                        try:
                            recovered = adapter.find_order_by_client_id(intent.client_order_id)
                            if recovered:
                                execution_result = OrderExecutionResult(
                                    success=True,
                                    exchange_order_id=recovered.get("id", ""),
                                    raw=recovered
                                )
                            else:
                                execution_result = MarketOrderExecutor(adapter).execute(intent)
                        except Exception as e:
                            # FAIL-SAFE: If we know an attempt was made, and we cannot verify if the broker accepted it,
                            # we MUST NOT resubmit. Blindly resubmitting here risks double-execution.
                            raise RuntimeError(f"Generic reconciliation lookup failed: {e}")
                    else:
                        execution_result = MarketOrderExecutor(adapter).execute(intent)
                else:
                    try:
                        execution_result = MarketOrderExecutor(adapter).execute(intent)
                        self._log_execution_attempt_end(
                            pending_order_id=order_id,
                            client_order_id=intent.client_order_id,
                            status="confirmed" if execution_result.success else "failed",
                            broker_order_id=execution_result.exchange_order_id or "",
                            error=execution_result.error or ""
                        )
                    except TimeoutError as e:
                        self._log_execution_attempt_end(order_id, intent.client_order_id, "timeout", error=str(e))
                        self._mark_failed(order_id, error="broker_timeout")
                        return
    ...
    def _execute_ibkr_order(
        ...
        *,
        ...
        try:
            ...
            # Layer 3 Reconciliation + Ledger for IBKR
            client_oid_val = intent_client_oid if 'intent_client_oid' in locals() else f"qd_{strategy_id}_{order_id}"
            reconciled = self._log_execution_attempt_start(order_id, client_oid_val)
            result = None
            if reconciled:
                _console_print(f"[worker] Layer 3 Reconciliation matched order: {client_oid_val}")
                if hasattr(client, "find_order_by_client_id"):
                    try:
                        rec = client.find_order_by_client_id(client_oid_val)
                        if rec:
                            result = type("OrderResult", (), {"success": True, "order_id": rec.get("id", ""), "filled": 0.0, "avg_price": 0.0, "message": "recovered", "raw": rec})()
                    except Exception as e:
                        raise RuntimeError(f"IBKR reconciliation lookup failed for {client_oid_val}: {e}")
            ...
            if not result:
                try:
                    ...
                    self._log_execution_attempt_end(
                        pending_order_id=order_id,
                        client_order_id=client_oid_val,
                        status="confirmed" if result.success else "failed",
                        broker_order_id=result.order_id or "",
                        error=result.message or ""
                    )
                except TimeoutError as e:
                    self._log_execution_attempt_end(order_id, client_oid_val, "timeout", error=str(e))
                    self._mark_failed(order_id, error="broker_timeout")
                    return
    ...
    def _execute_alpaca_order(
        ...
        *,
        ...
        try:
            ...
            protection_kwargs = {}
            # Layer 3 Reconciliation + Ledger for Alpaca
            client_oid_val = f"qd_{strategy_id}_{order_id}"
            reconciled = self._log_execution_attempt_start(order_id, client_oid_val)
            result = None
            if reconciled:
                _console_print(f"[worker] Layer 3 Reconciliation matched order: {client_oid_val}")
            ...
            if not result:
                try:
                    ...
                    self._log_execution_attempt_end(
                        pending_order_id=order_id,
                        client_order_id=f"qd_{strategy_id}_{order_id}",
                        status="confirmed" if result.success else "failed",
                        broker_order_id=result.order_id or "",
                        error=result.message or ""
                    )
                except TimeoutError as e:
                    self._log_execution_attempt_end(order_id, f"qd_{strategy_id}_{order_id}", "timeout", error=str(e))
                    self._mark_failed(order_id, error="broker_timeout")
                    return
    ...
    def _mark_deferred(self, order_id: int, reason: str) -> None:
        with get_db_connection() as db:
            ...
            db.commit()
            cur.close()
    def _log_execution_attempt_start(self, pending_order_id: int, client_order_id: str) -> bool:
        """
        Records that an execution attempt is starting.
        Returns True if a previous successful execution attempt for this client_order_id already exists (Layer 3 Recon).
        """
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            # Layer 3: Check if it already exists and is confirmed
            cur.execute(
                "SELECT status, broker_order_id, attempt_number FROM execution_attempts WHERE pending_order_id = %s AND client_order_id = %s ORDER BY attempt_number DESC LIMIT 1",
                (pending_order_id, client_order_id)
            )
            row = cur.fetchone()
            next_attempt = 1
            if row:
                status_val = row.get("status") if isinstance(row, dict) else row[0]
                if status_val in ('confirmed', 'submitted'):
                    return True
                last_attempt = row.get("attempt_number") if isinstance(row, dict) else row[2]
                next_attempt = (last_attempt or 1) + 1
                
            cur.execute(
                """
                INSERT INTO execution_attempts (pending_order_id, client_order_id, attempt_number, status)
                VALUES (%s, %s, %s, 'submitted')
                """,
                (pending_order_id, client_order_id, next_attempt)
            )
            db.commit()
            return False

    def _log_execution_attempt_end(self, pending_order_id: int, client_order_id: str, status: str, broker_order_id: str = "", error: str = "") -> None:
        """
        ...
        with get_db_connection() as db:
            ...
            cur.execute(
                ...
                SET status = %s, broker_order_id = %s, error = %s, response_received_at = NOW()
                WHERE pending_order_id = %s AND client_order_id = %s AND status = 'submitted'
                """,
                (status, broker_order_id, error, pending_order_id, client_order_id)
            )
            db.commit()
