...
def _get_okx_public_json(path: str) -> dict:
    ...
    try:
        response = requests.get(url, timeout=30, headers={"User-Agent": "CalculatedRisk/1.0"})
        ...
        for address in _resolve_ipv4_with_doh(hostname):
            ...
            try:
                response = pool.request(
                    "GET",
                    path,
                    headers={"Host": hostname, "User-Agent": "CalculatedRisk/1.0"},
                )
    ...
    for url, params in providers:
        try:
            response = requests.get(
                ...
                params=params,
                timeout=10,
                headers={"Accept": "application/dns-json", "User-Agent": "CalculatedRisk/1.0"},
            )
            response.raise_for_status()
...
def fetch_moex_symbols() -> List[SymbolMasterRow]:
    ...
    try:
        resp = requests.get(
            ...
            params={"iss.meta": "off"},
            timeout=20,
            headers={"User-Agent": "CalculatedRisk/1.0"},
        )
        resp.raise_for_status()
