...
def to_calculatedrisk_symbol(broker_symbol: str, exchange: str, token=None) -> str:
    """Best-effort mapping of a broker symbol to CalculatedRisk symbol format.
...
class BaseOrderUpdateAdapter(ABC):
    ...
    def normalize(self, raw_message: Any) -> dict | None:
        ...
        Use CalculatedRisk's common field names/vocabulary (see
        docs/prompt/order-constants.md and docs/prompt/services_documentation.md
        "Payload rules"), not broker-native ones:
            orderid, symbol (CalculatedRisk format — map broker symbol via
...
class PollingOrderUpdateAdapter:
    ...
    (auth_token + broker direct-auth path — already returns CalculatedRisk common
