...
notification_config = {
  "channels": ["browser", "email", "phone", "telegram", "discord", "webhook", "whatsapp"],
  "targets": {
...
def _signal_payload_text(payload: Dict[str, Any]) -> str:
    ...
    lines = [
        f"CalculatedRisk Signal - {symbol} - {' '.join([action, side]).strip()}",
...
def _profile_test_text(payload: Dict[str, Any]) -> str:
    title = str((payload or {}).get("title") or "CalculatedRisk notification test")
...
class SignalNotifier:
            ...
            try:
                if c == "browser":
                    ...
                    ok, err = self._notify_telegram(
                        ...
                        token_override=str(token or ""),
                        parse_mode="HTML",
                    )
                    results["telegram"] = {"ok": ok, "error": err}
                elif c == "whatsapp":
                    recipient = (targets.get("whatsapp") or "").strip()
                    token = (
                        targets.get("whatsapp_token")
                        or targets.get("whatsapp_bot_token")
                        or cfg.get("whatsapp_token")
                        or cfg.get("whatsapp_bot_token")
                    )
                    phone_id = (
                        targets.get("whatsapp_phone_number_id")
                        or cfg.get("whatsapp_phone_number_id")
                    )
                    ok, err = self._notify_whatsapp(
                        recipient=recipient,
                        text=rendered.get("whatsapp_text") or message_plain,
                        token_override=str(token or ""),
                        phone_id_override=str(phone_id or ""),
                    )
    ...
    def _render_messages(self, payload: Dict[str, Any]) -> Dict[str, str]:
        ...
        telegram_lines = [
            f"<b>CalculatedRisk Signal</b> | <code>{html.escape(symbol or '-')}</code>",
        ...
        telegram_lines.append("<i>Generated by CalculatedRisk signal engine.</i>")
        telegram_html = "\n".join([x for x in telegram_lines if x is not None])

        # WhatsApp (Markdown) message
        whatsapp_lines = [
            f"*{title}*",
            "",
            f"*Strategy*: {t_strategy}",
            f"*Symbol*: {symbol}",
            f"*Signal*: {stype}",
            f"*Reference price*: {price_s}",
            f"*Stake*: {stake_s}",
        ]
        if pending_id:
            whatsapp_lines.append(f"*Pending order*: {pending_id}")
        if mode:
            whatsapp_lines.append(f"*Mode*: {mode}")
        if ts_disp:
            whatsapp_lines.append(f"*{ts_lbl}*: {ts_disp}")
        whatsapp_lines.append("")
        whatsapp_lines.append("_Generated by CalculatedRisk signal engine._")
        whatsapp_text = "\n".join([x for x in whatsapp_lines if x is not None])

        # Email (HTML) message. Keep inline CSS for maximum compatibility.
        email_html = self._build_email_html(
            title_text="CalculatedRisk Signal",
    ...
    <div style="max-width:680px;margin:0 auto;padding:28px 18px;font-family:Inter,Arial,sans-serif;">
      <div style="background:#141821;border:1px solid #252b38;border-radius:8px;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,.35);">
        ...
        <div style="padding:14px 18px;color:#9ca3af;font-size:12px;border-top:1px solid #252b38;background:#0f131a;">
          Generated by CalculatedRisk signal engine. Review execution mode and exchange account before acting.
        </div>
      </div>
    </div>
  </body>
...
"""
    ...
    def _notify_webhook(
        ...
        *,
        ...
        URL hosts recognised automatically:
          ...
          - Generic     everything else (keeps CalculatedRisk's own schema)
        ...
        headers: Dict[str, str] = {
            ...
            "User-Agent": "CalculatedRisk/1.0 (+https://www.calculatedrisk.com)",
        }
        ...
        if signing_secret and dialect == 'feishu':
            ...
            # Original CalculatedRisk contract: body-bound HMAC-SHA256 hex
    ...
    def _notify_discord(self, *, url: str, payload: Dict[str, Any], fallback_text: str) -> Tuple[bool, str]:
        ...
        if str((payload or {}).get("event") or "") == "qd.profile_test":
            embed = {
                ...
                "color": 0x22C55E,
                "footer": {"text": "CalculatedRisk notification channel check"},
            }
        ...
        embed: Dict[str, Any] = {
            "title": f"CalculatedRisk Signal | {str(instrument.get('symbol') or '-')}",
            ...
            "footer": {"text": "Generated by CalculatedRisk signal engine"},
        }
        if payload.get("timestamp_iso"):
            embed["timestamp"] = str(payload.get("timestamp_iso") or "")
        if trace.get("pending_order_id"):
            embed["footer"] = {"text": f"Generated by CalculatedRisk signal engine | pending_order_id={int(trace.get('pending_order_id'))}"}
        return self._post_discord_embed(url=url, embed=embed, fallback_text=fallback_text)
    ...
    def _notify_whatsapp(
        ...
        *,
        ...
        token = (token_override or "").strip()
        phone_id = (phone_id_override or "").strip()
        
        if not token:
            return False, "missing_whatsapp_token (Please configure WhatsApp API Token)"
        if not phone_id:
            return False, "missing_whatsapp_phone_number_id (Please configure WhatsApp Phone ID)"
        if not recipient:
            return False, "missing_whatsapp_recipient"
        ...
        try:
            data: Dict[str, Any] = {
                ...
                "text": {
                    "preview_url": False,
                    "body": str(text or "")[:4096]
                }
            }
            resp = requests.post(url, headers=headers, json=data, timeout=self.timeout_sec)
            if 200 <= resp.status_code < 300:
                return True, ""
            return False, f"http_{resp.status_code}:{(resp.text or '')[:300]}"
        except Exception as e:
    ...
    def send_profile_test_notifications(
        ...
        *,
        ...
        title = "CalculatedRisk 通知测试" if zh else "CalculatedRisk notification test"
        plain = (
            "这是一条来自 CalculatedRisk 个人中心「通知设置」的测试消息。若您收到本条消息，说明该渠道配置正确。"
            if zh
            else "This is a test message from CalculatedRisk profile notification settings. "
            "If you received this, the channel is configured correctly."
        )
        html_body = f"<p>{html.escape(plain)}</p>"
        telegram_html = f"<b>{html.escape(title)}</b>\n\n{html.escape(plain)}\n\n<i>CalculatedRisk notification channel check passed.</i>"
        whatsapp_text = f"*{title}*\n\n{plain}\n\n_CalculatedRisk notification channel check passed._"
    ...
    <div style="max-width:640px;margin:0 auto;padding:28px 18px;font-family:Inter,Arial,sans-serif;">
      <div style="background:#141821;border:1px solid #252b38;border-radius:8px;overflow:hidden;">
        <div style="padding:22px 24px;background:linear-gradient(135deg, rgba(59,130,246,.20), rgba(34,197,94,.10));border-bottom:1px solid #252b38;">
          <div style="font-size:12px;color:#9ca3af;font-weight:700;text-transform:uppercase;">CalculatedRisk Notification</div>
        ...
        for ch in ch_list:
            ...
            try:
                if c == "browser":
                    ...
                    recipient = str((targets or {}).get("whatsapp") or "").strip()
                    token = (
                        (targets or {}).get("whatsapp_token")
                        or (targets or {}).get("whatsapp_bot_token")
                    )
                    phone_id = (
                        (targets or {}).get("whatsapp_phone_number_id")
                    )
                    ok, err = self._notify_whatsapp(
                        recipient=recipient,
                        text=whatsapp_text,
