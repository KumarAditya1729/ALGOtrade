...
VALID_NOTIFICATION_CHANNELS = {"browser", "email", "telegram", "discord", "webhook", "phone", "whatsapp"}
...
def update_notification_settings(user_id: int, data: dict[str, Any]) -> dict:
    ...
    settings = {
        ...
        "whatsapp_token": str(data.get("whatsapp_token") or data.get("whatsapp_bot_token") or "").strip(),
        "whatsapp_phone_number_id": str(data.get("whatsapp_phone_number_id") or "").strip(),
...
def send_test_notification(user_id: int, accept_language: str = "") -> tuple[bool, str, dict]:
    ...
    targets = {
        ...
        "whatsapp": (settings.get("whatsapp") or "").strip(),
        "whatsapp_token": (settings.get("whatsapp_token") or "").strip(),
        "whatsapp_phone_number_id": (settings.get("whatsapp_phone_number_id") or "").strip(),
        "email": notify_email,
