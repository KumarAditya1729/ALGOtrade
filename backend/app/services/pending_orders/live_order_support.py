...
def build_live_order_context(
    *,
    ...
    if strategy_id < 0:
        raise LiveOrderRejected(error="invalid_strategy_id")
    ...
    # Phase 6.1: Enforce execution mode as the authoritative invariant
    if execution_mode == "paper":
        exchange_config = {"exchange_id": "paper", "paper": True}
    else:
        if str(exchange_config.get("exchange_id") or "").strip().lower() == "paper":
            raise LiveOrderRejected(
                error="Execution mode invariant violated: live order routed to paper exchange config",
                strategy_id=strategy_id,
                console_message=f"[worker] order rejected: strategy_id={strategy_id} pending_id={order_id} invariant violation",
                strategy_log="Execution mode invariant violated: live order routed to paper exchange config"
            )
