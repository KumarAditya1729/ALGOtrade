"""Central registry for CalculatedRisk Copilot skills.
...
_SKILLS: tuple[SkillDefinition, ...] = (
    SkillDefinition(
        ...
        system_instruction="Use CalculatedRisk market snapshots first; do not invent live data; include triggers, invalidation, and risk controls.",
    ...
    SkillDefinition(
        ...
        description=_s("生成只用于图表展示的 CalculatedRisk 指标代码、参数和可视化标记", "Generate chart-only CalculatedRisk indicator code, parameters, and visual markers"),
        prompt_template=_s(
            "请基于 {symbol_label} 做一次指标研发。我的想法/偏好：\n希望观察的结构或信号：\n希望显示的标记/图层：\n可调参数：\n不希望出现的视觉干扰：\n请生成适合 CalculatedRisk 指标编辑器的图表指标草稿。指标只用于看图，不用于回测或实盘执行。",
            "Run indicator research for {symbol_label}. My preferences:\nStructures or signals to visualize:\nMarkers/layers I want:\nTunable parameters:\nVisual clutter to avoid:\nGenerate a CalculatedRisk Indicator editor draft. Indicators are chart-only and are not used for backtest or live execution.",
        ),
        system_instruction="Use CalculatedRisk indicator contracts for chart-only visualization. Do not output executable strategy code or Pine Script unless explicitly requested.",
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            "请设计一个 Strategy API V2 策略，策略源码负责标的、市场、周期、调度和交易逻辑。仅确认真正缺失的需求，再生成可运行的 CalculatedRisk Python 代码。",
            "Design a Strategy API V2 strategy. The source owns its universe, market, frequency, schedule, and trading logic. Confirm genuinely missing requirements before generating runnable CalculatedRisk Python.",
        ),
        system_instruction=(
            "Use CalculatedRisk Strategy API V2 exclusively. The source owns instruments, market, subscriptions, "
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            "请帮我检查 CalculatedRisk 当前部署是否完整：LLM/API Key、数据源、券商账户、积分余额、网络代理、通知配置。请按问题现象、可能原因、配置入口、验证方式输出。",
            "Check whether my CalculatedRisk deployment is complete: LLM/API key, data source, broker account, credits, proxy, and notifications. Output symptoms, causes, where to configure, and how to verify.",
        ),
...
_EXTRA_SKILLS: tuple[SkillDefinition, ...] = (
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            "请检查 CalculatedRisk 当前数据源配置是否足够支撑我的任务。请按行情、K线、成交量、财务、新闻、衍生品数据分别说明可用性、缺口、配置入口和验证方式。",
            "Check whether current CalculatedRisk data sources can support my task. Cover price, klines, volume, fundamentals, news, and derivatives data with gaps, config entry, and verification.",
        ),
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            "请使用 CalculatedRisk 系统数据源查询 {symbol_label} 的行情上下文。优先读取最新价格、1H/4H/1D K线、成交量、关键高低点和数据缺口；如果标的不明确，请先识别候选标的，不要直接套用下拉框。",
            "Use CalculatedRisk data sources to retrieve market context for {symbol_label}. Prefer latest price, 1H/4H/1D klines, volume, key highs/lows, and data gaps. If the symbol is ambiguous, resolve candidates first instead of forcing the dropdown symbol.",
        ),
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            "请先读取 CalculatedRisk 指标开发契约，再帮我设计指标。必须说明输入、输出、参数、绘图元素、信号标记和验证步骤。",
            "Read the CalculatedRisk indicator authoring contract first, then design the indicator. Cover inputs, outputs, parameters, drawings, signals, and validation steps.",
        ),
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            "请帮我验证这段指标代码是否符合 CalculatedRisk 指标 IDE 规范。请指出语法问题、契约问题、运行风险和修改建议。",
            "Validate this indicator code against CalculatedRisk Indicator IDE rules. Report syntax issues, contract mismatches, runtime risks, and fixes.",
        ),
    ...
    SkillDefinition(
        ...
        description=_s("配置 CalculatedRisk MCP Server、连接外部 AI 客户端并验证工具", "Configure CalculatedRisk MCP Server, connect external AI clients, and verify tools"),
        prompt_template=_s(
            "请帮我配置 CalculatedRisk MCP Server。请根据我的客户端说明环境变量、Agent Token 权限、连接方式、可用工具、验证步骤和常见故障。",
            "Help configure CalculatedRisk MCP Server. Based on my client, explain env vars, Agent Token scopes, transport, available tools, verification, and troubleshooting.",
        ),
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            "请帮我做一次 CalculatedRisk 发布体检。请检查前端、后端、Docker、数据库迁移、LLM、数据源、MCP、回测和实盘安全边界。",
            "Run a CalculatedRisk release health check. Cover frontend, backend, Docker, migrations, LLM, data sources, MCP, backtesting, and live safety boundaries.",
        ),
