...
def fetch_balance_raw(
    ...
    *,
    ...
    try:
        ...
        if hasattr(client, "get_funds"):
            raw = client.get_funds()
            return parse_balance(raw, exchange_id, market_type)
...
def parse_balance(raw: Any, exchange_id: str, market_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(raw, dict):
            if "availablecash" in raw:
                result["available"] = float(raw.get("availablecash") or 0)
                result["total"] = result["available"] + float(raw.get("collateral") or 0)
                result["currency"] = "INR"
                return result
