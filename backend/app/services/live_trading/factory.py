...
def create_client(exchange_config: Dict[str, Any], *, market_type: str = "swap") -> BaseRestClient:
    ...
    # Check if this is an CalculatedRisk broker
    try:
        module_path = f"app.services.live_trading.brokers.{exchange_id}.api.order_api"
        importlib.import_module(module_path)
        class CalculatedRiskDummyClient(BaseRestClient):
            def __init__(self, exchange_id: str, exchange_config: Dict[str, Any]):
            ...
            def get_positions(self, **kwargs):
                try:
                    mod = importlib.import_module(f"app.services.live_trading.brokers.{self._exchange_id}.api.order_api")
                    if hasattr(mod, "get_positions"):
                        # CalculatedRisk expects auth string (dummy)
                        res = mod.get_positions(self._exchange_config.get("api_key", ""))
            ...
            def get_orders(self, **kwargs):
                try:
                    mod = importlib.import_module(f"app.services.live_trading.brokers.{self._exchange_id}.api.order_api")
                    if hasattr(mod, "get_order_book"):
                        res = mod.get_order_book(self._exchange_config.get("api_key", ""))
                        return res.get("data", res) if isinstance(res, dict) else res
                except ImportError:
                    pass
                return []
            ...
            def get_holdings(self, **kwargs):
                try:
                    ...
                    if hasattr(mod, "get_holdings"):
                        res = mod.get_holdings(self._exchange_config.get("api_key", ""))
                        return res.get("data", res) if isinstance(res, dict) else res
                except ImportError:
                    pass
                return []
            ...
            def get_funds(self, **kwargs):
                try:
                    # In calculatedrisk funds are in funds.py
                    mod = importlib.import_module(f"app.services.live_trading.brokers.{self._exchange_id}.api.funds")
                    if hasattr(mod, "get_margin_data"):
                        res = mod.get_margin_data(self._exchange_config.get("api_key", ""))
                        return res.get("data", res) if isinstance(res, dict) else res
                except ImportError:
                    pass
                return {}
        return CalculatedRiskDummyClient(exchange_id, exchange_config)
    except ImportError:
        pass
