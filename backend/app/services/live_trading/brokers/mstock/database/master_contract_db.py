...
def convert_date(date_str):
    """
    Convert date format to CalculatedRisk expiry column format: DD-MMM-YY (with hyphens).
    Example: '19MAR2024' -> '19-MAR-24' or '2024-03-19' -> '19-MAR-24'

    CalculatedRisk Expiry Column Format: DD-MMM-YY (e.g., 28-MAR-24, 25-APR-24)
...
def fetch_and_process_mstock_indices():
    ...
    try:
        ...
        # Standardize NSE index symbols that differ from CalculatedRisk standard format
...
def process_mstock_json(json_data):
    """
    Processes the MStock JSON data to fit the CalculatedRisk database schema.
    ...
    # MStock API returns NSE/BSE for currency, but CalculatedRisk uses CDS/BCD
    # -------------------------------------------------------------------
    ...
    # Convert expiry dates to CalculatedRisk format (DD-MMM-YY)
    ...
    try:
        ...
        if bse_index_tokens:
            ...
            # Normalize BSE Index symbols to CalculatedRisk standard format
    ...
    # -------------------------------------------------------------------
    # Format F&O Symbols (CalculatedRisk Standard)
    ...
    # -------------------------------------------------------------------
    # Normalize instrumenttype to CalculatedRisk standard (match Angel format)
...
def master_contract_download():
    ...
    try:
        ...
        if not json_data:
            logger.error("No data received from MStock API.", "mstock").api_key
            socketio.emit(
