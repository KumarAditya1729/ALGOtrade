...
from app.services.live_trading.brokers.mstock.api.data import BrokerData
...
class MstockWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            auth_token = CredentialService.load_by_exchange(user_id, "mstock").api_key
            if not auth_token:
    ...
    def _get_fresh_auth_token(self) -> str | None:
        ...
        if not self.user_id:
            return None
        try:
            return CredentialService.load_by_exchange(self.user_id, "mstock").api_key
