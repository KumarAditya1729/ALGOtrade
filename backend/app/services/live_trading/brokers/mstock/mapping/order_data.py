...
def map_broker_exchange_to_calculatedrisk(broker_exchange, instrumenttype=""):
    """
    Maps mStock broker exchange to CalculatedRisk exchange format.
    mStock returns NSE for both equity and derivatives,
    but CalculatedRisk uses NFO for derivatives.
    ...
    Returns:
    - CalculatedRisk exchange format (e.g., 'NFO' for NSE derivatives)
    """
...
def map_order_data(order_data):
    ...
    Converts broker symbols to CalculatedRisk symbols and maps product types.
    ...
    if order_data:
        for order in order_data:
            ...
            # Get CalculatedRisk symbol from database using symboltoken
            ...
            # Map product types to CalculatedRisk format
...
def transform_order_data(orders):
    """
    Transforms mStock Type B order data to CalculatedRisk format.

    Parameters:
    - orders: List of order dictionaries from mStock Type B API

    Returns:
    - List of orders in CalculatedRisk format
    """
    ...
    for order in orders:
        ...
        # Normalize status to CalculatedRisk format (lowercase)
        ...
        transformed_order = {
            ...
            "exchange": map_broker_exchange_to_calculatedrisk(
...
def map_trade_data(trade_data):
    ...
    for trade in trade_data:
        ...
        if exchange:
            # First try: Get CalculatedRisk symbol using symboltoken
            if symboltoken:
                symbol_from_db = get_symbol(symboltoken, lookup_exchange)
                if symbol_from_db:
...
def transform_tradebook_data(tradebook_data):
    """
    Transforms mStock Type B tradebook data to CalculatedRisk format.

    Parameters:
    - tradebook_data: List of trade dictionaries from mStock Type B API

    Returns:
    - List of trades in CalculatedRisk format
    """
    transformed_data = []
...
def transform_positions_data(positions_data):
    """
    Transforms mStock Type B positions data to CalculatedRisk format.

    Parameters:
    - positions_data: List of position dictionaries (already mapped by map_position_data)

    Returns:
    - List of positions in CalculatedRisk format
    """
    transformed_data = []
...
def transform_holdings_data(holdings_data):
    """
    Transforms mStock Type B holdings data to CalculatedRisk format.
    ...
    Returns:
    - List of holdings in CalculatedRisk format
    """
    transformed_data = []
...
def map_portfolio_data(portfolio_data):
    ...
    # Convert to expected format for compatibility with CalculatedRisk
    if isinstance(data, list):
        ...
        for holding in data:
            ...
            # Get CalculatedRisk symbol from broker symbol
            if symbol:
