...
from app.services.live_trading.brokers.tradesmart.api.baseurl import post, resolve_uid
from app.services.live_trading.brokers.tradesmart.api.rate_limiter import (
    MAX_RETRIES,
    apply_rate_limit,
    is_rate_limit_error,
    retry_delay,
...
def _normalize_data_exchange(exchange):
    """Map CalculatedRisk index pseudo-exchanges to their parent cash exchange for data."""
...
class BrokerData:
    def __init__(self, auth_token):
        ...
        # CalculatedRisk interval -> TradeSmart TPSeries interval (minutes). 'D' -> EOD.
        self.timeframe_map = {
            "1m": "1",
            "3m": "3",
            "5m": "5",
    ...
    def _quote_dict(self, response):
        """Build the CalculatedRisk quote dict from a GetQuotes response."""
        return {
