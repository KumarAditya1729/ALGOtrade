...
OAuth-style flow:
  1. The user logs in on the TradeSmart portal and is redirected back to the
     CalculatedRisk callback with a ``code`` query param.
...
from app.services.live_trading.brokers.tradesmart.api.baseurl import GENACSTOK_URL, get_api_key
...
def authenticate_broker(code, password=None, totp_code=None):
    ...
    try:
        api_key = get_api_key()
        secret_key = get_broker_env("BROKER_API_SECRET")
