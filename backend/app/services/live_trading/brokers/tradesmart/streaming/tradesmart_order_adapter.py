...
from app.services.live_trading.brokers.tradesmart.api.baseurl import parse_auth, resolve_uid
...
# CalculatedRisk has only CNC/NRML/MIS, so F/H/B have no equivalent and fall through
...
def create_tradesmart_order_adapter(user_id: str) -> "TradeSmartOrderUpdateAdapter | None":
    ...
    stored_token = CredentialService.load_by_exchange(user_id, "tradesmart").api_key
    if not stored_token:
        logger.warning(
