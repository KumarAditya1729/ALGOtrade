# Mapping CalculatedRisk API Request to Delta Exchange API Parameters
...
def transform_data(data, token):
    """
    Transforms the CalculatedRisk API request structure to Delta Exchange POST /v2/orders payload.
...
def transform_modify_order_data(data):
    """
    Transforms CalculatedRisk modify order data to Delta Exchange PUT /v2/orders payload.
    ...
    CalculatedRisk pricetype mapping:
...
def map_order_type(pricetype):
    """Maps CalculatedRisk pricetype to Delta Exchange order_type string."""
    mapping = {
        "MARKET": "market_order",
        "LIMIT": "limit_order",
        "SL": "limit_order",
...
def reverse_map_product_type(br_product):
    """
    Maps Delta Exchange position category back to CalculatedRisk product type.
...
def map_exchange(br_exchange):
    """
    Maps a Delta Exchange broker exchange field back to CalculatedRisk exchange code.
...
def map_exchange_type(exchange):
    """
    Maps an CalculatedRisk exchange code to the Delta Exchange context.
