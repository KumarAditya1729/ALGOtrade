...
from app.services.live_trading.brokers.deltaexchange.api.baseurl import get_auth_headers, get_url
from app.services.live_trading.brokers.deltaexchange.mapping.transform_data import (
    map_exchange_type,
    map_product_type,
...
def get_api_response(endpoint, auth, method="GET", payload="", params=None):
    ...
    Args:
        ...
        auth:     api_key (BROKER_API_KEY stored in CalculatedRisk DB after login)
...
def get_positions(auth):
    ...
    appear in the CalculatedRisk position book alongside derivative positions.
    """
    positions = []
...
def _set_leverage(product_id: int, leverage: str, auth: str) -> None:
    ...
    """
    abort = get_broker_env("DELTA_ABORT_ON_LEVERAGE_FAILURE", "false").strip().lower() == "true"
...
def place_order_api(data, auth):
    ...
    if not leverage:
        try:
            from app.services.live_trading.calculatedrisk_compat import get_leverage
    ...
    if not leverage:
        leverage = get_broker_env("DELTA_DEFAULT_LEVERAGE", "")
...
def close_all_positions(current_api_key, auth):
    ...
    for pos in positions:
        ...
        # Resolve CalculatedRisk symbol from DB.
