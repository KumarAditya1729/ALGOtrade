...
from app.services.live_trading.brokers.deltaexchange.mapping.margin_data import parse_margin_response, transform_margin_positions
...
def calculate_margin_api(positions, auth):
    ...
    Args:
        positions: List of CalculatedRisk-format position dicts
            {symbol, exchange, action, quantity, product, price, pricetype}
        auth (str): api_key stored in the CalculatedRisk auth DB.

    Returns:
        Tuple of (MockResponse, response_data) matching CalculatedRisk broker interface.
    """
    api_key = auth
    api_secret = get_broker_env("BROKER_API_SECRET", "")
