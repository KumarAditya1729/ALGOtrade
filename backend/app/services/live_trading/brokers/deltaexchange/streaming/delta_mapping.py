...
class DeltaExchangeMapper:
    """Maps CalculatedRisk exchange codes to Delta Exchange equivalents.

    Delta Exchange uses plain symbol strings (e.g. "BTCUSD").
    All products trade on a single exchange named "CRYPTO" in CalculatedRisk.
    """

    # CalculatedRisk exchange code → Delta Exchange exchange code
    EXCHANGE_SEGMENTS = {
        "CRYPTO": "CRYPTO",
...
class DeltaModeMapper:
    """Maps CalculatedRisk subscription mode integers to Delta Exchange channel names."""

    # CalculatedRisk mode → Delta WS channel name
    MODE_CHANNELS = {
