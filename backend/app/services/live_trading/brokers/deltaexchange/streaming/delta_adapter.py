"""
delta_adapter.py
CalculatedRisk WebSocket adapter for Delta Exchange.
...
from app.services.live_trading.brokers.deltaexchange.streaming.delta_websocket import DeltaWebSocket
from app.services.live_trading.brokers.deltaexchange.streaming.delta_mapping import (
...
class DeltaWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if auth_data:
            ...
            # CalculatedRisk stores the api_key as the auth token
            api_key    = CredentialService.load_by_exchange(user_id, "deltaexchange").api_key or ""
            api_secret = get_broker_env("BROKER_API_SECRET", "")
    ...
    def _on_data(self, wsapp, msg: dict) -> None:
        ...
        try:
            ...
            # Find ALL CalculatedRisk subscriptions matching this broker symbol + channel.
    ...
    def _handle_private_event(self, event_type: str, msg: dict) -> None:
        ...
        CalculatedRisk service can subscribe to them via ZeroMQ:
    ...
    def _normalise_ticker(self, msg: dict, cache_key: str) -> dict:
        """
        Map v2/ticker fields to CalculatedRisk market data format.
    ...
    def _normalise_l2_orderbook(self, msg: dict, cache_key: str) -> dict:
        """
        Map l2_orderbook message to CalculatedRisk depth format.
