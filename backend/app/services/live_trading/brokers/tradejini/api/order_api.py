...
from app.services.live_trading.brokers.tradejini.api.auth_api import API_KEY_MISSING_ERROR, get_api_key
from app.services.live_trading.credentials.service import CredentialService
from app.services.live_trading.calculatedrisk_compat import get_br_symbol, get_oa_symbol, get_token
...
def get_order_book(auth):
    ...
    Returns:
        dict: Order book data in CalculatedRisk format
    """
    try:
        # Get API key from environment
        api_key = get_api_key()
        ...
        # Transform response data to CalculatedRisk format
        ...
        if response_data.get("s") == "ok":
            ...
            # Transform each order to CalculatedRisk format
            transformed_orders = []
            for order in response_data.get("d") or []:
                ...
                try:
                    ...
                    # Get CalculatedRisk symbol using the broker symbol id and exchange
                    calculatedrisk_symbol = get_oa_symbol(symbol_id, exchange)
                    if not calculatedrisk_symbol:
                        # Fallback to the broker symbol if the lookup misses
                        calculatedrisk_symbol = sym_trading_symbol(sym) or sym_base_symbol(sym)

                    transformed_order = {
                        "stat": "Ok",  # CalculatedRisk expects 'stat' field
                        "data": {
...
def get_trade_book(auth):
    ...
    Returns:
        dict: Trade book data in CalculatedRisk format {'data': [...], 'status': 'success'}
    """
    try:
        # Get API key from environment
        api_key = get_api_key()
        ...
        # Transform trades directly to CalculatedRisk format
        transformed_trades = []
        for trade in trades_data:
            try:
                ...
                # Get CalculatedRisk symbol
                calculatedrisk_symbol = None
                try:
                    calculatedrisk_symbol = get_oa_symbol(
                ...
                # Create transformed trade - match CalculatedRisk format exactly
                # Determine the symbol to use (CalculatedRisk symbol if available)
                final_symbol = ""
                if calculatedrisk_symbol:
                    final_symbol = calculatedrisk_symbol
                else:
                    # Fallback to exchange symbol if CalculatedRisk symbol isn't available
                ...
                transformed_trade = {
                    ...
                    "symbol": final_symbol,  # Using CalculatedRisk symbol here
...
def get_positions(auth):
    ...
    Returns:
        dict: Positions data in CalculatedRisk format
    """
    try:
        # Get API key from environment
        api_key = get_api_key()
        ...
        if response_data.get("s") == "ok":
            ...
            for position in positions:
                try:
                    ...
                    # Get CalculatedRisk symbol - follow same approach as TradeBook implementation
                    calculatedrisk_symbol = None
                    try:
                        # First try with the symbol ID from sym object
                        symid_from_object = sym_id(sym)
                        if symid_from_object:
                            calculatedrisk_symbol = get_oa_symbol(symid_from_object, exchange)
                            logger.debug(
                                f"Symbol lookup with sym.id: {symid_from_object} -> {calculatedrisk_symbol}"
                            )

                        # If not found and we have the position symId, try that
                        if not calculatedrisk_symbol and symbol_id:
                            calculatedrisk_symbol = get_oa_symbol(symbol_id, "")
                            logger.debug(
                                f"Symbol lookup with position.symId: {symbol_id} -> {calculatedrisk_symbol}"
                            )

                        # If still not found, try with exchange symbol
                        if not calculatedrisk_symbol:
                            calculatedrisk_symbol = get_oa_symbol(exchange_symbol, exchange)
                            logger.debug(
                                f"Symbol lookup with exchange symbol: {exchange_symbol} -> {calculatedrisk_symbol}"
                            )
                    ...
                    if calculatedrisk_symbol:
                        final_symbol = calculatedrisk_symbol
                        logger.debug(f"Using CalculatedRisk symbol: {final_symbol}")
                    else:
                        # Fallback to exchange symbol if CalculatedRisk symbol isn't available
                    ...
                    # Format the position data according to CalculatedRisk format
                    # Removing tradingsymbol field as requested
                    transformed_position = {
                        "symbol": final_symbol,  # Use final symbol (CalculatedRisk or fallback)
                        "exchange": exchange,
                    ...
                    logger.debug(f"Position transformed: {tradingsymbol} → {calculatedrisk_symbol}")
            ...
            # Return in CalculatedRisk format - same pattern as orderbook and tradebook
...
def get_holdings(auth):
    ...
    Returns:
        dict: Holdings data in CalculatedRisk format
        {
            "data": {
                "holdings": [
                    {
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    try:
        ...
        # Check if this is already in CalculatedRisk format
        if positions_response.get("status") == "success" and "data" in positions_response:
            logger.debug("get_open_position - Processing CalculatedRisk format positions")
            ...
            for position in positions:
                ...
                logger.debug(
                    f"get_open_position - Checking CalculatedRisk position: {pos_symbol} on {pos_exch}, qty: {pos_qty}"
                )

                if pos_exch == exchange and pos_symbol == tradingsymbol and pos_qty != 0:
                    logger.debug(
                        f"get_open_position - Found matching CalculatedRisk position: {pos_symbol} with quantity {pos_qty}"
                    )
                    return str(pos_qty)

            logger.debug(
                f"get_open_position - No matching CalculatedRisk position found for {tradingsymbol} on {exchange}"
            )
            return "0"
    ...
    try:
        ...
        with symbol_lock:
            ...
            try:
                ...
                # Format response to match CalculatedRisk's expected format
                if (
                    response
...
def close_all_positions(current_api_key, auth):
    ...
    Returns:
        dict: Response with status and message in CalculatedRisk format
              {
    ...
    try:
        ...
        # Prepare final response in CalculatedRisk format
