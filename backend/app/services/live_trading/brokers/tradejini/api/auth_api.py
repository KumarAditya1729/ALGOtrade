...
def get_api_key():
    ...
    for name in ("BROKER_API_KEY", "BROKER_API_SECRET"):
        value = (get_broker_env(name) or "").strip()
...
def authenticate_broker(password=None, twofa=None, twofa_type=None):
    ...
    try:
        ...
        source = (
            "BROKER_API_KEY"
            if api_key == (get_broker_env("BROKER_API_KEY") or "").strip()
...
def get_auth_url():
    ...
    REDIRECT_URI = get_broker_env("REDIRECT_URI")
...
def authenticate_broker_oauth(code):
    try:
        ...
        data = {
            "code": code,
            "client_id": get_api_key(),
            "client_secret": get_broker_env("BROKER_API_SECRET"),
            "redirect_uri": get_broker_env("REDIRECT_URI"),
