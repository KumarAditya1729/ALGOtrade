...
from app.services.live_trading.brokers.tradejini.api.auth_api import get_api_key
...
class TradejiniWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            auth_token = CredentialService.load_by_exchange(user_id, "tradejini").api_key
    ...
    def _attempt_connection(self, delay: float) -> None:
        ...
        try:
            ...
            fresh_token = CredentialService.load_by_exchange(self.user_id, "tradejini").api_key
            if fresh_token:
                api_key = get_api_key()
