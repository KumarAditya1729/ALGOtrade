...
from app.services.live_trading.brokers.tradejini.mapping.transform_data import (
    reverse_map_order_type,
    reverse_map_product_type,
)
from app.services.live_trading.calculatedrisk_compat import get_oa_symbol, get_symbol
...
def map_order_data(order_data):
    ...
    if orders_data:
        for order in orders_data:
            ...
            # Map Tradejini status to CalculatedRisk status
            ...
            # them to the CalculatedRisk MARKET/LIMIT/SL/SL-M set.
...
def transform_order_data(orders):
    """
    Processes and modifies a list of order dictionaries into the final CalculatedRisk format.

    Parameters:
    - orders: List of orders with modified fields

    Returns:
    - Dictionary with orders in CalculatedRisk format
    """
    ...
    for order in orders:
        # Convert to CalculatedRisk format if needed
        transformed_order = {
...
def map_trade_data(trade_data):
    ...
    # Handle CalculatedRisk format with status and data fields
    if (
    ...
    if trades_data:
        for trade in trades_data:
            ...
            mapped_trade = {
                ...
                "sym_id": sym_id(symbol) or trade.get("symId", ""),  # For CalculatedRisk lookup
            }
...
def transform_tradebook_data(trades):
    """
    Transforms mapped trade data to CalculatedRisk format.

    Args:
        trades: List of mapped trade dictionaries or raw API response

    Returns:
        dict: Trade book data in CalculatedRisk format with {'data': [...], 'status': 'success'}
    """
    logger.debug(f"transform_tradebook_data - Input trades type: {type(trades)}")

    # Check if already in CalculatedRisk format
    if isinstance(trades, dict) and "status" in trades and "data" in trades:
        logger.debug("transform_tradebook_data - Already in CalculatedRisk format")
    ...
    for trade in trades:
        ...
        # Get CalculatedRisk symbol if possible
        try:
            # get_oa_symbol(brsymbol, exchange) - pass positionally, the first
            # parameter is the broker symbol id, not an CalculatedRisk symbol.
        ...
        transformed_trade = {
            ...
            "symbol": calculatedrisk_symbol or trading_symbol,
...
def map_position_data(position_data):
    ...
    for position in position_data:
        try:
            ...
            # Get CalculatedRisk symbol - follow same approach as the main implementation
            calculatedrisk_symbol = None
            try:
            ...
            if calculatedrisk_symbol:
                ...
                logger.info(f"Using CalculatedRisk symbol: {final_symbol}")
            else:
...
def transform_positions_data(positions_data):
    """
    Transform mapped position data to CalculatedRisk format.
    DEPRECATED: This function is kept for backward compatibility only.
...
def calculate_portfolio_statistics(holdings_data):
    """
    Aggregate holdings into the totals CalculatedRisk shows above the holdings table.
    """
...
def transform_holdings_data(holdings_data):
    """
    Transforms Tradejini holdings data to CalculatedRisk format.
    ...
    try:
        ...
        for holding in holdings_data:
            try:
                ...
                # Resolve to the CalculatedRisk symbol so holdings line up with the
                ...
                transformed_holding = {
                    ...
                    "symbol": (calculatedrisk_symbol or trade_symbol).strip(),
