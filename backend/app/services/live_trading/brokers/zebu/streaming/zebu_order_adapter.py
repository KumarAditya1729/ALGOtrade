...
Credentials: accesstoken = CredentialService.load_by_exchange(user_id) (Zebu stores the Noren
susertoken as the auth token, "zebu").api_key; uid/actid = the trading user id from the ":::"
...
def create_zebu_order_adapter(user_id: str) -> "ZebuOrderUpdateAdapter | None":
    ...
    susertoken = CredentialService.load_by_exchange(user_id, "zebu").api_key
    if not susertoken:
        logger.warning(
    ...
    full_api_key = get_broker_env("BROKER_API_KEY", "")
