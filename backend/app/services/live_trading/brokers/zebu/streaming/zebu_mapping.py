...
class ZebuExchangeMapper:
    """Maps between CalculatedRisk exchange names and Zebu exchange codes"""

    # CalculatedRisk to Zebu exchange mapping (same as Flattrade/Noren)
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    def to_zebu_exchange(cls, oa_exchange: str) -> str | None:
        """Convert CalculatedRisk exchange to Zebu exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, zebu_exchange: str) -> str | None:
        """Convert Zebu exchange to CalculatedRisk exchange format"""
