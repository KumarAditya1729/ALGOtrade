...
from app.services.live_trading.brokers.indmoney.mapping.order_data import normalize_order_status
...
def _canonical_order_id(raw_id, user_id):
    ...
    try:
        ...
        from app.services.live_trading.brokers.indmoney.api.order_api import get_order_book
        from app.services.live_trading.credentials.service import CredentialService

        auth = CredentialService.load_by_exchange(user_id)
        if not auth:
            return order_id
        ...
        book = get_order_book(auth, "indmoney").api_key or []
...
class IndmoneyOrderUpdateAdapter(BaseOrderUpdateAdapter):
    ...
    def _map_status(self, raw_status):
        """Map the stream's status code to an CalculatedRisk canonical status."""
