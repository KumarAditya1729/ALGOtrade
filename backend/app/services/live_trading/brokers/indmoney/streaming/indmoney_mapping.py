...
class IndmoneyExchangeMapper:
    """Maps CalculatedRisk exchange codes to INDmoney-specific segment codes"""
...
class IndmoneyModeMapper:
    ...
    # Mode mapping: CalculatedRisk mode number -> INDmoney mode string
    MODE_MAP = {
    ...
    def get_indmoney_mode(mode):
        """
        Convert CalculatedRisk mode number to INDmoney mode string

        Args:
            mode (int): CalculatedRisk mode (1: LTP, 2: Quote)
    ...
    @staticmethod
    def get_calculatedrisk_mode(indmoney_mode):
        """
        Convert INDmoney mode string to CalculatedRisk mode number

        Args:
            indmoney_mode (str): INDmoney mode ('ltp' or 'quote')

        Returns:
            int: CalculatedRisk mode number
        """
