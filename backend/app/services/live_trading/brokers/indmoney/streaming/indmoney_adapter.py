...
from app.services.live_trading.brokers.indmoney.streaming.indWebSocket import IndWebSocket
...
class IndmoneyWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            access_token = CredentialService.load_by_exchange(user_id, "indmoney").api_key
    ...
    def _fetch_fresh_token(self) -> str | None:
        ...
        if not self.user_id:
            return None
        return CredentialService.load_by_exchange(self.user_id, "indmoney").api_key
    ...
    def subscribe(
        ...
        #
        # Build this from the CalculatedRisk `exchange`, NOT `brexchange`. brexchange
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> dict[str, Any]:
        ...
        # built from the CalculatedRisk `exchange`, not the parent `brexchange`.
        instrument_token = IndmoneyExchangeMapper.create_instrument_token(exchange, token)
        if not instrument_token:
            return self._create_error_response(
                "UNSUPPORTED_EXCHANGE",
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            ...
            # Map INDmoney mode to CalculatedRisk mode string
