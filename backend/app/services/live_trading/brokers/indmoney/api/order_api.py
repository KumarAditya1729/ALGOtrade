...
from app.services.live_trading.brokers.indmoney.api.baseurl import get_url
from app.services.live_trading.brokers.indmoney.api.rate_limiter import rate_limited_request
from app.services.live_trading.brokers.indmoney.mapping.order_data import (
    OPEN_STATUSES,
    SMART_ORDER_TYPES,
    TRIGGER_PENDING_STATUSES,
    map_product_to_calculatedrisk,
    resolve_exchange,
)
from app.services.live_trading.brokers.indmoney.mapping.transform_data import (
    map_product_type,
    transform_data,
    transform_modify_order_data,
)
from app.services.live_trading.calculatedrisk_compat import get_br_symbol, get_symbol, get_token
...
# Scrip-code segment prefixes for the quote API, keyed by CalculatedRisk exchange.
...
def _get_cached_positions(auth):
    ...
    # Cache miss or expired - fetch from app.services.live_trading.brokers. The smart-order path only reads
...
def _position_exchange(position):
    """Resolve the CalculatedRisk exchange for one IndMoney position row."""
...
def get_open_position(tradingsymbol, exchange, product, auth):
    ...
    if all_positions and isinstance(all_positions, list):
        for position in all_positions:
            ...
            # Compare the exact CalculatedRisk exchange. The old code collapsed NFO to
...
def place_order_api(data, auth):
    ...
    if res.status_code == 200 or res.status_code == 201:
        if response_data and response_data.get("status") == "success":
            ...
            # Format response to match CalculatedRisk API standard
...
def close_all_positions(current_api_key, auth):
    ...
    if all_positions:
        ...
        for position in all_positions:
            ...
            product = map_product_to_calculatedrisk(
