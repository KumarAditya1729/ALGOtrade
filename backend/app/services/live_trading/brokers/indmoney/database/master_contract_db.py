...
def download_csv_indmoney_data(output_path):
    ...
    try:
        ...
        if auth_obj:
            from app.services.live_trading.calculatedrisk_compat import decrypt_token
...
def reformat_symbol(row, file_segment=None):
    """
    Reformat symbols according to CalculatedRisk standards based on Indmoney data structure
    """
    ...
    # Format expiry date for CalculatedRisk format (DDMMMYY)
...
def derive_name(row):
    ...
    TRADING_SYMBOL before the first hyphen, and fall back to the CalculatedRisk
...
def process_indmoney_csv(path):
    ...
    for segment in segments:
        ...
        # Generate CalculatedRisk formatted symbol
