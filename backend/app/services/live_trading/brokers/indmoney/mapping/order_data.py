...
# CalculatedRisk canonical statuses used by other brokers (e.g. Zerodha):
...
# "NSE"/"BSE" and `segment` is "EQUITY"/"DERIVATIVE". CalculatedRisk needs one code,
...
# How the broker's order types map to CalculatedRisk price types. The request
...
def resolve_exchange(security_id, exchange, segment, default="NSE"):
    """
    Resolve the CalculatedRisk exchange for an IndMoney order / trade / position row.
...
def map_product_to_calculatedrisk(product, exchange):
    """
    Map an IndMoney product to the CalculatedRisk product code.

    IndMoney: CNC / INTRADAY / MARGIN.  CalculatedRisk: CNC / MIS / NRML.
    """
...
def normalize_order_status(status):
    """
    Normalize an Indmoney order status to an CalculatedRisk canonical status.
    ...
    try:
        ...
        if order_data:
            for order in order_data:
                ...
                # Map product types
                order["productType"] = map_product_to_calculatedrisk(
...
def calculate_order_statistics(order_data):
    ...
    try:
        ...
        for order in order_data:
            ...
            # Normalize status to CalculatedRisk canonical value and count it
...
def transform_order_data(orders):
    try:
        ...
        for order in orders:
            ...
            # stop-loss order types, which is how CalculatedRisk should show them.
...
def map_trade_data(trade_data):
    ...
    try:
        ...
        for trade in trade_data:
            ...
            # Map product type from IndMoney format to CalculatedRisk format
            product = trade.get("product", "")
            trade["productType"] = map_product_to_calculatedrisk(product, exchange)
...
def transform_tradebook_data(tradebook_data):
    """
    Transform trade book data to CalculatedRisk standard format.
...
def transform_positions_data(positions_data):
    """
    Transform positions data to CalculatedRisk standard format.
    Matches the structure used by Angel broker for consistency.

    CalculatedRisk Standard Fields:
    ...
    try:
        ...
        for position in positions_data:
            ...
            # CalculatedRisk standard format (matching Angel broker structure)
            transformed_position = {
                ...
                ),  # Float as per CalculatedRisk standard
