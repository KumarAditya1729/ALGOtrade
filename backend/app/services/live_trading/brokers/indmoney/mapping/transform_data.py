...
from app.services.live_trading.brokers.indmoney.api.data import BrokerData
...
def transform_data(data, token):
    """
    Transforms the CalculatedRisk API request structure to Indmoney API structure.
    ...
    if data["pricetype"] == "MARKET":
        ...
        auth_token = CredentialService.load_by_exchange(username) if username else None
        ...
        if not auth_token:
            ...
            broker_data = BrokerData(auth_token, "indmoney").api_key
    ...
    # Basic mapping from CalculatedRisk to Indmoney
...
def transform_modify_order_data(data):
    """
    Transforms CalculatedRisk modify order data to Indmoney format.
    """
    transformed = {
...
def map_order_type(pricetype):
    """
    Maps CalculatedRisk pricetype to Indmoney order_type.
...
def map_segment(exchange):
    """
    Maps CalculatedRisk exchange to Indmoney segment.
    """
    segment_mapping = {
        "NSE": "EQUITY",
        "BSE": "EQUITY",
...
def map_exchange_type(exchange):
    """
    Maps CalculatedRisk exchange to Indmoney exchange format.
    """
    exchange_mapping = {
        "NSE": "NSE",
        "BSE": "BSE",
...
def map_exchange(br_exchange):
    """
    Maps Indmoney exchange back to CalculatedRisk exchange.
    """
...
def map_product_type(product):
    """
    Maps CalculatedRisk product type to Indmoney product type.
    """
...
def reverse_map_product_type(product):
    """
    Maps Indmoney product type back to CalculatedRisk product type.
    """
