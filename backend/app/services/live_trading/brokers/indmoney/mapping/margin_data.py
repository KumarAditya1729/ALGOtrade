...
def transform_margin_positions(positions):
    """
    Transform CalculatedRisk margin position format to IndMoney margin format.
...
def map_segment(exchange):
    ...
    CalculatedRisk: NSE, BSE, NFO, BFO, CDS, BCD, MCX
...
def parse_margin_response(response_data):
    """
    Parse IndMoney margin calculator response to CalculatedRisk standard format.

    Args:
        response_data: Raw response from IndMoney margin calculator API

    Returns:
        Standardized margin response matching CalculatedRisk format
    """
    try:
        ...
        # Extract only the three required values as per CalculatedRisk API specification
        total_margin = data.get("total_margin", 0)
        span_margin = data.get("span_margin", 0)
        exposure_margin = data.get("exposure_margin", 0)

        # Return standardized format matching CalculatedRisk API specification
