...
2. **`ReqType` decides which fields exist.** The docs' "Response body" table
   ...
   `ExchOrderID`. CalculatedRisk's 5Paisa orderbook keys on `BrokerOrderId`
...
Credentials: access_token = CredentialService.load_by_exchange(user_id, "fivepaisa").api_key; client_code = the third
...
from app.services.live_trading.brokers.fivepaisa.mapping.transform_data import (
    reverse_map_exchange,
    reverse_map_product_type,
)
from app.services.live_trading.brokers.fivepaisa.streaming.fivepaisa_websocket import (
...
# 5Paisa order statuses -> CalculatedRisk's lowercase order_status vocabulary.
...
def _pricetype(at_market, trigger_price: float) -> str:
    """Derive the CalculatedRisk pricetype the same way the REST orderbook does
...
def create_fivepaisa_order_adapter(user_id: str) -> "FivepaisaOrderUpdateAdapter | None":
    ...
    access_token = CredentialService.load_by_exchange(user_id, "fivepaisa").api_key
    if not access_token:
        logger.warning(
