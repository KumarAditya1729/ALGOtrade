...
Only order frames are published — CalculatedRisk has no position-update event, so
...
from app.services.live_trading.brokers.kotak.mapping.transform_data import map_exchange
...
# Kotak ordSt values (lowercase free text) -> CalculatedRisk's lowercase
...
# Kotak prcTp codes -> CalculatedRisk pricetype constants. "L" is what
...
class KotakOrderUpdateAdapter(BaseOrderUpdateAdapter):
    ...
    def normalize(self, raw_message):
        ...
        # canonical CalculatedRisk exchange, same as mapping/order_data.py does for
        ...
        return {
            ...
            "product": data.get("prod", ""),  # NRML/MIS/CNC already match CalculatedRisk
...
def create_kotak_order_adapter(user_id: str) -> "KotakOrderUpdateAdapter | None":
    ...
    """
    auth_token = CredentialService.load_by_exchange(user_id, "kotak").api_key
    if not auth_token:
        logger.warning(
