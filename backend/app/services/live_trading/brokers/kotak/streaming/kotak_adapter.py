...
class KotakWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """
    Adapter for Kotak WebSocket streaming, suitable for CalculatedRisk or similar frameworks.
    ...
    def __init__(self):
        ...
        # Mapping from Kotak format to CalculatedRisk format - critical for data flow
        self._kotak_to_calculatedrisk = {}  # {(kotak_exchange, token): (exchange, symbol)}
    ...
    def initialize(self, broker_name: str, user_id: str, auth_data=None):
        ...
        auth_string = CredentialService.load_by_exchange(user_id, "kotak").api_key
        if not auth_string:
    ...
    def _on_data_received(self, parsed_data):
        ...
        try:
            ...
            with self._lock:
                ...
                if (broker_exchange, token) in self._kotak_to_calculatedrisk:
            ...
            with self._lock:
                if mapping_key in self._kotak_to_calculatedrisk:
                    exchange, symbol = self._kotak_to_calculatedrisk[mapping_key]
    ...
    def disconnect(self):
        ...
        try:
            ...
            with self._lock:
                ...
                self._kotak_to_calculatedrisk.clear()
    ...
    def _recreate_ws_client(self):
        ...
        try:
            auth_string = CredentialService.load_by_exchange(self._user_id, "kotak").api_key
            if not auth_string:
                logger.error(
    ...
    def unsubscribe(self, symbol, exchange, mode):
        ...
        try:
            ...
            with self._lock:
                ...
                from app.services.live_trading.brokers.kotak.streaming.kotak_mapping import get_kotak_exchange
    ...
    def subscribe_quote(self, exchange, symbol, mode):
        ...
        try:
            ...
            with self._lock:
                mapping_key = (kotak_exchange, str(token))
                self._kotak_to_calculatedrisk[mapping_key] = (exchange, symbol)
            ...
            with self._lock:
                ...
                if mapping_key in self._symbol_modes:
                    ...
                    if not self._symbol_modes[mapping_key]:
                        self._kotak_to_calculatedrisk.pop(mapping_key, None)
