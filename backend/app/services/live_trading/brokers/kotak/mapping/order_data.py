...
def _calculatedrisk_symbol(row, exchange):
    """Resolve one Kotak row to its CalculatedRisk symbol.
    ...
    downstream lookup that expects an CalculatedRisk symbol. brsymbol in the master
    ...
    logger.debug(
        f"No CalculatedRisk symbol for token '{token}' / symbol '{broker_symbol}' on {exchange}. "
...
def map_order_data(order_data):
    ...
    if order_data:
        for order in order_data:
            ...
            symbol_from_db = _calculatedrisk_symbol(order, exchange)
            if symbol_from_db:
