...
from app.services.live_trading.brokers.paytm.streaming.paytm_websocket import PaytmWebSocket
...
class PaytmWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            public_access_token = CredentialService.load_by_exchange(user_id)
        ...
        # adapter (see _on_close, "paytm").access_token, not in PaytmWebSocket itself.
    ...
    def _refresh_feed_token(self) -> None:
        ...
        CredentialService.load_by_exchange() in initialize(). get_feed_token(, "paytm").access_token does not support
