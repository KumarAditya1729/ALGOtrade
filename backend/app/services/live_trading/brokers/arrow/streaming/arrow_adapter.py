"""Arrow WebSocket adapter -> CalculatedRisk unified streaming.
...
master contract). The publish topic EXCHANGE_SYMBOL_MODE keeps the CalculatedRisk
...
from app.services.live_trading.brokers.arrow.streaming.arrow_mapping import ArrowCapabilityRegistry
from app.services.live_trading.brokers.arrow.streaming.arrow_websocket import ArrowWebSocket
...
# Arrow subscription mode -> CalculatedRisk topic suffix. The proxy fans a higher mode
...
class ArrowWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(self, broker_name, user_id, auth_data=None):
        try:
            ...
            app_id = get_broker_env("BROKER_API_KEY")
            if auth_data and auth_data.get("token"):
                access_token = auth_data["token"]
            else:
                access_token = CredentialService.load_by_exchange(user_id, "arrow").api_key
