...
from app.services.live_trading.brokers.arrow.api.baseurl import WS_MARKET_DATA_URL
...
class ArrowWebSocket:
    ...
    def __init__(self, app_id, access_token, on_ticks=None, user_id=None):
        ...
        self.token_exchange_map: dict[int, str] = {}  # token -> CalculatedRisk exchange
    ...
    def _refresh_access_token(self):
        ...
        if not self.user_id:
            return False
        try:
            token = CredentialService.load_by_exchange(self.user_id, "arrow").api_key
            if not token:
