...
#
# Central exchange-code translation between CalculatedRisk and Arrow, plus index
...
#   NSECO           NSE commodities (CalculatedRisk exchange NCO)
...
# Arrow ExchSeg -> CalculatedRisk exchange. Verified against the live CSV.
ARROW_EXCHSEG_TO_OA = {
    "NSECM": "NSE",
    "BSECM": "BSE",
    "NSEFO": "NFO",
...
# CalculatedRisk exchange -> Arrow exchange code used by the /info/quote* endpoints.
...
# CalculatedRisk exchange -> Arrow historical-API path segment (lowercase). Indices
...
# ("Nifty 50", "BSE IT", "CrudeOil"). CalculatedRisk standardizes these to the
...
# NSE index display names (normalized) -> CalculatedRisk symbol.
_NSE_INDEX_MAP = {
    # Major indices
    "NIFTY50": "NIFTY",
...
# BSE index short codes (normalized CSV `Symbol`) -> CalculatedRisk symbol.
_BSE_INDEX_MAP = {
    "SENSEX": "SENSEX",
    "BANKEX": "BANKEX",
    "SENSEX50": "SENSEX50",
...
# MCX iCOMDEX index names (normalized CSV `Symbol`) -> CalculatedRisk symbol.
...
# CalculatedRisk name yet, so they get a consistent MCX-prefixed symbol.
_MCX_INDEX_MAP = {
...
def to_arrow_quote_exchange(oa_exchange):
    """CalculatedRisk exchange -> Arrow quote exchange (indices -> INDEX)."""
    return _OA_TO_ARROW_QUOTE.get(oa_exchange, oa_exchange)


def to_arrow_history_exchange(oa_exchange):
    """CalculatedRisk exchange -> Arrow historical path segment (lowercase)."""
...
def arrow_exchange_to_oa(exchseg, segment=None, exchange=None):
    """Map an Arrow instrument's raw exchange fields to an CalculatedRisk exchange.
