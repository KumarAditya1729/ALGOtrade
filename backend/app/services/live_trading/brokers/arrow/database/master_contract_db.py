...
#
# Builds the CalculatedRisk SymToken table from Arrow's instrument master.
#
...
# standardized to the documented CalculatedRisk index symbols in mapping/exchange.py.
...
from app.services.live_trading.brokers.arrow.api.baseurl import INSTRUMENTS_URL, ROOT_URL, get_arrow_headers
from app.services.live_trading.brokers.arrow.mapping.exchange import (
    ARROW_EXCHSEG_TO_OA,
    OA_INDEX_EXCHANGES,
    classify_index_symbol,
)
from app.services.live_trading.credentials.service import CredentialService
from app.services.live_trading.calculatedrisk_compat import db_session as auth_db_session
...
def _broker_auth_token():
    ...
    CalculatedRisk is single-user, so there is at most one.
    """
    login_username = get_broker_env("LOGIN_USERNAME")
    if login_username:
        token = CredentialService.load_by_exchange(login_username, "arrow").api_key
        if token:
            return token

    auth_obj = Auth.query.filter_by(broker="arrow", is_revoked=False).first()
    if auth_obj:
        return CredentialService.load_by_exchange(auth_obj.name)
    return None
...
def download_arrow_instruments(auth_token):
    ...
    headers = get_arrow_headers(auth_token, "arrow").api_key
...
# CalculatedRisk exchanges whose instruments can be futures (used by the
...
def process_arrow_csv(df):
    ...
    # NOTE: CalculatedRisk stores indices with instrumenttype "EQ" (verified against
    ...
    # "CrudeOil"); standardize to the documented CalculatedRisk index symbols.
    ...
    # Drop rows that failed to map to a valid CalculatedRisk exchange.
        ...
        rows.append(
            {
                ...
                "strike": 0.0,
                "lotsize": 0,
                # CalculatedRisk stores indices as instrumenttype "EQ" (the
