...
from app.services.live_trading.brokers.arrow.api.baseurl import HISTORICAL_URL, ROOT_URL, get_arrow_headers
from app.services.live_trading.brokers.arrow.database.master_contract_db import SymToken, db_session
...
class BrokerData:
    def __init__(self, auth_token):
        ...
        # CalculatedRisk timeframe -> Arrow interval string. Keys are what
    ...
    def _lookup(self, symbol, exchange):
        """Resolve an CalculatedRisk (symbol, exchange) to the Arrow instrument.
    ...
    def _quote_index(self, mode, symbol, br_symbol, token):
        ...
        Tries the CalculatedRisk symbol, then the uppercased display name, then the
    ...
    def get_quotes(self, symbol, exchange):
        """Return the CalculatedRisk quote dict. Uses Arrow `full` mode so bid/ask are
    ...
    def get_depth(self, symbol, exchange):
        """Return CalculatedRisk 5-level market depth. Indices supported via INDEX."""
        try:
            q = self._fetch_quote("full", symbol, exchange)
    ...
    @staticmethod
    def _format_quote(q):
        """Arrow FULL-quote payload -> CalculatedRisk quote dict (same shape as get_quotes)."""
        bids = q.get("bids") or [{}]
        asks = q.get("asks") or [{}]
        return {
            "ask": _scale(asks[0].get("price", 0)),
    ...
    def get_history(self, symbol, exchange, timeframe, from_date, to_date):
        """Historical candles -> CalculatedRisk DataFrame (timestamp, open, high, low,
