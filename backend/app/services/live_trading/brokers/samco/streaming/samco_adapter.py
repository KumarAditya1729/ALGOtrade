...
from app.services.live_trading.brokers.samco.streaming.samcoWebSocket import SamcoWebSocket
...
class SamcoWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            session_token = CredentialService.load_by_exchange(user_id, "samco").api_key
    ...
    def _get_index_listing_id(self, symbol: str, exchange: str) -> str:
        ...
        if not self._broker_data:
            auth_token = CredentialService.load_by_exchange(self.user_id, "samco").api_key
            if not auth_token:
        ...
        try:
            while self.running and self.reconnect_attempts < self.max_reconnect_attempts:
                try:
                    ...
                    fresh_token = CredentialService.load_by_exchange(self.user_id, "samco").api_key
                    if fresh_token:
