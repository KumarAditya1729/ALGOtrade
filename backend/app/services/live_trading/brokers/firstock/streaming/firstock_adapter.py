...
class FirstockWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            auth_token = CredentialService.load_by_exchange(user_id, "firstock").api_key
            ...
            if broker_api_key:
                ...
                # Fallback to CalculatedRisk user ID if BROKER_API_KEY not set
                self.firstock_user_id = user_id
                self.logger.warning(
                    f"BROKER_API_KEY not found in environment. Using CalculatedRisk user ID '{user_id}' which may fail authentication"
                )
    ...
    def _fetch_fresh_auth_token(self) -> str | None:
        ...
        if not self.user_id:
            return None
        try:
            fresh_token = CredentialService.load_by_exchange(self.user_id, "firstock").api_key
            if not fresh_token:
                self.logger.warning(
