...
# Database setup
DATABASE_URL = get_broker_env("DATABASE_URL")
...
def process_firstock_nse_data(output_path):
    """
    Processes the Firstock NSE data (NSE_symbols.csv) to generate CalculatedRisk symbols.
    ...
    # Apply transformation for CalculatedRisk symbols
    def get_calculatedrisk_symbol(broker_symbol):
    ...
    df["symbol"] = df["brsymbol"].apply(get_calculatedrisk_symbol)

    # For index rows, normalize to CalculatedRisk-standard symbol using the
    ...
    if index_mask.any():
        df.loc[index_mask, "symbol"] = df.loc[index_mask].apply(
            lambda r: map_to_calculatedrisk_index_symbol(
...
def process_firstock_nfo_data(output_path):
    """
    Processes the Firstock NFO data (NFO_symbols.csv) to generate CalculatedRisk symbols.
...
def process_firstock_bse_data(output_path):
    """
    Processes the Firstock BSE data (BSE_symbols.csv) to generate CalculatedRisk symbols.

    Indices (SENSEX, SENSEX50, etc.) are identified by empty ISIN + zero
    TickSize + zero FreezeQty (same heuristic as NSE) and routed to
    exchange='BSE_INDEX' with CalculatedRisk-standard symbol normalization.
    ...
    # For index rows, normalize to CalculatedRisk-standard symbol (e.g. SENSEX,
...
def process_firstock_bfo_data(output_path):
    """
    Processes the Firstock BFO data (BFO_symbols.csv) to generate CalculatedRisk symbols.
...
# CalculatedRisk standard index symbols per symbol_Openalgo.md.
# Matching any fetched idxname/tradingSymbol against these (case- and
# whitespace-insensitive) normalizes to the CalculatedRisk format; unmatched
...
# Explicit aliases for broker/exchange names that diverge from the CalculatedRisk
...
# Pre-computed normalized lookup dicts: normalized_key -> canonical CalculatedRisk symbol.
...
def _basic_index_cleanup(value):
    ...
    the stored symbol is a valid CalculatedRisk-format token.
    """
    if not value:
        return ""
...
def map_to_calculatedrisk_index_symbol(trading_symbol, idxname, br_exchange):
    """
    Resolve the CalculatedRisk-standard index symbol for a fetched Firstock index.

    Match order:
      1. tradingSymbol is already an CalculatedRisk canonical (NIFTY, SENSEX, ...)
      2. tradingSymbol/idxname normalizes to an explicit alias (e.g. BSEIT)
      3. tradingSymbol/idxname normalizes to a canonical CalculatedRisk symbol
...
def fetch_firstock_indices():
    ...
    from app.services.live_trading.calculatedrisk_compat import Auth, decrypt_token
    ...
    try:
        ...
        user_id = get_broker_env("BROKER_API_KEY", "").replace("_API", "")
        if not user_id:
        ...
        for item in indices:
            ...
            calculatedrisk_symbol = map_to_calculatedrisk_index_symbol(
