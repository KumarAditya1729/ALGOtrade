...
def transform_data(data, token, auth_token=None):
    """
    Transforms the CalculatedRisk API request to Firstock's V1 /placeOrder structure.

    For MARKET and SL-M orders, applies client-side Market Price Protection
    (MPP): fetch LTP, compute a protected price via the CalculatedRisk MPP slab,
    ...
    Args:
        data: Order data dictionary (CalculatedRisk format).
    ...
    raw_apikey = data.get("apikey") or get_broker_env("BROKER_API_KEY", "")
    ...
    if data["pricetype"] in ("MARKET", "SL-M"):
        ...
        try:
            if auth_token:
                ...
                from app.services.live_trading.brokers.firstock.api.data import BrokerData
...
def transform_modify_order_data(data, token, auth_token=None):
    ...
    used by transform_data so the price is owned by CalculatedRisk. When we can't
    ...
    Expects data["symbol"] to be in CalculatedRisk format; the broker symbol is
...
def map_order_type(pricetype):
    """
    Maps the CalculatedRisk pricetype to Firstock's order type.
    """
...
def map_product_type(product):
    """
    Maps the CalculatedRisk product type to Firstock's product type.
    """
...
def reverse_map_product_type(product):
    """
    Maps Firstock's product type to CalculatedRisk product type.
    """
