...
Credentials: accesstoken = CredentialService.load_by_exchange(user_id) (Flattrade's jKey session
token, "flattrade").api_key; uid/actid = the trading user id from BROKER_API_KEY, which is always
...
# Noren order-status free text -> CalculatedRisk's lowercase order_status vocabulary.
...
# Noren prctyp codes -> CalculatedRisk pricetype constants. The doc lists only
...
# Noren product codes -> CalculatedRisk product constants
...
def create_flattrade_order_adapter(user_id: str) -> "FlattradeOrderUpdateAdapter | None":
    ...
    accesstoken = CredentialService.load_by_exchange(user_id, "flattrade").api_key
    if not accesstoken:
        logger.warning(
    ...
    api_key = get_broker_env("BROKER_API_KEY", "")
