...
class FlattradeExchangeMapper:
    """Maps between CalculatedRisk exchange names and Flattrade exchange codes"""

    # CalculatedRisk to Flattrade exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    def to_flattrade_exchange(cls, oa_exchange: str) -> str | None:
        """Convert CalculatedRisk exchange to Flattrade exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, flattrade_exchange: str) -> str | None:
        """Convert Flattrade exchange to CalculatedRisk exchange format"""
