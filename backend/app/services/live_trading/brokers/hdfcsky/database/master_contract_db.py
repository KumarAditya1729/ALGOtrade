...
#
# Builds the CalculatedRisk SymToken table from HDFC Sky's Security Master.
#
...
from app.services.live_trading.brokers.hdfcsky.api.baseurl import SECURITY_MASTER_URL, USER_AGENT
...
# into CalculatedRisk's NSE_INDEX / BSE_INDEX.
...
# CalculatedRisk standardizes both to the symbols documented in
...
# BSE short codes (the CSV's `trading_symbol` for IDX rows) -> CalculatedRisk symbol.
_BSE_INDEX_MAP = {
    "SENSEX": "SENSEX",
    "BANKEX": "BANKEX",
    "SNSX50": "SENSEX50",
...
def classify_index_symbol(display_name, oa_exchange):
    """HDFC Sky index display name -> the CalculatedRisk index symbol.
...
def process_security_master(df):
    ...
    # Derivatives -> the CalculatedRisk common format.
    ...
    # Indices -> the documented CalculatedRisk index symbols.
    if is_index.any():
        symbol.loc[is_index] = [
