...
from app.services.live_trading.brokers.hdfcsky.mapping.transform_data import PRODUCT_NUMERIC_CODES
from app.services.live_trading.calculatedrisk_compat import get_logger

logger = get_logger(__name__)

# CalculatedRisk exchange -> HDFC Sky `segment` (proto Segment enum).
_SEGMENT_MAP = {
    "NSE": "Capital",
    "BSE": "Capital",
    "NSE_INDEX": "Capital",
...
def build_margin_leg(position, row, series_type, underlying_price):
    ...
    Args:
        position: the CalculatedRisk position dict (symbol, exchange, action,
...
def parse_margin_response(result):
    """HDFC Sky margin `result` block -> the CalculatedRisk standard payload.
