...
#
# HDFC Sky's vocabulary is very close to CalculatedRisk's -- the only real
...
from app.services.live_trading.brokers.hdfcsky.api.baseurl import get_client_id
from app.services.live_trading.calculatedrisk_compat import get_symbol_info, get_token
from app.services.live_trading.calculatedrisk_compat import get_logger
from app.services.live_trading.calculatedrisk_compat import calculate_protected_price
...
# ~182k rows). The CSV's `exchange` column already uses CalculatedRisk-compatible
# codes (NSE / BSE / NFO / BFO / CDS / MCX); the only split CalculatedRisk needs is
# pulling indices out into NSE_INDEX / BSE_INDEX, which the CSV flags via the
# `segment` column (NSE -> "INDICES", BSE -> "IDX").

# CalculatedRisk index exchanges (quote-only).
OA_INDEX_EXCHANGES = {"NSE_INDEX", "BSE_INDEX"}

# CalculatedRisk exchange -> the code HDFC Sky's REST endpoints (orders, positions,
...
# HDFC Sky REST exchange code -> CalculatedRisk exchange (normalizing responses).
...
# WebSocket scripId prefix per CalculatedRisk exchange. Straight from the docs'
...
def to_rest_exchange(oa_exchange):
    """CalculatedRisk exchange -> HDFC Sky REST exchange code."""
    return _OA_TO_HDFCSKY_REST.get(oa_exchange, oa_exchange)


def to_ltp_exchange(oa_exchange):
    """CalculatedRisk exchange -> the exchange code the /fetch-ltp endpoint expects.

    Unlike every other REST endpoint, fetch-ltp addresses indices by their own
    codes, "NSE_INDEX" and "BSE_INDEX", which happen to match the CalculatedRisk
...
# CalculatedRisk pricetype -> HDFC Sky `order_type`.
_ORDER_TYPE_MAP = {
    "MARKET": "MARKET",
    "LIMIT": "LIMIT",
    "SL": "SL",
...
# HDFC Sky `order_type` -> CalculatedRisk pricetype (normalizing responses).
...
def map_order_type(pricetype):
    """CalculatedRisk pricetype -> HDFC Sky order_type. Defaults to MARKET."""
...
def map_product_type(product):
    """CalculatedRisk product -> HDFC Sky product. Defaults to MIS."""
    return _PRODUCT_MAP.get(str(product).upper(), "MIS")


def reverse_map_product_type(exchange, product):
    """HDFC Sky product -> CalculatedRisk product. `exchange` is accepted for
...
def _protect_market_order(data, auth_token, pricetype, action, fallback_price):
    ...
    from app.services.live_trading.brokers.hdfcsky.api.data import BrokerData
...
def transform_data(data, auth_token):
    """CalculatedRisk order dict -> HDFC Sky place-order payload.

    CalculatedRisk order fields: symbol, exchange, action, pricetype, quantity,
...
def transform_modify_order_data(data, auth_token):
    """CalculatedRisk modify-order dict -> HDFC Sky modify payload.
