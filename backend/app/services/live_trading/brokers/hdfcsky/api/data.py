...
from app.services.live_trading.brokers.hdfcsky.api.baseurl import base_params, get_hdfcsky_headers, get_root_url
from app.services.live_trading.brokers.hdfcsky.database.master_contract_db import SymToken, db_session
from app.services.live_trading.brokers.hdfcsky.mapping.transform_data import (
...
class BrokerData:
    def __init__(self, auth_token):
        ...
        # the full CalculatedRisk interval set. The value is the pair (native
    ...
    def _lookup(self, symbol, exchange):
        """Resolve an CalculatedRisk (symbol, exchange) to its master-contract row."""
    ...
    def get_depth(self, symbol, exchange):
        """CalculatedRisk 5-level market depth.
    ...
    # CalculatedRisk exchanges whose instruments carry open interest. OI is served
    ...
    def _chart_symbols(row):
        ...
        master contract's:
          ...
          - Indices are inconsistent. The headline ones want the compact
            CalculatedRisk symbol ("NIFTY", "BANKNIFTY"; "NIFTY 50" yields nothing),
    ...
    def _fetch_candles(self, row, chart_type, start, end, symbol=None):
        ...
        trading day (zero on every earlier day), which CalculatedRisk does not carry,
