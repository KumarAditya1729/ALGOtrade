...
from app.services.live_trading.brokers.hdfcsky.api.baseurl import USER_AGENT, get_ws_url
from app.services.live_trading.brokers.hdfcsky.streaming import hdfcsky_market_pb2 as pb
...
class HDFCSkyWebSocket:
    ...
    def _refresh_access_token(self):
        ...
        if not self.user_id:
            return False
        try:
            token = CredentialService.load_by_exchange(self.user_id, "hdfcsky").api_key
            if not token:
