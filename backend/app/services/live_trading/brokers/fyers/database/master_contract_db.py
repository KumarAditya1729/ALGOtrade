...
def reformat_symbol_detail(s):
    ...
    # Reorder and format the parts to match the CalculatedRisk standard symbol format
...
def process_fyers_nse_csv(path):
    ...
    # Override specific symbols where normalized name differs from CalculatedRisk standard
...
def process_fyers_cds_json(path):
    ...
    # Map JSON fields to CalculatedRisk schema
    df["token"] = df["fyToken"]
