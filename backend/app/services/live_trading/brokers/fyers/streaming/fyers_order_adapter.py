...
# "status" numeric codes -> CalculatedRisk's lowercase order_status vocabulary.
...
_STATUS_MAP = {
    ...
    5: "rejected",
    6: "open",  # Pending
    7: "expired",  # no exact CalculatedRisk equivalent; passed through verbatim
}

# "type" numeric codes -> CalculatedRisk pricetype constants
_PRICETYPE_MAP = {1: "LIMIT", 2: "MARKET", 3: "SL-M", 4: "SL"}

# "side" numeric codes -> CalculatedRisk action constants
_ACTION_MAP = {1: "BUY", -1: "SELL"}

# (exchange, segment) numeric codes -> CalculatedRisk exchange constants.
...
class FyersOrderUpdateAdapter(BaseOrderUpdateAdapter):
    ...
    def normalize(self, raw_message):
        ...
        # CalculatedRisk exchange from (exchange, segment) codes; symbol via
        ...
        symbol = to_calculatedrisk_symbol(data.get("symbol", ""), exchange)
...
def create_fyers_order_adapter(user_id: str) -> "FyersOrderUpdateAdapter | None":
    ...
    access_token = CredentialService.load_by_exchange(user_id, "fyers").api_key
    if not access_token:
