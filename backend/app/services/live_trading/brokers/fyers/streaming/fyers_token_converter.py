...
Converts CalculatedRisk symbols to Fyers HSM format for WebSocket streaming
...
class FyersTokenConverter:
    ...
    def get_brsymbols_from_database(
        ...
        """
        Lookup brsymbols from database using CalculatedRisk symbol and exchange
        Uses the existing get_br_symbol function from the token database
        (which returns the broker's symbol like 'NSE:RELIANCE-EQ').
    ...
    def convert_calculatedrisk_symbols_to_hsm(
        self, symbol_info_list: list[dict], data_type: str = "SymbolUpdate"
    ) -> tuple[list[str], dict[str, str], list[str]]:
        """
        Convert CalculatedRisk symbols to HSM tokens using database lookup for brsymbols
        ...
        try:
            ...
            # self.logger.info(f"Converting CalculatedRisk symbols: {symbol_exchange_pairs}")
            ...
            self.logger.error(f"CalculatedRisk symbol conversion error: {e}")
    ...
    def convert_calculatedrisk_to_fyers_symbol(self, exchange: str, symbol: str) -> str:
        """
        Convert CalculatedRisk format (exchange, symbol) to Fyers symbol format
