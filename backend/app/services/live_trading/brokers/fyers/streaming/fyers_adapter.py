"""
Fyers WebSocket Adapter for CalculatedRisk
...
class FyersAdapter:
    """
    Fyers WebSocket adapter for CalculatedRisk streaming service
    Follows CalculatedRisk adapter pattern similar to Angel, Zerodha etc.
    """
    ...
    def subscribe_symbols(self, symbols: list[dict[str, str]], data_type: str, callback: Callable):
        ...
        try:
            with self.lock:
                ...
                self.logger.debug(
                    f"Converting {len(valid_symbols)} CalculatedRisk symbols to HSM format using database lookup..."
                )

                # Convert CalculatedRisk symbols directly to HSM tokens using database lookup
                hsm_tokens, token_mappings, invalid_symbols = (
                    self.token_converter.convert_calculatedrisk_symbols_to_hsm(valid_symbols, data_type)
                )
                ...
                # Build the HSM<->CalculatedRisk mapping by JOINING through brsymbol.
                #
                ...
                # map from valid_symbols lets us recover the correct CalculatedRisk
                ...
                brsymbol_to_calculatedrisk: dict[str, tuple[str, str]] = {}
                for s in valid_symbols:
                    br = get_br_symbol(s["symbol"], s["exchange"])
                    if br:
                        brsymbol_to_calculatedrisk[br] = (s["exchange"], s["symbol"])
                ...
                for hsm_token in hsm_tokens:
                    ...
                    if not brsym:
                        ...
                        continue
                    pair = brsymbol_to_calculatedrisk.get(brsym)
                    if not pair:
                        self.logger.warning(
    ...
    def _on_message(self, fyers_data: dict[str, Any]):
        ...
        try:
            ...
            # Map to CalculatedRisk format first to get symbol info
            ...
            callback = None
            calculatedrisk_data_type = "Quote"  # Default
            ...
            for cb, calculatedrisk_data_type in dispatches:
                ...
                if calculatedrisk_data_type == "Depth":
