"""
Fyers Data Mapping
Maps Fyers HSM data to CalculatedRisk format for compatibility
"""
...
class FyersDataMapper:
    """
    Maps Fyers HSM WebSocket data to CalculatedRisk format
    """
    ...
    def map_to_calculatedrisk_ltp(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to CalculatedRisk LTP format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            CalculatedRisk LTP format dict or None if mapping fails
        """
        try:
            ...
            # Map to CalculatedRisk LTP format
            calculatedrisk_data = {
            ...
            return calculatedrisk_data

        except Exception as e:
            logger.debug(f"Error mapping LTP data: {e}")
            return None

    def map_to_calculatedrisk_quote(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to CalculatedRisk Quote format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            CalculatedRisk Quote format dict or None if mapping fails
        """
        try:
            if not fyers_data:
                return None
            ...
            # Map to CalculatedRisk Quote format
            calculatedrisk_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_to_calculatedrisk_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers depth data to CalculatedRisk Depth format

        Args:
            fyers_data: Raw depth data from Fyers HSM WebSocket

        Returns:
            CalculatedRisk Depth format dict or None if mapping fails
        """
        try:
            ...
            # Map to CalculatedRisk Depth format (matching other brokers)
            calculatedrisk_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_tbt_depth_to_calculatedrisk(
        self, ticker: str, tbt_depth: dict[str, Any], symbol: str, exchange: str
    ) -> dict[str, Any] | None:
        """
        Map Fyers TBT 50-level depth data to CalculatedRisk Depth format

        Args:
            ticker: Original Fyers ticker (e.g., 'NSE:RELIANCE-EQ')
            tbt_depth: Depth data from TBT WebSocket
            symbol: CalculatedRisk symbol name
            exchange: CalculatedRisk exchange name
        ...
        try:
            ...
            # Map to CalculatedRisk Depth format
            calculatedrisk_data = {
                "symbol": symbol,
                "exchange": exchange,
                "token": ticker,
    ...
    def map_index_to_synthetic_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers index data to synthetic CalculatedRisk Depth format
    ...
    def map_fyers_data(
        ...
        """
        Map Fyers data to appropriate CalculatedRisk format based on requested type
        ...
        Returns:
            Mapped CalculatedRisk data or None if mapping fails
        """
        if not fyers_data:
            return None
        ...
        if requested_type == "LTP":
            return self.map_to_calculatedrisk_ltp(fyers_data)
        elif requested_type == "Quote":
            return self.map_to_calculatedrisk_quote(fyers_data)
        elif requested_type == "Depth" and fyers_type == "dp":
            return self.map_to_calculatedrisk_depth(fyers_data)
    ...
    def extract_symbol_info(self, symbol: str) -> dict[str, str]:
        """
        Extract exchange and symbol from CalculatedRisk format
