"""
Fyers WebSocket Adapter for CalculatedRisk WebSocket Proxy
Integrates with the CalculatedRisk WebSocket proxy system
"""
...
class FyersWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Fyers-specific implementation of the WebSocket adapter for CalculatedRisk proxy"""
    ...
    def __init__(self):
        ...
        self.tbt_symbol_to_ticker = {}  # CalculatedRisk symbol -> Fyers ticker
        self.tbt_ticker_to_symbol = {}  # Fyers ticker -> CalculatedRisk symbol
    ...
    def initialize(
        ...
        try:
            ...
            if auth_data and "access_token" in auth_data:
                ...
                # Get from database
                auth_token = CredentialService.load_by_exchange(user_id, "fyers").api_key
                if not auth_token:
    ...
    def subscribe(self, symbol: str, exchange: str, mode: int = 2, depth_level: int = 5):
        ...
        try:
            ...
            with self.lock:
                # Convert to CalculatedRisk format
    ...
    def _subscribe_tbt_depth(
        ...
        Args:
            symbol: CalculatedRisk symbol (actual symbol without suffix)
            exchange: Exchange name
    ...
    def _unsubscribe_tbt_depth(self, symbol: str, exchange: str) -> bool:
        ...
        Args:
            symbol: CalculatedRisk symbol (may include :50 suffix)
            exchange: Exchange name
    ...
    def _convert_to_fyers_ticker(self, symbol: str, exchange: str) -> str | None:
        """
        Convert CalculatedRisk symbol to Fyers ticker format using database lookup

        Args:
            symbol: CalculatedRisk symbol (e.g., 'RELIANCE', 'NIFTY24DEC25000CE')
    ...
    def _on_tbt_depth_update(self, ticker: str, depth_data: dict[str, Any]):
        ...
        try:
            ...
            # Map to CalculatedRisk format
            symbol = subscription["symbol"]
            exchange = subscription["exchange"]

            self.logger.debug(f"Mapping TBT depth for {exchange}:{symbol}")

            mapped_data = self.data_mapper.map_tbt_depth_to_calculatedrisk(
    ...
    def _map_fyers_to_calculatedrisk(
        self, fyers_data: dict[str, Any], mode: int
    ) -> dict[str, Any] | None:
        """
        Map Fyers data to CalculatedRisk WebSocket format
        ...
        Returns:
            Mapped data in CalculatedRisk format
        """
        try:
            if not fyers_data:
                return None
            ...
            if mode == 1:  # LTP
                ...
                calculatedrisk_data.update({"ltp": converted_ltp, "data_type": "LTP"})
            elif mode == 2:  # Quote
