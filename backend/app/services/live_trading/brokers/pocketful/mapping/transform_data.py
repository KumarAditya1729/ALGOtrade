...
from app.services.live_trading.brokers.pocketful.api.data import BrokerData
from app.services.live_trading.calculatedrisk_compat import get_br_symbol, get_symbol_info, get_token
...
def transform_data(data, client_id=None, auth_token=None):
    """
    Transforms CalculatedRisk order format to Pocketful order format.
    ...
    Args:
        data: CalculatedRisk order data dictionary
...
def transform_modify_order_data(data, client_id=None):
    """
    Transforms CalculatedRisk order modification format to Pocketful order format.
...
def map_order_type(pricetype):
    """
    Maps CalculatedRisk pricetype to Pocketful order type.
    """
    order_type_mapping = {
        "MARKET": "MARKET",
        "LIMIT": "LIMIT",
...
def map_product_type(product):
    """
    Maps CalculatedRisk product type to Pocketful product type.
    """
...
def reverse_map_product_type(exchange, product):
    """
    Reverse maps the broker product type to the CalculatedRisk product type, considering the exchange.
    """
    # Exchange to CalculatedRisk product type mapping for 'D'
    exchange_mapping = {
        "CNC": "CNC",
        "NRML": "NRML",
        "MIS": "MIS",
