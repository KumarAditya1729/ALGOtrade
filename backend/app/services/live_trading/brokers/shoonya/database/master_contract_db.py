...
def drop_incomplete_rows(df, exchange, required=("brsymbol", "name")):
    """Drops rows that are missing the fields needed to build an CalculatedRisk symbol.
...
def process_shoonya_nse_data(output_path):
    """
    Processes the shoonya NSE data (NSE_symbols.txt) to generate CalculatedRisk symbols.
    ...
    def get_calculatedrisk_symbol(broker_symbol):
        ...
        if "-EQ" in broker_symbol:
            ...
            # For other symbols (including index), CalculatedRisk symbol remains the same as broker symbol
...
def process_shoonya_nfo_data(output_path):
    """
    Processes the shoonya NFO data (NFO_symbols.txt) to generate CalculatedRisk symbols.
...
def process_shoonya_cds_data(output_path):
    """
    Processes the shoonya CDS data (CDS_symbols.txt) to generate CalculatedRisk symbols.
    Handles both futures and options formatting.
    """
...
def process_shoonya_mcx_data(output_path):
    """
    Processes the shoonya MCX data (MCX_symbols.txt) to generate CalculatedRisk symbols.
    Handles both futures and options formatting.
    """
...
def process_shoonya_bse_data(output_path):
    """
    Processes the shoonya BSE data (BSE_symbols.txt) to generate CalculatedRisk symbols.
    ...
    # Apply transformation for CalculatedRisk symbols (no special logic needed here)
    def get_calculatedrisk_symbol(broker_symbol):
        return broker_symbol
...
def process_shoonya_bfo_data(output_path):
    """
    Processes the shoonya BFO data (BFO_symbols.txt) to generate CalculatedRisk symbols and correctly extract the name column.
