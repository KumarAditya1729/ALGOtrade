...
Credentials: accesstoken = CredentialService.load_by_exchange(user_id, "shoonya").api_key (Shoonya stores the Noren
...
def create_shoonya_order_adapter(user_id: str) -> "ShoonyaOrderUpdateAdapter | None":
    ...
    susertoken = CredentialService.load_by_exchange(user_id, "shoonya").api_key
    if not susertoken:
        logger.warning(
