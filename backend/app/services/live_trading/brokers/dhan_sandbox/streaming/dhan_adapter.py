"""
Fixed Dhan WebSocket adapter for CalculatedRisk.
...
from app.services.live_trading.calculatedrisk_compat import get_token
from websocket_proxy.base_adapter import BaseBrokerWebSocketAdapter

from .dhan_mapping import get_dhan_exchange, get_calculatedrisk_exchange
...
def _get_dhan_client_id() -> str | None:
    ...
    broker_api_key = get_broker_env("BROKER_API_KEY")
...
class DhanWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    Implements CalculatedRisk WebSocket proxy interface with proper mode mapping.
    """
    ...
    def __init__(self):
        ...
        # Extended mode mapping to handle all possible CalculatedRisk modes
        self.mode_map = {
            # Standard CalculatedRisk modes
    ...
    def initialize(self, broker_name: str, user_id: str, **kwargs):
        ...
        try:
            ...
            self.access_token = get_broker_env("BROKER_API_SECRET")
            self.logger.info(
    ...
    def subscribe(
        ...
        try:
            ...
            # Map CalculatedRisk mode to Dhan mode with explicit handling of all possible modes
            if mode == 1:
                # Standard LTP mode
                dhan_mode = DhanWebSocket.MODE_LTP
                self.logger.info(f"Mapped CalculatedRisk mode {mode} (LTP) to Dhan mode '{dhan_mode}'")
            elif mode == 2:
                # Standard QUOTE mode
                dhan_mode = DhanWebSocket.MODE_QUOTE
                self.logger.info(f"Mapped CalculatedRisk mode {mode} (QUOTE) to Dhan mode '{dhan_mode}'")
            elif mode == 3:
                # Standard DEPTH mode
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(f"Mapped CalculatedRisk mode {mode} (DEPTH) to Dhan mode '{dhan_mode}'")
            else:
                # All other modes (4-8) map to FULL/DEPTH
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(
                    f"Mapped CalculatedRisk mode {mode} (DEPTH/FULL) to Dhan mode '{dhan_mode}'"
                )

            # Add symbol to subscription tracking
            with self.lock:
                self.subscribed_symbols[symbol] = {
                    "exchange": exchange,  # Store original CalculatedRisk exchange
            ...
            # Map CalculatedRisk exchange to Dhan exchange code
    ...
    def _on_ticks(self, ticks: list[dict[str, Any]]):
        ...
        try:
            ...
            for tick in ticks:
                ...
                # Convert to CalculatedRisk format for data field
                ...
                # Normalize tick format to CalculatedRisk standard
    ...
    def _generate_topic(self, symbol: str, exchange: str, mode_str: str) -> str:
        ...
        Args:
            ...
            exchange: The exchange code in CalculatedRisk format
    ...
    def _normalize_tick(self, tick: dict[str, Any]) -> dict[str, Any]:
        """
        Normalize Dhan tick data to CalculatedRisk format.
        ...
        try:
            ...
            # Ensure exchange is in CalculatedRisk format
            exchange = tick.get("exchange")
            if exchange:
                calculatedrisk_exchange = get_calculatedrisk_exchange(exchange)
            else:
    ...
    def validate_tick_parsing(self, ticker_symbols=None, timeout=30):
        ...
        try:
            ...
            original_callbacks = {}
            for mode in [1, 2, 3, 4, 5]:  # CalculatedRisk modes
