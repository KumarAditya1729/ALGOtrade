...
AUTH_BASE_URL = get_broker_env("DHAN_AUTH_BASE_URL", "https://auth.dhan.co").rstrip("/")
API_BASE_URL = get_broker_env("DHAN_API_BASE_URL", "https://api.dhan.co").rstrip("/")
...
def _get_partner_credentials():
    partner_id = get_broker_env("BROKER_PARTNER_ID")
    partner_secret = get_broker_env("BROKER_PARTNER_SECRET")
...
def authenticate_broker(code):
    """
    CalculatedRisk auth entrypoint for dhan_sandbox.
    ...
    """
    try:
        env_access_token = get_broker_env("BROKER_API_SECRET")
