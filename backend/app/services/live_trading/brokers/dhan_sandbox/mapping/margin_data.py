...
from app.services.live_trading.brokers.dhan_sandbox.mapping.transform_data import (
    map_exchange_type,
    map_order_type,
    map_product_type,
)
...
def transform_margin_position(position, client_id):
    """
    Transform a single CalculatedRisk margin position to Dhan Sandbox margin format.

    Note: Dhan Sandbox margin calculator API accepts only one order at a time, not a batch.

    Args:
        position: Position in CalculatedRisk format
...
def map_product_type_for_margin(product):
    """
    Maps CalculatedRisk product type to Dhan Sandbox product type for margin calculation.
...
def parse_margin_response(response_data):
    """
    Parse Dhan Sandbox margin response to CalculatedRisk standard format.
    ...
    try:
        ...
        # Return standardized format with CalculatedRisk-compatible aliases.
        return {
            "status": "success",
            "data": {
...
def parse_multi_margin_response(response_data):
    """
    Parse Dhan Sandbox /v2/margincalculator/multi response to CalculatedRisk standard format.
