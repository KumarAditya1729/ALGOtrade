...
from app.services.live_trading.brokers.compositedge.streaming.compositedge_websocket import CompositedgeWebSocketClient
...
class CompositedgeWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            auth_token = CredentialService.load_by_exchange(user_id, "compositedge").api_key
            feed_token = CredentialService.load_by_exchange(user_id)

            if not auth_token or not feed_token:
                self.logger.error(f"No authentication tokens found for user {user_id}")
                raise ValueError(f"No authentication tokens found for user {user_id}", "compositedge").access_token

            # For XTS, we need API key and secret, not just tokens
            # These should be stored in environment variables or config
            api_key = get_broker_env("BROKER_API_KEY_MARKET")
            api_secret = get_broker_env("BROKER_API_SECRET_MARKET")
            ...
            api_key = auth_data.get("api_key", get_broker_env("BROKER_API_KEY_MARKET"))
            api_secret = auth_data.get("api_secret", get_broker_env("BROKER_API_SECRET_MARKET"))
    ...
    def _process_json_data(self, data: dict):
        ...
        try:
            ...
            self.logger.debug(f"Environment ZMQ_PORT: {get_broker_env('ZMQ_PORT', 'Not Set')}")
