...
def normalize_bse_index_symbols(symbol_series: pd.Series) -> pd.Series:
    """Normalize raw BSE index symbols to CalculatedRisk naming and format."""
...
def process_compositedge_bse_csv(path):
    ...
    # Normalize BSE index short codes from SPOT rows to CalculatedRisk index symbols
...
def process_index_data(index_data):
    """
    Processes index data from API to fit the CalculatedRisk database schema.
    ...
    # NSE index mapping (raw broker index names -> CalculatedRisk symbols)
    nse_index_map = {
        "NIFTY 50": "NIFTY",
