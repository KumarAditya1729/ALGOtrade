...
from app.services.live_trading.calculatedrisk_compat import get_symbol_info
...
# CalculatedRisk exchange -> the `exchange` value InvestRight's REST endpoints want.
...
# (REST exchange, instrument_segment) -> CalculatedRisk exchange. This is what makes
...
def to_rest_exchange(oa_exchange):
    """CalculatedRisk exchange -> InvestRight REST `exchange` value."""
    return _OA_TO_IR_REST.get(oa_exchange, oa_exchange)


def to_ltp_exchange(oa_exchange):
    """CalculatedRisk exchange -> the exchange code /fetch-ltp expects.
...
def to_oa_exchange(ir_exchange, instrument_segment=None):
    """InvestRight (exchange, instrument_segment) -> CalculatedRisk exchange.
...
def instrument_segment(oa_exchange, instrumenttype, name, underlying=None):
    ...
    Args:
        oa_exchange: CalculatedRisk exchange (NSE, NFO, BFO, CDS, MCX, ...).
        instrumenttype: SymToken instrumenttype (EQ / FUT / CE / PE).
        name: the underlying's CalculatedRisk symbol (SymToken.name).
...
# CalculatedRisk pricetype -> InvestRight `order_type`. The profile endpoint reports
...
# CalculatedRisk product -> InvestRight product, per exchange class. Delivery only
...
def map_order_type(pricetype):
    """CalculatedRisk pricetype -> InvestRight order_type. Defaults to MARKET."""
...
def map_product_type(product, exchange="NSE"):
    """CalculatedRisk product -> InvestRight product. Defaults to INTRADAY."""
...
def reverse_map_product_type(exchange, product):
    """InvestRight product -> CalculatedRisk product.
...
def reverse_map_transaction_type(transaction_type):
    """InvestRight echoes the side title-cased ("Buy"); CalculatedRisk wants BUY."""
...
def transform_data(data):
    """CalculatedRisk order dict -> InvestRight place-order payload.
...
def transform_modify_order_data(data):
    """CalculatedRisk modify-order dict -> InvestRight modify payload.
