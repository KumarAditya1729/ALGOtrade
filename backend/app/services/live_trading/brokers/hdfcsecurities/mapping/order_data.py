...
# CalculatedRisk common format (docs/api/account-services/*).
#
...
from app.services.live_trading.brokers.hdfcsecurities.mapping.transform_data import (
...
# InvestRight order status -> CalculatedRisk lowercase status. Keys are normalized by
...
def _format_strike(strike):
    """Render a strike the way CalculatedRisk symbols embed it: 22400 or 262.5."""
    value = _float(strike)
...
def _is_known_underlying(name, exchange):
    ...
    from app.services.live_trading.brokers.hdfcsecurities.database.master_contract_db import SymToken, db_session
...
def _reconstruct_symbol(row, exchange):
    """Rebuild the CalculatedRisk symbol from the parts InvestRight spells out.
...
def _oa_symbol(row, exchange):
    """Broker row -> CalculatedRisk symbol, never blank when anything is resolvable.
    ...
    only identifier on the row. It is not a tradable CalculatedRisk symbol and
...
def transform_positions_data(positions_data):
    """Positions -> the CalculatedRisk common format.
