...
#
# Builds the CalculatedRisk SymToken table from HDFC Securities InvestRight's
# Security Master.
#
...
#   - `exchange` is only NSE / BSE / MCX. CalculatedRisk's NFO / BFO / CDS come from
...
#     CalculatedRisk form (NIFTY, BANKNIFTY, M&M, GOLD), so no suffix-stripping is
#     needed; the resulting CalculatedRisk symbols are collision-free across all
...
from app.services.live_trading.brokers.hdfcsecurities.api.baseurl import SECURITY_MASTER_URL, USER_AGENT
...
# (broker exchange, instrument_segment) -> CalculatedRisk exchange. Cash rows are
...
# InvestRight index display names that differ from the CalculatedRisk symbol, plus
...
def classify_index_symbol(display_name, security_id):
    """InvestRight index row -> the CalculatedRisk index symbol."""
...
def process_security_master(df):
    ...
    # underlyings) are not tradable and carry no CalculatedRisk exchange.
    usable &= exchange != ""
    ...
    # CalculatedRisk stores indices as instrumenttype "EQ" (the NSE_INDEX/BSE_INDEX
