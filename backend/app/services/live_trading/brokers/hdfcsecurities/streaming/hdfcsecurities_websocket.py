...
from app.services.live_trading.brokers.hdfcsecurities.api.baseurl import USER_AGENT, get_ws_url
from app.services.live_trading.brokers.hdfcsecurities.streaming import hdfcsecurities_market_pb2 as pb
from app.services.live_trading.brokers.hdfcsecurities.streaming.hdfcsecurities_mapping import HDFCSecuritiesCapabilityRegistry
...
# packetType -> CalculatedRisk exchange. `instrumentId` is only unique WITHIN an
# exchange: 840 tokens in the live security master appear on more than one
# CalculatedRisk exchange (836 NSE/CDS pairs alone, plus BSE_INDEX SENSEX sharing
...
class HDFCSecuritiesWebSocket:
    ...
    def _refresh_access_token(self):
        ...
        if not self.user_id:
            return False
        try:
            token = CredentialService.load_by_exchange(self.user_id, "hdfcsecurities").api_key
            if not token:
    ...
    def _send_subscribe(self, scrip_ids, subscription_type):
        try:
            ...
            with self.lock:
                ...
                # instrument live at the broker after CalculatedRisk dropped it.
                scrip_ids = [
                    scrip_id
                    for scrip_id in scrip_ids
