"""HDFC Securities InvestRight WebSocket adapter -> CalculatedRisk unified streaming.

Subclasses BaseBrokerWebSocketAdapter. Resolves CalculatedRisk (symbol, exchange) to
...
BSE_INDEX_ scripId prefixes, and the publish topic keeps the CalculatedRisk exchange
...
Two properties of the feed shape the state kept here:
  ...
  - `instrumentId` is unique only WITHIN an exchange. 840 tokens in the live
    security master appear on more than one CalculatedRisk exchange, so subscription
...
from app.services.live_trading.brokers.hdfcsecurities.mapping.transform_data import ws_scrip_id
from app.services.live_trading.brokers.hdfcsecurities.streaming.hdfcsecurities_mapping import (
    HDFCSecuritiesCapabilityRegistry,
)
from app.services.live_trading.brokers.hdfcsecurities.streaming.hdfcsecurities_websocket import HDFCSecuritiesWebSocket
...
# CalculatedRisk numeric mode -> topic suffix. The proxy fans a published mode DOWN to
...
class HDFCSecuritiesWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(self, broker_name, user_id, auth_data=None):
        try:
            ...
            if auth_data and auth_data.get("token"):
                ...
                access_token = CredentialService.load_by_exchange(user_id, "hdfcsecurities").api_key
    ...
    def _on_ticks(self, ticks):
        for tick in ticks:
            try:
                ...
                # MBP stream for options and are not part of the CalculatedRisk tick
    ...
    def _normalize(self, tick, symbol, exchange, topic_mode):
        """Build the CalculatedRisk normalized tick (same key set as the Zerodha
