...
from app.services.live_trading.brokers.ibulls.api.data import BrokerData
...
def transform_data(data, token):
    ...
    if data["pricetype"] == "MARKET":
        try:
            ...
            feed_token = CredentialService.load_by_exchange(username if username else "kalaivani")

            logger.info(f"Using feed token for user: {username if username else 'kalaivani'}", "ibulls").access_token
