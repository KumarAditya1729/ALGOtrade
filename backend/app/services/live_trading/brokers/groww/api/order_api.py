...
from app.services.live_trading.brokers.groww.database.master_contract_db import (
    format_groww_to_calculatedrisk_symbol,
    format_calculatedrisk_to_groww_symbol,
)
from app.services.live_trading.brokers.groww.mapping.transform_data import (
    EXCHANGE_BSE,
    EXCHANGE_NSE,
    ORDER_STATUS_ACKED,
...
from app.services.live_trading.calculatedrisk_compat import get_br_symbol, get_oa_symbol, get_symbol, get_token
...
def direct_get_order_book(auth):
    ...
    try:
        ...
        # Convert all symbols from Groww format to CalculatedRisk format
        for order in all_orders:
            if "trading_symbol" in order:
                ...
                # First, determine the correct CalculatedRisk exchange
                ...
                if any(suffix in groww_symbol for suffix in ["CE", "PE", "C", "P"]):
                    exchange = "NFO"
                    is_derivative = True
                    order["exchange"] = "NFO"  # Set CalculatedRisk exchange format
                    logger.debug(
                ...
                # For NFO derivatives (options or futures), convert from Groww format to CalculatedRisk format
                if is_derivative:
                    ...
                    try:
                        from app.services.live_trading.calculatedrisk_compat import get_oa_symbol

                        if token:
                            calculatedrisk_symbol = get_oa_symbol(token, "NFO")
                            logger.debug(f"CalculatedRisk Symbol: {calculatedrisk_symbol}")
                            if calculatedrisk_symbol:
                                order["symbol"] = calculatedrisk_symbol
                                logger.debug(
                                    f"Converted NFO symbol by token: {groww_symbol} -> {calculatedrisk_symbol}"
                                )
                        ...
                        try:
                            ...
                            if not is_future:
                                ...
                                if match:
                                    ...
                                    # Format as CalculatedRisk expects: NIFTY15MAY2526650CE
                                    calculatedrisk_symbol = (
                                        f"{symbol_name}{day}{month_name}{year}{strike}{option_type}"
                                    )
                                    order["symbol"] = calculatedrisk_symbol
                                    logger.debug(
                                        f"Converted Groww option symbol by pattern: {groww_symbol} -> {calculatedrisk_symbol}"
                                    )
                                    symbol_converted = True
                            ...
                            # For Futures: Convert from "NIFTY2551FUT" to "NIFTY29MAY25FUT"
                                ...
                                if match:
                                    ...
                                    # Format as CalculatedRisk expects: NIFTY29MAY25FUT
                                    calculatedrisk_symbol = f"{symbol_name}{day}{month_name}{year}FUT"
                                    order["symbol"] = calculatedrisk_symbol
                                    logger.debug(
                                        f"Converted Groww futures symbol by pattern: {groww_symbol} -> {calculatedrisk_symbol}"
                                    )
                                    symbol_converted = True
                        except Exception as e:
...
def get_trade_book(auth):
    ...
    try:
        ...
        # Format trades to match CalculatedRisk's expected format (as used in the REST API)
        # This matches the format expected by the order_data.py mapping functions
        calculatedrisk_trades = []
        for trade in all_trades:
            ...
            calculatedrisk_trade = {
                # Fields expected by CalculatedRisk's UI
            ...
            calculatedrisk_trades.append(calculatedrisk_trade)

        # Log the first transformed trade for debugging
        if calculatedrisk_trades:
            logger.debug(
                f"Sample CalculatedRisk trade format: {json.dumps(calculatedrisk_trades[0], indent=2, default=str)}"
            )
        ...
        response = {
            ...
            "data": calculatedrisk_trades,  # This is what map_trade_data will look for first
            "tradebook": calculatedrisk_trades,  # For compatibility with different naming conventions
        ...
        try:
            ...
            if response_obj.status_code == 200 and response_data.get("status") == "SUCCESS":
                ...
                if "payload" in response_data and "positions" in response_data["payload"]:
                    ...
                    # Transform positions to match CalculatedRisk's expected format
                    for position in raw_positions:
                        ...
                        calculatedrisk_symbol = groww_symbol
                        symbol_converted = False
                        ...
                        try:
                            ...
                            try:
                                ...
                                from calculatedrisk.database.token_db import get_oa_symbol

                            # First try database lookup for any symbol
                            db_symbol = get_oa_symbol(groww_symbol, "NFO")
                            if db_symbol:
                                calculatedrisk_symbol = db_symbol
                                logger.debug(
                                    f"Database: Converted Groww symbol: {groww_symbol} -> {calculatedrisk_symbol}"
                                )
                                symbol_converted = True
                            else:
                                ...
                                if option_match:
                                    ...
                                    logger.debug(
                                        f"Pattern: Converted Groww option symbol: {groww_symbol} -> {calculatedrisk_symbol}"
                                    )
                                    symbol_converted = True
                                else:
                                    ...
                                    if future_match:
                                        ...
                                        calculatedrisk_symbol = f"{symbol_name}{day}{month_name}{year}FUT"
                                        logger.debug(
                                            f"Pattern: Converted Groww futures symbol: {groww_symbol} -> {calculatedrisk_symbol}"
                                        )
                                        symbol_converted = True
                        ...
                        # Map exchange to CalculatedRisk format
                        exchange = position.get("exchange", "")
                        if exchange == "NSE":
                            calculatedrisk_exchange = "NSE_EQ"
                        elif exchange == "BSE":
                            calculatedrisk_exchange = "BSE_EQ"
                        elif exchange == "NFO":
                            calculatedrisk_exchange = "NSE_FO"
                        else:
                            calculatedrisk_exchange = exchange

                        # Create position object in CalculatedRisk format
                        ...
                        if position.get("segment") == "CASH":
                            ...
                            position_symbol = (
                                calculatedrisk_symbol  # Use converted symbol for other segments
                            )

                        transformed_position = {
                            # Standard CalculatedRisk fields
                            "symbol": position_symbol,
                            "tradingsymbol": position_symbol,
                            "exchange": calculatedrisk_exchange,
                            ...
                            "segment": "EQ",  # CalculatedRisk format for CASH segment
                            # Specific Groww fields (renamed to match CalculatedRisk expectations)
                ...
                if fno_response.status_code == 200:
                    ...
                    if (
                        ...
                        for position in fno_positions:
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    if db_symbol:
                                        calculatedrisk_symbol = db_symbol
                                        logger.debug(
                                            f"Database: Converted Groww FNO symbol: {groww_symbol} -> {calculatedrisk_symbol}"
                                        )
                                        symbol_converted = True
                                    else:
                                        # Fallback to pattern matching if database lookup fails
                                        # For Options: Convert from Groww format to CalculatedRisk format
                                        # Groww format: "NIFTY25051334000CE" or "BANKNIFTY25051332500PE"
                                        # CalculatedRisk format: "NIFTY13MAY2534000CE" or "BANKNIFTY13MAY2532500PE"
                                        groww_pattern = re.compile(
                                            r"([A-Z]+)(\d{2})(\d{2})(\d{2})(\d+)([CP]E)"
                                        )
                                        match = groww_pattern.match(groww_symbol)
                                    ...
                                    if match:
                                        ...
                                        # Format as CalculatedRisk expects: NIFTY15MAY2526650CE
                                        calculatedrisk_symbol = f"{symbol_name}{day}{month_name}{year}{strike}{option_type}"
                                        logger.debug(
                                            f"Pattern: Converted Groww option position symbol: {groww_symbol} -> {calculatedrisk_symbol}"
                                        )
                                        symbol_converted = True
                                    ...
                                    # For Futures: Convert from "NIFTY2551FUT" to "NIFTY29MAY25FUT"
                                        ...
                                        if match:
                                            ...
                                            logger.debug(
                                                f"Pattern: Converted Groww futures position symbol: {groww_symbol} -> {calculatedrisk_symbol}"
                                            )
                                            symbol_converted = True
                                except Exception as e:
                                    logger.error(f"Error converting position symbol: {e}")
                            ...
                            exchange = position.get("exchange", "")
                            if exchange == "NSE":
                                calculatedrisk_exchange = "NSE"
                            elif exchange == "BSE":
                                calculatedrisk_exchange = "BSE"
                            elif exchange == "NFO":
                                calculatedrisk_exchange = "NSE_FO"
                            else:
                                calculatedrisk_exchange = exchange
                            ...
                            transformed_position = {
                                "symbol": calculatedrisk_symbol,
                                "tradingsymbol": calculatedrisk_symbol,
                                "exchange": calculatedrisk_exchange,
                                "product": position.get("product", ""),
                                "quantity": net_qty,
                                "net_quantity": net_qty,
                                ...
                                "buy_quantity": buy_qty,
                                "sell_quantity": sell_qty,
                                "segment": "FO",  # CalculatedRisk format for FNO segment
                ...
                if "holdings" in response_data["payload"]:
                    ...
                    for holding in raw_holdings:
                        transformed_holding = {
                            ...
                            # Using the key names CalculatedRisk expects
...
def get_open_position(tradingsymbol, exchange, product, auth):
    ...
    Returns:
        str: Net quantity
    """
    # Convert Trading Symbol from CalculatedRisk Format to Broker Format Before Search
    ...
    if positions_data:
        ...
        # Accept both CalculatedRisk-standard exchange codes and the segment-suffixed
...
def direct_place_order_api(data, auth):
    ...
    Args:
        data (dict): Order data in CalculatedRisk format
        auth (str): Authentication token
    ...
    try:
        ...
        from app.services.live_trading.brokers.groww.database.master_contract_db import SymToken, db_session
        ...
        if db_record and db_record.brsymbol:
            ...
            trading_symbol = format_calculatedrisk_to_groww_symbol(original_symbol, original_exchange)
            logger.debug(
        ...
        if resp.status_code == 200:
            ...
            if response_data.get("status") == "SUCCESS":
                ...
                formatted_response = {
                    ...
                    "trading_symbol": trading_symbol,
                    "symbol": original_symbol,  # Add original CalculatedRisk symbol to response
                }
...
def place_smartorder_api(data, auth):
    ...
    try:
        ...
        with symbol_lock:
            ...
            try:
                from app.services.live_trading.calculatedrisk_compat import get_br_symbol
            except ImportError:
                from calculatedrisk.database.token_db import get_br_symbol
...
def get_holdings(auth):
    ...
    try:
        ...
        # Transform holdings to CalculatedRisk format
...
def cancel_order(orderid, auth, segment=None, symbol=None, exchange=None):
    ...
    Args:
        ...
        symbol (str, optional): Trading symbol in CalculatedRisk format
    ...
    """
    try:
        # If symbol is provided, convert it from CalculatedRisk to Groww format
        if symbol and exchange:
            groww_symbol = format_calculatedrisk_to_groww_symbol(symbol, exchange)
        ...
        if response_obj.status_code == 200:
            ...
            # If symbol is provided, include it in CalculatedRisk format in the response
            if symbol:
                # Add the original CalculatedRisk format symbol to the response
                response["symbol"] = symbol
                logger.debug(f"Including CalculatedRisk symbol in cancel response: {symbol}")
...
def direct_modify_order(data, auth):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # If symbol was provided in the original request, include it in CalculatedRisk format
            if "symbol" in data and data["symbol"]:
                response["symbol"] = data["symbol"]
                logger.debug(f"Including CalculatedRisk symbol in modify response: {data['symbol']}")
            ...
            if order_status.upper() in [s.upper() for s in cancellable_statuses]:
                try:
                    ...
                    if (
                        ...
                        # Get and include symbol in the CalculatedRisk format
                        if "symbol" in order:
                            broker_symbol = order.get("symbol", "")

                            # For NFO symbols that have spaces, convert to CalculatedRisk format
                            ...
                            if exchange == "NFO" and " " in broker_symbol:
                                try:
                                    ...
                                    calculatedrisk_symbol = format_groww_to_calculatedrisk_symbol(
                                        broker_symbol, exchange
                                    )
                                    if calculatedrisk_symbol:
                                        cancelled_item["symbol"] = calculatedrisk_symbol
                                        cancelled_item["brsymbol"] = (
                                            broker_symbol  # Keep original broker symbol for reference
                                        )
                                        logger.debug(
                                            f"Transformed cancelled order symbol for UI: {broker_symbol} -> {calculatedrisk_symbol}"
                                        )
                                except Exception as e:
                                    logger.error(
