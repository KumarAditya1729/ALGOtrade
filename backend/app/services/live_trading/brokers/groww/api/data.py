...
class BrokerData:
    ...
    def _convert_calculatedrisk_to_groww_derivative_symbol(self, symbol):
        """
        Convert CalculatedRisk NFO/BFO symbol format to Groww format
    ...
    def _convert_to_groww_params(self, symbol, exchange):
        ...
        if exchange in ["NFO", "BFO"]:
            ...
            if br_symbol:
                ...
                trading_symbol = self._convert_calculatedrisk_to_groww_derivative_symbol(symbol)
    ...
    def get_history(
        ...
        try:
            ...
            if not df.empty:
                ...
                # NOTE: For CalculatedRiskXTS, return non-indexed DataFrame with timestamp as a column
    ...
    def get_intervals(self) -> dict[str, dict[str, list[str]]]:
        """
        Get supported timeframes for Groww historical data in the CalculatedRisk format.
        ...
        # Return in the standard CalculatedRisk format
    ...
    def get_quotes(self, symbol_list, exchange=None, timeout: int = 5) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Quote data in CalculatedRisk format
        """
        ...
        for sym in symbol_list:
            try:
                ...
                # Map CalculatedRisk exchange to Groww exchange format
                if exchange == "NSE":
                    groww_exchange = EXCHANGE_NSE
                    segment = SEGMENT_CASH
                elif exchange == "BSE":
                ...
                try:
                    ...
                    if response and not response.get("error"):
                        ...
                        if response.get("status") == "SUCCESS" and isinstance(
                            ...
                            # Create quote_item in CalculatedRisk format
                            ...
                            # CRITICAL: Build the quote item directly with values extracted from the response, using field names that CalculatedRisk understands
        ...
        # Single symbol case - return in simpler format for CalculatedRisk frontend
    ...
    def _format_single_quote_response(self, quote_data):
        """Helper method to convert from standard dict to the format expected by CalculatedRisk frontend
        ...
        logger.debug(f"Final CalculatedRisk quote format (data only): {result}")
        return result
        ...
        logger.info(f"Formatting single quote response for CalculatedRisk frontend: {quote}")

        # Based on the sample response, CalculatedRisk expects exactly these fields
        ...
        # LTP - preferred field name in CalculatedRisk
        ...
        # Return exact structure expected by CalculatedRisk
        result = {"status": "success", "data": simple_data}

        logger.info(f"FINAL FORMATTED RESULT: {result}")
        logger.info(f"Formatted result for CalculatedRisk frontend: {result}")
    ...
    def get_depth(self, symbol_list, exchange=None, timeout: int = 5) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Market depth data in CalculatedRisk format
        """
        ...
        try:
            ...
            # Create a properly formatted response for CalculatedRisk
            depth_response = {}
            ...
            # Format the depth response according to CalculatedRisk requirements
            depth_response = {
                "bids": bids,
                "asks": asks,
                "ltp": ltp,
    ...
    def _process_quotes_batch(self, symbols: list) -> list:
        ...
        for item in symbols:
            ...
            try:
                # Get broker symbol from the token database. The brsymbol stored in
                # SymToken is already the format Groww expects (mirrors what
                # the CSV's trading_symbol provides). Re-running the CalculatedRisk
                # → Groww regex on it would mangle valid Groww symbols whose
                # shape happens to match the CalculatedRisk pattern (e.g. JUN26
                # contracts: "NIFTY26JUN22350PE" → "NIFTY22JUN350PE").
                # Only convert from CalculatedRisk when DB lookup misses.
                br_symbol = get_br_symbol(symbol, exchange)

                if not br_symbol:
                    if exchange in ["NFO", "BFO"]:
                        br_symbol = self._convert_calculatedrisk_to_groww_derivative_symbol(symbol)
                    if not br_symbol:
                        logger.warning(
