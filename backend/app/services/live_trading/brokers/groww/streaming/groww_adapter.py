...
class _GrowwMarketCache:
    ...
    carries `buy[]/sell[]` book levels. Every other broker in CalculatedRisk
...
class GrowwWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            auth_token = CredentialService.load_by_exchange(user_id, "groww").api_key
    ...
    def _fetch_fresh_token(self) -> str | None:
        ...
        if not self.user_id:
            return None
        return CredentialService.load_by_exchange(self.user_id, "groww").api_key
    ...
    def _resubscribe_batch(self, existing: list) -> None:
        ...
        for correlation_id, sub_info in existing:
            ...
            spec = {
                ...
                "calculatedrisk_exchange": sub_info["exchange"],
            ...
            if "symbol" in data and "exchange" in data:
                ...
                with self.lock:
                    for cid, sub in self.subscriptions.items():
                        ...
                        # For index subscriptions, the CalculatedRisk exchange is NSE_INDEX/BSE_INDEX but Groww sends NSE/BSE
                        is_index_match = (
