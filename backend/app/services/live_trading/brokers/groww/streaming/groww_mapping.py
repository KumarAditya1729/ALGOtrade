...
class GrowwExchangeMapper:
    """Maps CalculatedRisk exchange codes to Groww exchange/segment format"""

    # Mapping from CalculatedRisk exchange to Groww exchange and segment
    EXCHANGE_MAP = {
    ...
    @classmethod
    def get_exchange(cls, calculatedrisk_exchange: str) -> str:
        """
        Get Groww exchange from CalculatedRisk exchange code

        Args:
            calculatedrisk_exchange: CalculatedRisk exchange code (e.g., 'NSE', 'NFO')

        Returns:
            str: Groww exchange code
        """
        mapping = cls.EXCHANGE_MAP.get(calculatedrisk_exchange, {})
        return mapping.get("exchange", calculatedrisk_exchange)

    @classmethod
    def get_segment(cls, calculatedrisk_exchange: str) -> str:
        """
        Get Groww segment from CalculatedRisk exchange code
    ...
    @classmethod
    def get_exchange_segment(cls, calculatedrisk_exchange: str) -> tuple:
        """
        Get both exchange and segment from CalculatedRisk exchange code

        Args:
            calculatedrisk_exchange: CalculatedRisk exchange code

        Returns:
            tuple: (exchange, segment)
        """
        mapping = cls.EXCHANGE_MAP.get(calculatedrisk_exchange, {})
        return mapping.get("exchange", calculatedrisk_exchange), mapping.get("segment", "CASH")
