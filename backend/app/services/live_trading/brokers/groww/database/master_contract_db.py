...
DATABASE_URL = get_broker_env("DATABASE_URL")  # Replace with your database path
...
# Functions for symbol format conversion between CalculatedRisk and Groww formats
def format_calculatedrisk_to_groww_symbol(symbol, exchange):
    """
    Convert CalculatedRisk symbol format to Groww symbol format

    Args:
        symbol (str): Symbol in CalculatedRisk format (e.g., AARTIIND29MAY25630CE)
        exchange (str): Exchange code (NSE, BSE, NFO, etc.)

    Returns:
        str: Symbol in Groww format (e.g., "AARTIIND 29MAY25 630 CE")
    """
    logger.info(f"Converting symbol from CalculatedRisk to Groww format: {{symbol}}, {exchange}")
...
def format_groww_to_calculatedrisk_symbol(groww_symbol, exchange):
    """
    Convert Groww symbol format to CalculatedRisk symbol format
    ...
    Returns:
        str: Symbol in CalculatedRisk format (e.g., "AARTIIND29MAY25630CE")
    """
    logger.info(f"Converting symbol from Groww to CalculatedRisk format: {{groww_symbol}}, {exchange}")
    ...
    if exchange == "NFO":
        ...
        # If already in CalculatedRisk format (no spaces), return as is
        ...
        if len(parts) >= 4 and parts[-1] in ("CE", "PE"):
            ...
            # Combine into CalculatedRisk format: [BaseSymbol][ExpirationDate][StrikePrice][OptionType]
            # Example: AARTIIND29MAY25630CE
            calculatedrisk_symbol = f"{base_symbol}{date_str}{strike_price}{option_type}"
            logger.info(f"Converted to CalculatedRisk format: {calculatedrisk_symbol}")
            return calculatedrisk_symbol
        ...
        # For futures
            ...
            # Combine into CalculatedRisk format: [BaseSymbol][ExpirationDate]FUT
            # Example: NIFTY29MAY25FUT
            calculatedrisk_symbol = f"{base_symbol}{date_str}FUT"
            logger.info(f"Converted to CalculatedRisk format: {calculatedrisk_symbol}")
            return calculatedrisk_symbol
        ...
        # Handle case where option type might be missing
            ...
            if is_date:
                try:
                    ...
                    calculatedrisk_symbol = f"{base_symbol}{date_str}{strike_price}{option_type}"
                    logger.info(
                        f"Converted to CalculatedRisk format (assumed {{option_type}}): {calculatedrisk_symbol}"
                    )
                    return calculatedrisk_symbol
                except ValueError:
...
def find_symbol_by_token(token, exchange):
    ...
    Returns:
        str: Symbol in CalculatedRisk format, or None if not found
    """
...
def find_token_by_symbol(symbol, exchange):
    ...
    Args:
        symbol (str): Symbol in either CalculatedRisk or Groww format
        exchange (str): Exchange code
    ...
    if exchange == "NFO":
        # Try with CalculatedRisk format if it was in Groww format
        calculatedrisk_symbol = format_groww_to_calculatedrisk_symbol(symbol, exchange)
        if calculatedrisk_symbol != symbol:
            result = (
                db_session.query(SymToken)
                .filter_by(symbol=calculatedrisk_symbol, exchange=exchange)
                .first()
            )
            if result:
                return result.token

        # Try with Groww format if it was in CalculatedRisk format
...
def process_groww_data(path):
    ...
    try:
        ...
        # Replace specific index symbols with standardized CalculatedRisk names
        # Mapping Groww index symbols -> CalculatedRisk standard symbols
...
def master_contract_download():
    ...
    try:
        ...
        # Step 5: Add CalculatedRisk symbols where needed (vectorized - remove spaces from brsymbol)
        # For NFO options with spaces in brsymbol, the CalculatedRisk format is just the symbol without spaces
        nfo_space_mask = (
