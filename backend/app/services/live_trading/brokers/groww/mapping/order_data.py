...
from app.services.live_trading.brokers.dhan.mapping.transform_data import map_exchange
from app.services.live_trading.calculatedrisk_compat import get_symbol
...
def map_order_data(order_data):
    """
    Processes and modifies order data from Groww format to CalculatedRisk format.

    Parameters:
    - order_data: A dictionary with either 'data' key or raw Groww API response with 'order_list'.

    Returns:
    - The modified order_data with standardized fields in CalculatedRisk format.
    """
    ...
    for i, order in enumerate(orders_to_process):
        ...
        # Convert broker symbol to CalculatedRisk format
        calculatedrisk_symbol = broker_symbol
        ...
        if exchange == "NFO" and broker_symbol and " " in broker_symbol:
            try:
                ...
                from app.services.live_trading.brokers.groww.database.master_contract_db import format_groww_to_calculatedrisk_symbol

                calculatedrisk_symbol = format_groww_to_calculatedrisk_symbol(broker_symbol, exchange)
                logger.info(f"Transformed display symbol: {broker_symbol} -> {calculatedrisk_symbol}")
            except Exception as e:
                logger.error(f"Error converting symbol format: {e}")

        # Look up in database as fallback
        if calculatedrisk_symbol == broker_symbol and " " in broker_symbol:
            try:
                # Look up in database
                ...
                with db_session() as session:
                    ...
                    if db_record and db_record.symbol:
                        calculatedrisk_symbol = db_record.symbol
                        logger.info(
                            f"Found symbol in database: {broker_symbol} -> {calculatedrisk_symbol}"
                        )
            except Exception as e:
        ...
        mapped_order = {
            ...
            "symbol": calculatedrisk_symbol,  # Using the converted CalculatedRisk format symbol
        ...
        # Map status to CalculatedRisk format
        status_map = {
            "NEW": "open",
            "ACKED": "open",
        ...
        # Map product type to CalculatedRisk format
...
def transform_order_data(orders):
    """
    Transform order data from Groww API format to CalculatedRisk standard format

    Args:
        orders (dict): Order data from Groww API

    Returns:
        list: Transformed orders in CalculatedRisk format for orderbook.py
    """
    ...
    for i, order in enumerate(orders_to_process):
        ...
        # Get proper CalculatedRisk symbol from database using token lookup
        token = None
        ...
        # If we have a token or brsymbol (tradingsymbol/trading_symbol), look up the CalculatedRisk symbol from the database
        try:
            from app.services.live_trading.calculatedrisk_compat import get_oa_symbol

            # Try to get the CalculatedRisk symbol using the token if available
            if token:
                calculatedrisk_symbol = get_oa_symbol(token, exchange)
                if calculatedrisk_symbol:
                    symbol = calculatedrisk_symbol
                    logger.info(f"Found CalculatedRisk symbol by token: {broker_symbol} -> {symbol}")

            # If token lookup failed or token wasn't available, try by broker symbol
            elif broker_symbol:
                # First check if we already have the CalculatedRisk symbol
                if exchange == "NFO" and (
                    broker_symbol.endswith("CE") or broker_symbol.endswith("PE")
                ):
                    # Query the database to find the CalculatedRisk symbol for this broker symbol
                    from app.services.live_trading.brokers.groww.database.master_contract_db import SymToken, db_session
                    ...
                    with db_session() as session:
                        ...
                        if record and record.symbol:
                            ...
                            logger.info(
                                f"Found CalculatedRisk symbol in database: {broker_symbol} -> {symbol}"
                            )
        except Exception as e:
            logger.error(f"Error looking up CalculatedRisk symbol from database: {e}")
        ...
        # Map order type to CalculatedRisk format
        ...
        # For NFO instruments, ensure the symbol is in CalculatedRisk format (AARTIIND29MAY25630CE)
        exchange = order.get("exchange", "NSE")
        if exchange == "NFO" and " " in symbol:
            try:
                ...
                calculatedrisk_symbol = format_groww_to_calculatedrisk_symbol(symbol, exchange)
                if calculatedrisk_symbol:
                    # Store broker symbol for reference
                    broker_symbol = symbol
                    # Use CalculatedRisk symbol format for display
                    symbol = calculatedrisk_symbol
                    logger.info(f"Transformed order symbol for UI: {broker_symbol} -> {symbol}")
            except Exception as e:
                logger.error(f"Error converting order symbol format: {e}")

        # Create transformed order in CalculatedRisk format
        transformed_order = {
            "symbol": symbol,  # Now guaranteed to be in CalculatedRisk format
            "exchange": order.get("exchange", "NSE"),
    ...
    # Final check to ensure all symbols are in CalculatedRisk format using database lookups
        ...
        if order.get("exchange") == "NFO" and "symbol" in order and order["symbol"]:
            ...
            if token:
                try:
                    ...
                    calculatedrisk_symbol = get_oa_symbol(token, order.get("exchange", "NSE"))
                    if calculatedrisk_symbol:
                        order["symbol"] = calculatedrisk_symbol
                        logger.info(f"Final token lookup: {symbol} -> {calculatedrisk_symbol}")
                        continue
                except Exception as e:
...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        ...
        # Try to get CalculatedRisk symbol from database
        try:
            from app.services.live_trading.calculatedrisk_compat import get_oa_symbol
