...
from app.services.live_trading.brokers.nubra.api.baseurl import INDEX_MASTER_PATH, get_nubra_headers, get_url
...
def download_nubra_instruments(output_path):
    ...
    auth_token = CredentialService.load_by_exchange(login_username)

    if not auth_token:
        raise Exception(f"No valid auth token found for user '{login_username}'. Please login first.", "nubra").api_key
...
def process_nubra_json(path):
    ...
    and converts it into CalculatedRisk master contract format.
    ...
    - Symbol format follows CalculatedRisk F&O spec:
    ...
    # CalculatedRisk exchange mapping
    ...
    # ---------------- Symbol Construction (CalculatedRisk format) ----------------
...
def process_nubra_indexes(path):
    """
    Processes the Nubra index CSV file to fit the CalculatedRisk database schema.
    ...
    Output format follows CalculatedRisk convention:
    - exchange = NSE_INDEX or BSE_INDEX
    - brexchange = NSE or BSE (original exchange)
    - instrumenttype = 'INDEX'
    - symbol = Standardized CalculatedRisk format (NIFTY, BANKNIFTY, SENSEX, etc.)
    ...
    # Map CSV columns to CalculatedRisk database schema
    ...
    # Map to CalculatedRisk index exchange format
    ...
    # Common Index Symbol Formats - map Nubra INDEX_SYMBOL to CalculatedRisk standard
    # Reference: CalculatedRisk symbols.md
    nubra_to_calculatedrisk_index = {
        # ---- NSE Indexes ----
        'INDIA_VIX': 'INDIAVIX',
    ...
    df['symbol'] = df['symbol'].replace(nubra_to_calculatedrisk_index)
