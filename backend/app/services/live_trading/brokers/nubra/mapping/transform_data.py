...
def sanitize_strat_tag(tag):
    """
    Normalize an CalculatedRisk strategy name into a Nubra V3 ``stratTags`` entry.
    ...
    CalculatedRisk strategy name ("My Strategy_v2") becomes a legal tag
    ("my-strategy-v2").
    """
    slug = re.sub(r"[^A-Za-z0-9]+", "-", str(tag or "calculatedrisk")).strip("-").lower()
    return slug or "calculatedrisk"


def map_order_side(action):
    """CalculatedRisk action -> Nubra V3 ``side``. V3 uses bare BUY/SELL."""
    return "SELL" if str(action).upper() == "SELL" else "BUY"


def map_order_delivery_type(product):
    """
    CalculatedRisk product -> Nubra V3 ``deliveryType``.
...
def reverse_map_product_type(delivery_type):
    """Nubra V3 ``deliveryType`` -> CalculatedRisk product."""
    return {"CNC": "CNC", "IDAY": "MIS"}.get(str(delivery_type).upper(), "MIS")


def map_price_type(pricetype):
    """
    CalculatedRisk pricetype -> Nubra V3 ``priceType``.
...
def build_entry_trigger(pricetype, trigger_price, action):
    ...
    CalculatedRisk SL/SL-M carry a trigger_price; V3 expresses that as an LTP entry
...
def transform_data(data, token):
    """
    Transform an CalculatedRisk place-order request into ONE Nubra V3 order item.

    The caller wraps the result as ``{"orders": [item]}`` before POSTing it to
    /sentinel/orders/create.

    CalculatedRisk in:  action, product, pricetype, price, trigger_price, quantity
...
def transform_modify_order_data(data, orderid):
    """
    Transform an CalculatedRisk modify request into ONE Nubra V3 modify item.
...
def map_product_type(product):
    """CalculatedRisk product -> V3 deliveryType, for position lookups."""
    return map_order_delivery_type(product)


# --- Exchange mapping (Nubra -> CalculatedRisk) -----------------------------------
#
...
#
# Everything keyed on the CalculatedRisk exchange therefore misses unless that is
# folded back first -- get_symbol, get_oa_symbol and get_token are all keyed on
# the CalculatedRisk exchange, and so is the ``pos_exchange == exchange`` comparison
# in the smart-order path.
#
# The forward direction (CalculatedRisk -> Nubra, used when subscribing to the market
# feed) lives in streaming/nubra_mapping.py.

# Nubra cash exchange -> the CalculatedRisk derivatives exchange its F&O rows live
# under. MCX is absent on purpose: Nubra reports commodity futures and options
# as "MCX", which is already the CalculatedRisk exchange for them.
...
def map_exchange(brexchange, derivative_type=None):
    """
    Maps the Broker Exchange to the CalculatedRisk Exchange.
...
def candidate_exchanges(brexchange, derivative_type=None):
    """
    CalculatedRisk exchanges worth probing for a Nubra row, most likely first.
