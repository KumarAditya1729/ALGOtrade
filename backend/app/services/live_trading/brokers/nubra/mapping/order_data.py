...
from app.services.live_trading.brokers.nubra.mapping.transform_data import (
    brexchange_of,
    candidate_exchanges,
    derivative_type_of,
    map_exchange,
...
def resolve_instrument(brexchange, derivative_type=None, ref_id=None, broker_symbol=None):
    """
    Resolve a Nubra instrument to (CalculatedRisk symbol, CalculatedRisk exchange).

    The exchange is confirmed rather than inferred: map_exchange() only decides
    which CalculatedRisk exchanges are worth probing, and both values returned come
...
def resolve_position(position):
    ...
    CalculatedRisk symbol for cash instruments -- an option is ``NIFTY26AUG24000CE``
    on Nubra and ``NIFTY25AUG2624000CE`` in CalculatedRisk.
...
# Nubra V3 lifecycle bucket -> CalculatedRisk status vocabulary.
...
def _resolve_symbol(order):
    ...
    ``exchange`` is folded to the CalculatedRisk exchange before any lookup -- Nubra
    ...
    # CalculatedRisk symbol -- it is the last resort purely so the row still renders,
...
def _order_type(order, trigger_paise):
    """
    Derive the CalculatedRisk pricetype for a V3 order.
...
def map_order_data(order_data):
    """
    Normalize the bucketed GET /sentinel/orders response to CalculatedRisk format.
    ...
    for order in orders:
        ...
        # CalculatedRisk "trigger pending" (matches the zerodha reference vocabulary).
...
def transform_tradebook_data(tradebook_data):
    """
    Transform normalized trade data to final CalculatedRisk UI format.
    """
    transformed_data = []
...
def map_position_data(position_data):
    """
    Map Nubra's V3 positions response to CalculatedRisk normalized format.
...
def transform_holdings_data(holdings_data):
    """
    Transform mapped Nubra holdings data to final CalculatedRisk UI format.
...
def map_portfolio_data(portfolio_data):
    ...
    Symbols are mapped to CalculatedRisk format via get_oa_symbol().
    ...
    for h in raw_holdings:
        ...
        # Look up CalculatedRisk symbol from database using the broker symbol
