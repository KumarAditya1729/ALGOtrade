...
fields CalculatedRisk needs. Prices arrive in paise (÷100, matching
...
from app.services.live_trading.brokers.nubra.api.auth_api import get_ws_urls
from app.services.live_trading.brokers.nubra.mapping.order_data import resolve_instrument
from app.services.live_trading.brokers.nubra.mapping.transform_data import map_exchange
...
# NubraIntentOrderStatus enum -> CalculatedRisk's lowercase order_status vocabulary.
...
# OrderSide enum -> CalculatedRisk action constants
_ACTION_MAP = {1: "BUY", 2: "SELL"}

# PriceType enum -> CalculatedRisk pricetype constants
_PRICETYPE_MAP = {1: "LIMIT", 2: "MARKET"}

# OrderDeliveryType enum -> CalculatedRisk product constants (IDAY = intraday)
...
class NubraOrderUpdateAdapter(BaseOrderUpdateAdapter):
    ...
    def get_ws_url(self) -> str:
        ...
        if self._ws_url is None:
            ...
            self._ws_url = user_ws_url or get_broker_env(
...
def create_nubra_order_adapter(user_id: str) -> "NubraOrderUpdateAdapter | None":
    ...
    session_token = CredentialService.load_by_exchange(user_id, "nubra").api_key
