"""
Nubra WebSocket streaming adapter for CalculatedRisk.
...
Data Flow:
    Nubra WS -> NubraWebSocket (protobuf decode) -> callback override
      -> nubra_adapter.py (transform to CalculatedRisk format)
...
from app.services.live_trading.brokers.nubra.api.nubrawebsocket import (
    INDEX_NAME_MAP,
    SUBSCRIPTION_MAP,
    NubraWebSocket,
)
...
class NubraWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        try:
            ...
            auth_token = CredentialService.load_by_exchange(user_id, "nubra").api_key
            if not auth_token:
    ...
    def _get_fresh_auth_token(self) -> str | None:
        ...
        if not self.user_id:
            return None
        try:
            return CredentialService.load_by_exchange(self.user_id, "nubra").api_key
        except Exception as e:
    ...
    def _on_index_data(self, obj):
        ...
        via the index channel. Transforms to CalculatedRisk format and publishes
    ...
    def _on_orderbook_data(self, obj):
        ...
        channel (mode 3 / Depth). Transforms to CalculatedRisk format and publishes
