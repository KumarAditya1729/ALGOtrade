...
from app.services.live_trading.brokers.nubra.api.baseurl import (
    SESSION_EXPIRED_MESSAGE,
    SESSION_EXPIRED_STATUS,
    get_base_url,
    get_nubra_headers,
)
from app.services.live_trading.brokers.nubra.mapping.order_data import (
    extract_positions,
...
# CalculatedRisk status vocabulary. Orders arrive grouped by bucket rather than as a
...
def _error_payload(status_code, data):
    """
    Standardize a failed Nubra response for CalculatedRisk's service layer.
...
def _promote_error_to_message(response_data):
    ...
    docs say to surface the ``error`` string directly, but CalculatedRisk's service
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    for position in extract_positions(positions_data):
        ...
        # both sides of these comparisons are in CalculatedRisk terms. Comparing the
...
def place_order_api(data, auth):
    ...
    # Transform CalculatedRisk data to the Nubra V3 intent-order payload
    ...
    if response.status_code in (200, 201) and intent_order_id:
        ...
        # Normalize response format for CalculatedRisk compatibility
        response_data["status"] = True
        response_data["data"] = {"orderid": orderid}
        # CalculatedRisk service layer expects status 200 for success
        response.status = 200
    else:
        orderid = None
...
def close_all_positions(current_api_key, auth):
    ...
    for position in positions:
        ...
        # so both must already be in CalculatedRisk terms -- passing Nubra's own
        ...
        if not symbol or not exchange:
            logger.error(
                ...
                f"refId={position.get('refId')!r} to an CalculatedRisk symbol"
            )
