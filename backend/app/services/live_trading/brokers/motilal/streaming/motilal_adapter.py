"""
Motilal Oswal WebSocket adapter for CalculatedRisk streaming proxy.

This adapter integrates Motilal Oswal's WebSocket client with CalculatedRisk's
...
from app.services.live_trading.brokers.motilal.api.motilal_websocket import MotilalWebSocket
...
class MotilalWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            auth_token = CredentialService.load_by_exchange(user_id, "motilal").api_key
    ...
    def _get_fresh_auth_token(self) -> str | None:
        ...
        if not self.user_id:
            return None
        try:
            return CredentialService.load_by_exchange(self.user_id, "motilal").api_key
