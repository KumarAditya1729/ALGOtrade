...
# "SNXT50", ...) while CalculatedRisk needs canonical symbols per symbol_Openalgo.md.
# The mapping is kept local to this broker loader so other brokers — which
# already feed clean strings — aren't affected.

# Broker-house-style NSE index name -> CalculatedRisk canonical symbol. Keys are
...
# Broker-house-style BSE index name -> CalculatedRisk canonical symbol. Keys are
...
def standardize_index_symbols(df):
    """
    Standardize NSE_INDEX and BSE_INDEX symbol names to CalculatedRisk canonical form
...
def process_motilal_index_csv(df, exchange_name):
    """
    Processes the Motilal Index CSV file to fit the CalculatedRisk database schema.
    ...
    # Map exchange to CalculatedRisk format with _INDEX suffix
    ...
    # Standardize index symbols to CalculatedRisk format
...
def process_motilal_csv(df, exchange_name):
    """
    Processes the Motilal CSV file to fit the CalculatedRisk database schema.
    ...
    # Rename columns based on Motilal API format to CalculatedRisk schema
    df = df.rename(
        columns={
            "scripcode": "token",
            "scripname": "symbol",
    ...
    # Map Motilal exchange names to CalculatedRisk exchange names
    exchange_map = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NSEFO": "NFO",
    ...
    # Format symbols according to CalculatedRisk standards
