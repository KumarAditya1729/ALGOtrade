...
from app.services.live_trading.calculatedrisk_compat import get_br_symbol, get_symbol_info
from app.services.live_trading.calculatedrisk_compat import get_logger
from app.services.live_trading.calculatedrisk_compat import calculate_protected_price, get_instrument_type_from_symbol
...
def map_exchange(exchange):
    """
    Maps CalculatedRisk exchange names to Motilal Oswal exchange names.

    CalculatedRisk uses: NSE, BSE, NFO, CDS, MCX, BFO
...
def reverse_map_exchange(exchange):
    """
    Reverse maps Motilal Oswal exchange names to CalculatedRisk exchange names.
    """
...
def transform_data(data, token, auth_token=None):
    """
    Transforms the CalculatedRisk API request structure to Motilal Oswal expected structure.
    ...
    calculatedrisk_exchange = data["exchange"]
    motilal_exchange = map_exchange(calculatedrisk_exchange)
    ...
    if pricetype == "MARKET":
        logger.info(
            ...
            f"Exchange={calculatedrisk_exchange}, Action={action}"
        )
        try:
            if auth_token:
                from app.services.live_trading.brokers.motilal.api.data import BrokerData

                broker_data = BrokerData(auth_token)
                quote_data = broker_data.get_quotes(data["symbol"], calculatedrisk_exchange)
                ltp = float(quote_data.get("ltp", 0)) if quote_data else 0

                if ltp > 0:
                    instrument_type = get_instrument_type_from_symbol(data["symbol"])
                    tick_size = None
                    symbol_info = get_symbol_info(data["symbol"], calculatedrisk_exchange)
    ...
    transformed = {
        ...
        "producttype": map_product_type(
            data["product"], calculatedrisk_exchange
        ),  # Pass CalculatedRisk exchange for context
...
def transform_modify_order_data(data, token, lastmodifiedtime, qtytradedtoday):
    ...
    Args:
        data: CalculatedRisk modify order request data
...
def map_order_type(pricetype):
    """
    Maps CalculatedRisk pricetype to Motilal Oswal order type.
...
def map_product_type(product, exchange=None):
    """
    Maps CalculatedRisk product type to Motilal Oswal product type based on exchange.
    ...
    Args:
        product: CalculatedRisk product type (CNC, MIS, NRML)
        exchange: CalculatedRisk exchange name (NSE, BSE, NFO, CDS, MCX, BFO)
    """
    # Determine if this is a cash segment or derivative segment
    # Using CalculatedRisk exchange names
...
def reverse_map_product_type(product, exchange=None):
    """
    Reverse maps Motilal Oswal product type to CalculatedRisk product type.
    Context:
