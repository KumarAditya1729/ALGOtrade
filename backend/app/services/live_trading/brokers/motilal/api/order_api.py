...
from app.services.live_trading.brokers.motilal.mapping.transform_data import (
    map_exchange,
    map_product_type,
...
from app.services.live_trading.calculatedrisk_compat import get_br_symbol, get_symbol, get_symbol_info, get_token
...
def get_api_response(endpoint, auth, method="GET", payload=""):
    AUTH_TOKEN = auth
    api_key = get_broker_env("BROKER_API_SECRET")
    ...
    headers = {
        ...
        "SourceId": "WEB",
        "vendorinfo": get_broker_env("BROKER_VENDOR_CODE", ""),
        "osname": "Windows 10",
        "osversion": "10.0.19041",
        "devicemodel": "AHV",
        "manufacturer": "DELL",
        "productname": "CalculatedRisk",
    ...
    base_url = get_broker_env("BROKER_API_URL", "https://openapi.motilaloswal.com")
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    # Convert Trading Symbol from CalculatedRisk Format to Broker Format Before Search in OpenPosition
    tradingsymbol = get_br_symbol(tradingsymbol, exchange)
    # Map exchange from CalculatedRisk format to Motilal format for comparison
...
def place_order_api(data, auth):
    AUTH_TOKEN = auth
    BROKER_API_SECRET = get_broker_env("BROKER_API_SECRET")
...
def close_all_positions(current_api_key, auth):
    ...
    if positions_response.get("status") == "SUCCESS":
        ...
        for position in positions_response["data"]:
            ...
            # Convert Motilal exchange to CalculatedRisk exchange for symbol lookup
            motilal_exchange = position["exchange"]
            calculatedrisk_exchange = reverse_map_exchange(motilal_exchange)

            # Get calculatedrisk symbol to send to placeorder function
            symbol = get_symbol(position["symboltoken"], calculatedrisk_exchange)
            logger.debug(f"The Symbol is {symbol}")

            if not symbol:
                logger.error(
                    f"Symbol not found for token {position['symboltoken']} and exchange {calculatedrisk_exchange}"
                )
                continue
            ...
            place_order_payload = {
                ...
                "symbol": symbol,
                "action": action,
                "exchange": calculatedrisk_exchange,  # Use CalculatedRisk exchange format
                "pricetype": "MARKET",
                "product": reverse_map_product_type(position["productname"], calculatedrisk_exchange),
