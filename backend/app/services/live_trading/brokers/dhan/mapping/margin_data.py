...
from app.services.live_trading.brokers.dhan.mapping.transform_data import map_exchange_type
...
def transform_margin_position(position, client_id):
    """
    Transform a single CalculatedRisk margin position to Dhan margin format.
...
def map_product_type_for_margin(product):
    """
    Maps CalculatedRisk product type to Dhan product type for margin calculation.
...
def parse_margin_response(response_data):
    """
    Parse Dhan single-order margin response to CalculatedRisk standard format.

    Args:
        response_data: Raw response from Dhan API

    Returns:
        Standardized margin response matching CalculatedRisk RESTX margin format
    """
    try:
