# Dhan Forever Order payload transforms (CalculatedRisk ⇄ Dhan).
# Dhan v2 reference: https://dhanhq.co/docs/v2/forever/

from app.services.live_trading.brokers.dhan.mapping.transform_data import (
    map_exchange,
    map_exchange_type,
    map_product_type,
    reverse_map_product_type,
)
from app.services.live_trading.calculatedrisk_compat import get_oa_symbol, get_token


# Dhan Forever Order status → CalculatedRisk GTT status.
_STATUS_MAP = {
    "TRANSIT": "active",
    "PENDING": "active",
    "CONFIRM": "active",
...
def transform_place_gtt(data):
    """Transform an CalculatedRisk flat place-GTT payload into Dhan's POST /forever/orders body.
    ...
    # CalculatedRisk's ``strategy`` doubles as Dhan's correlationId (max 30 chars).
...
def transform_modify_gtt(data, leg_name):
    """Transform an CalculatedRisk modify-GTT payload into Dhan's PUT /forever/orders/{id} body.

    Dhan modifies one leg at a time. Field semantics dispatch by
    ``trigger_type`` (the CalculatedRisk flag), not by ``leg_name`` — Dhan's leg
    ...
    for CalculatedRisk a SINGLE always carries its values in ``price`` and the
    resolved trigger.
...
def map_gtt_book(gtt_list):
    """Normalise Dhan's GET /forever/orders response into an CalculatedRisk-shaped list.
    ...
    one CalculatedRisk entry per order. ``last_price`` is not returned by Dhan, so
