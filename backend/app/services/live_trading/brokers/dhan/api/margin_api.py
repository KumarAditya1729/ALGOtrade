...
from app.services.live_trading.brokers.dhan.api.baseurl import get_url
from app.services.live_trading.brokers.dhan.mapping.margin_data import parse_margin_response, transform_margin_position
from app.services.live_trading.calculatedrisk_compat import get_user_id, verify_api_key
...
def get_client_id(api_key=None):
    ...
    Args:
        api_key: CalculatedRisk API key (optional)
...
def parse_basket_margin_response(response_data):
    ...
    can use camelCase. Accept both and expose CalculatedRisk's common margin fields.
    """
    try:
...
def calculate_margin_api(positions, auth, api_key=None):
    ...
    Args:
        ...
        api_key: CalculatedRisk API key (optional, for client ID lookup)
