...
from websocket_proxy.order_adapter import BaseOrderUpdateAdapter, to_calculatedrisk_symbol

logger = get_logger(__name__)

DHAN_ORDER_UPDATE_WS_URL = "wss://api-order-update.dhan.co"

# Live Order Update "Status" values -> CalculatedRisk's lowercase order_status
# vocabulary (open/complete/rejected/cancelled). TRANSIT/EXPIRED have no
# exact CalculatedRisk equivalent; TRANSIT is treated as still-open (in flight to
...
# Live Order Update "Product" single-letter codes -> CalculatedRisk product constants
_PRODUCT_MAP = {
    "C": "CNC",
    "I": "MIS",
    "M": "NRML",  # MARGIN — closest CalculatedRisk equivalent
    "F": "NRML",  # MTF — closest CalculatedRisk equivalent
}

# Live Order Update "OrderType" codes -> CalculatedRisk pricetype constants
_PRICETYPE_MAP = {
    "LMT": "LIMIT",
    "MKT": "MARKET",
    "SL": "SL",
...
# Live Order Update "TxnType" codes -> CalculatedRisk action constants
_ACTION_MAP = {"B": "BUY", "S": "SELL"}

# (Exchange, Segment) from the order-update payload -> CalculatedRisk exchange code.
...
class DhanOrderUpdateAdapter(BaseOrderUpdateAdapter):
    ...
    def normalize(self, raw_message):
        ...
        # CalculatedRisk exchange from (exchange, segment); symbol via securityId —
        ...
        symbol = to_calculatedrisk_symbol(
...
def create_dhan_order_adapter(user_id: str) -> "DhanOrderUpdateAdapter | None":
    ...
    access_token = CredentialService.load_by_exchange(user_id, "dhan").api_key
    if not access_token:
