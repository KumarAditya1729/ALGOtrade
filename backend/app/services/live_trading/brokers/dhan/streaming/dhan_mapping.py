...
class DhanExchangeMapper:
    """Maps between CalculatedRisk exchange names and Dhan exchange codes"""

    # CalculatedRisk to Dhan exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE_EQ",
        "BSE": "BSE_EQ",
        "NFO": "NSE_FNO",
    ...
    # Dhan exchange segment codes (numeric) to CalculatedRisk exchange mapping
    ...
    @classmethod
    def get_dhan_exchange(cls, calculatedrisk_exchange: str) -> str | None:
        """Convert CalculatedRisk exchange to Dhan exchange format"""
        return cls.EXCHANGE_MAP.get(calculatedrisk_exchange)

    @classmethod
    def get_calculatedrisk_exchange(cls, dhan_exchange: str) -> str | None:
        """Convert Dhan exchange to CalculatedRisk exchange format"""
        return cls.DHAN_TO_OPENALGO.get(dhan_exchange)

    @classmethod
    def get_exchange_from_segment(cls, segment_code: int) -> str | None:
        """Convert Dhan exchange segment code to CalculatedRisk exchange"""
    ...
    def get_segment_from_exchange(cls, exchange: str) -> int | None:
        """Convert CalculatedRisk exchange to Dhan exchange segment code"""
