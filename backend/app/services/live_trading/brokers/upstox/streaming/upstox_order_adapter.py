...
# Upstox order "status" free-text values -> CalculatedRisk's lowercase order_status
...
# Upstox "product" codes -> CalculatedRisk product constants
...
class UpstoxOrderUpdateAdapter(BaseOrderUpdateAdapter):
    ...
    def normalize(self, raw_message):
        ...
        # Map to CalculatedRisk symbol format (NHPC-EQ -> NHPC) via the instrument
...
def create_upstox_order_adapter(user_id: str) -> "UpstoxOrderUpdateAdapter | None":
    ...
    access_token = CredentialService.load_by_exchange(user_id, "upstox").api_key
    if not access_token:
