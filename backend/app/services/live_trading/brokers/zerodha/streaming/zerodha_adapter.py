...
class ZerodhaWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    Properly implements CalculatedRisk WebSocket proxy interface with correct topic formatting.
    """
    ...
    def initialize(
        ...
        try:
            ...
            self.api_key = get_broker_env("BROKER_API_KEY")
            if not self.api_key:
                return {"status": "error", "message": "API key not found in environment variables"}

            # Get auth token with bypass_cache=True so the initial
    ...
    def _transform_tick(self, tick: dict) -> dict | None:
        """Transform Zerodha tick to CalculatedRisk format with index support"""
        try:
            token = tick.get("instrument_token")
            if not token:
                return None
