...
Credentials: CalculatedRisk stores Zerodha's DB auth token as the composite
...
# Kite order "status" values -> CalculatedRisk's lowercase order_status vocabulary.
...
class ZerodhaOrderUpdateAdapter(BaseOrderUpdateAdapter):
    ...
    def normalize(self, raw_message):
        ...
        # already match CalculatedRisk's constants — no mapping tables needed.
        # The postback also carries "instrument_token" (see
        # broker-api-docs/zerodha-api-docs/12-postbacks.md's sample payload);
        # passing it lets to_calculatedrisk_symbol try the more reliable
...
def create_zerodha_order_adapter(user_id: str) -> "ZerodhaOrderUpdateAdapter | None":
    ...
    the ticker URL.
    """
    auth_token = CredentialService.load_by_exchange(user_id, "zerodha").api_key
    if not auth_token:
