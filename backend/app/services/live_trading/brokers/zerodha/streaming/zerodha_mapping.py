...
and CalculatedRisk's standard format.
"""
...
class ZerodhaExchangeMapper:
    """Maps exchange codes between Zerodha and CalculatedRisk formats"""

    # Map CalculatedRisk exchange codes to Zerodha exchange codes
    ...
    # Map Zerodha exchange codes to CalculatedRisk exchange codes
    ...
    def to_zerodha_exchange(cls, oa_exchange: str) -> str:
        """
        Convert CalculatedRisk exchange code to Zerodha exchange code.

        Args:
            oa_exchange: CalculatedRisk exchange code (e.g., 'NSE', 'NFO')
    ...
    def to_oa_exchange(cls, zerodha_exchange: str) -> str:
        """
        Convert Zerodha exchange code to CalculatedRisk exchange code.

        Args:
            zerodha_exchange: Zerodha exchange code

        Returns:
            CalculatedRisk exchange code
        """
...
class ZerodhaCapabilityRegistry:
    ...
    # Map CalculatedRisk capability flags to Zerodha subscription modes
    CAPABILITY_MAP = {
        "LTP": "ltp",
        "QUOTE": "quote",
        "DEPTH": "full",
    ...
    def get_zerodha_mode(cls, capability: str) -> str:
        ...
        Args:
            capability: CalculatedRisk capability (LTP, QUOTE, DEPTH)
...
class ZerodhaDataTransformer:
    """Transforms data between Zerodha and CalculatedRisk formats"""
    ...
    def transform_tick(self, tick_data: dict, symbol: str, exchange: str) -> dict:
        """
        Transform Zerodha tick data to CalculatedRisk format.
        ...
        Returns:
            Transformed tick data in CalculatedRisk format
        """
        try:
            if not tick_data:
                return {}
    ...
    def transform_order_update(self, order_data: dict) -> dict:
        """
        Transform Zerodha order update to CalculatedRisk format.

        Args:
            order_data: Raw order data from Zerodha WebSocket

        Returns:
            Transformed order data in CalculatedRisk format
        """
        try:
            ...
            # Map Zerodha status to CalculatedRisk status
            status_map = {
                "OPEN": "open",
                "COMPLETE": "complete",
                "CANCELLED": "cancelled",
    ...
    def transform_position(self, position_data: dict) -> dict:
        """
        Transform Zerodha position data to CalculatedRisk format.

        Args:
            position_data: Raw position data from Zerodha

        Returns:
            Transformed position data in CalculatedRisk format
        """
        try:
