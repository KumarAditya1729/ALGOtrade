...
def download_csv_zerodha_data(output_path):
    ...
    try:
        ...
        AUTH_TOKEN = CredentialService.load_by_exchange(login_username, "zerodha").api_key
...
def process_zerodha_csv(path):
    ...
    # BEFORE we remap to CalculatedRisk names, so brexchange retains the broker's
    ...
    # NSE Index Symbol Mapping (Zerodha tradingsymbol → CalculatedRisk format)
