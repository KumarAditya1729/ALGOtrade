# Zerodha GTT payload transforms (CalculatedRisk ⇄ Kite).
...
def transform_place_gtt(data):
    """Transform an CalculatedRisk flat place-GTT payload into Kite's `{type, condition, orders}`.
...
def transform_modify_gtt(data):
    """Transform an CalculatedRisk modify-GTT payload (flat shape) into Kite's body.
...
def map_gtt_book(gtt_data):
    """Normalise Kite's GET /gtt/triggers response into an CalculatedRisk-shaped list.
    ...
    Kite tradingsymbol back to CalculatedRisk's symbol.
    """
