...
from app.services.live_trading.calculatedrisk_compat import get_br_symbol, get_token
from app.services.live_trading.calculatedrisk_compat import get_logger

logger = get_logger(__name__)


# ─── Product / Order type mappings (CalculatedRisk ↔ AliceBlue V2) ──────────────────


def map_product_type(product):
    """Map CalculatedRisk product type to AliceBlue V2 product type."""
    mapping = {
        "CNC": "LONGTERM",
        "NRML": "NRML",
        "MIS": "INTRADAY",
...
def reverse_map_product_type(product):
    """Map AliceBlue V2 product type back to CalculatedRisk product type."""
    mapping = {
        "LONGTERM": "CNC",
        "NRML": "NRML",
        "INTRADAY": "MIS",
...
def map_order_type(pricetype):
    """Map CalculatedRisk price type to AliceBlue V2 order type."""
    mapping = {
        "MARKET": "MARKET",
        "LIMIT": "LIMIT",
        "SL": "SL",
...
def reverse_map_order_type(order_type):
    """Map AliceBlue V2 order type back to CalculatedRisk price type."""
    mapping = {
        "MARKET": "MARKET",
        "LIMIT": "LIMIT",
        "SL": "SL",
...
# ─── Payload builders (CalculatedRisk request → AliceBlue V2 API payload) ──────────


def transform_data(data):
    """
    Transform an CalculatedRisk place-order request into an AliceBlue V2 API payload item.
    """
    ...
    return {
        ...
        "apiOrderSource": "",
        "algoId": "",
        "orderTag": "calculatedrisk",
    }


def transform_modify_order_data(data):
    """
    Transform an CalculatedRisk modify-order request into an AliceBlue V2 API modify payload.
    """
    return {
