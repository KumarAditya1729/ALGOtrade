...
class AliceblueWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        try:
            if auth_data:
                ...
                auth_token = CredentialService.load_by_exchange(user_id, "aliceblue").api_key
                feed_token = CredentialService.load_by_exchange(user_id, "aliceblue").access_token
            ...
            with self.lock:
                ...
                if mode > existing_mode:
                    self.subscriptions[sub_key] = {
                        ...
                        "original_symbol": symbol,  # Store original CalculatedRisk symbol for lookup
                        "original_exchange": exchange,  # Store original CalculatedRisk exchange
    ...
    def _schedule_reconnect(self) -> None:
        ...
        def reconnect():
            ...
            if not self.running:  # Only reconnect if not already running
                ...
                if self.user_id:
                    fresh_token = CredentialService.load_by_exchange(self.user_id, "aliceblue").api_key
                    if fresh_token:
    ...
    def _on_data_received(self, parsed_data):
        ...
        try:
            ...
            # Also, for NFO/BFO symbols, AliceBlue returns broker symbols but we need CalculatedRisk symbols
