        ...
        for order in orders:
            ...
            if symbol and exchange:
                ...
                if oa_symbol:
                    ...
                    # Map product types to CalculatedRisk constants following Angel pattern
...
def transform_order_data(orders):
    ...
    for order in orders:
        ...
        # Map DefinedGe order status to CalculatedRisk format
        # API order_status values: CANCELED / COMPLETE / NEW / OPEN / REJECTED / REPLACED
        status = order.get("order_status", "").upper()
        if status in ["COMPLETE", "EXECUTED"]:
        ...
        # Map API price types (LIMIT/MARKET/SL-LIMIT/SL-MARKET) to CalculatedRisk constants
...
def map_trade_data(trade_data):
    ...
    if trades:
        for trade in trades:
            ...
            # Map product types to CalculatedRisk constants (following CalculatedRisk order constants)
...
def transform_tradebook_data(tradebook_data):
    """
    Transform DefinedGe tradebook data to CalculatedRisk format.

    Parameters:
    - tradebook_data: List of trade dictionaries from DefinedGe API

    Returns:
    - List of transformed trade dictionaries matching CalculatedRisk format
    """
    transformed_data = []
    ...
    for trade in tradebook_data:
        ...
        transformed_trade = {
            "symbol": trade.get("tradingsymbol", ""),
            "exchange": trade.get("exchange", ""),
            "product": product_type,  # Mapped to CalculatedRisk constants
...
def map_position_data(position_data):
    ...
    if positions:
        for position in positions:
            ...
            # Map product types to CalculatedRisk constants
...
def transform_positions_data(positions_data):
    """
    Transform DefinedGe positions data to CalculatedRisk format.
...
def transform_holdings_data(holdings_data):
    """
    Transform DefinedGe holdings data to CalculatedRisk format.
    Following Angel's pattern for consistency.
    """
    transformed_data = []
