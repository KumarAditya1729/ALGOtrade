...
def get_algo_id(exchange):
    ...
    override = get_broker_env("DEFINEDGE_ALGO_ID")
    if override:
        return override
...
def transform_data(data, token_id):
    """Transform CalculatedRisk order data to DefinedGe Securities format"""
    ...
    try:
        ...
        # Map CalculatedRisk fields to DefinedGe fields based on API docs
        transformed_data = {
...
def map_exchange(exchange):
    """Map CalculatedRisk exchange to DefinedGe exchange"""
    exchange_mapping = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
...
def reverse_map_exchange(exchange):
    """Map DefinedGe exchange to CalculatedRisk exchange"""
    reverse_mapping = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
...
def map_product_type(product):
    """Map CalculatedRisk product type to DefinedGe product type.
...
def reverse_map_product_type(product):
    """Map DefinedGe product type to CalculatedRisk product type"""
    reverse_mapping = {
        "INTRADAY": "MIS",
        "CNC": "CNC",
...
def map_price_type(pricetype):
    """Map CalculatedRisk price type to DefinedGe price type.
...
def map_definedge_price_type(pricetype):
    """Map CalculatedRisk price type to DefinedGe API price type (for modify order)"""
    price_mapping = {"MARKET": "MARKET", "LIMIT": "LIMIT", "SL": "SL-LIMIT", "SL-M": "SL-MARKET"}
    return price_mapping.get(pricetype, "LIMIT")


def map_product_type_for_modify(product):
    """Map CalculatedRisk product type to DefinedGe product type for modify order"""
    product_mapping = {
        "MIS": "INTRADAY",
...
def reverse_map_price_type(pricetype):
    """Map DefinedGe price type to CalculatedRisk price type"""
    reverse_mapping = {
        "MARKET": "MARKET",
        "LIMIT": "LIMIT",
        "SL-LIMIT": "SL",
...
def map_order_status(status):
    """Map DefinedGe order status to CalculatedRisk status.
...
def transform_order_data(order):
    """Transform DefinedGe order data to CalculatedRisk format"""
    try:
        transformed_order = {
...
def transform_position_data(position):
    """Transform DefinedGe position data to CalculatedRisk format"""
    try:
...
def transform_holding_data(holding):
    """Transform DefinedGe holding data to CalculatedRisk format"""
    try:
