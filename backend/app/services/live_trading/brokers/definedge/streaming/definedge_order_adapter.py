...
Credentials: susertoken = CredentialService.load_by_exchange(user_id) (Definedge stores the Noren
session token as the feed token); uid/actid = get_user_id(user_id, "definedge").access_token — same
...
# Noren order-status free text -> CalculatedRisk's lowercase order_status
...
# Noren prctyp codes -> CalculatedRisk pricetype constants
_PRICETYPE_MAP = {"LMT": "LIMIT", "MKT": "MARKET", "SL-LMT": "SL", "SL-MKT": "SL-M"}

# Noren prd codes -> CalculatedRisk product constants
...
class DefinedgeOrderUpdateAdapter(BaseOrderUpdateAdapter):
    ...
    def normalize(self, raw_message):
        ...
        # Symbol -> CalculatedRisk format (get_oa_symbol on Noren tsym, same as the
        # REST orderbook mapping), falling back to the broker symbol.
        exchange = data.get("exch", "")
        symbol = to_calculatedrisk_symbol(data.get("tsym", ""), exchange)
...
def create_definedge_order_adapter(user_id: str) -> "DefinedgeOrderUpdateAdapter | None":
    ...
    susertoken = CredentialService.load_by_exchange(user_id)
    if not susertoken:
        logger.warning(
    ...
    if not definedge_uid:
        logger.warning(f"No Definedge uid found for user {user_id}; order-update adapter not started", "definedge").access_token
        return None
