...
from app.services.live_trading.brokers.definedge.api.baseurl import get_url
from app.services.live_trading.brokers.definedge.api.rate_limiter import rate_limited_request
from app.services.live_trading.brokers.definedge.mapping.transform_data import (
    map_product_type,
...
def close_all_positions(current_api_key, auth):
    ...
    for position in positions_to_close:
        try:
            ...
            if not symbol:
                logger.error(f"Failed to get CalculatedRisk symbol for {tradingsymbol} on {exchange}")
                symbol = tradingsymbol  # Use original as fallback

            logger.debug(f"CalculatedRisk symbol: {symbol}")
