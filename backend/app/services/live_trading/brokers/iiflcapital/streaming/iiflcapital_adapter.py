"""
IIFL Capital WebSocket adapter — connects CalculatedRisk's WebSocket proxy to the
...
from app.services.live_trading.calculatedrisk_compat import get_brexchange, get_token
...
class IiflcapitalWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """
    CalculatedRisk broker adapter for IIFL Capital's MQTT market-data feed.
    ...
    decoded dict into three CalculatedRisk-shaped payloads on the way out, exactly
    like Zerodha's `full` → `ltp/quote/full` fan-out.
    """

    # CalculatedRisk mode ints → internal IIFL feed mode strings.
    ...
    def __init__(self) -> None:
        ...
        # {
        #   "exchange": str,           # CalculatedRisk exchange (e.g. NSE_INDEX)
        #   "symbol": str,
        #   "segment": str,            # IIFL brexchange (NSEEQ, NSEFO, …)
        #   "token": str,
        #   "mode": int,               # CalculatedRisk mode int (1/2/3)
        #   "is_index": bool,
        # }
        self.subscribed_symbols: dict[str, dict] = {}

        # Reverse lookup: f"{segment}/{token}" → (symbol, exchange) — used to
        # rebuild the CalculatedRisk topic when a tick comes back from the broker.
    ...
    def _handle_ticks(self, ticks: list[dict]) -> None:
        ...
        the CalculatedRisk ZeroMQ bus, slicing the single broker packet into LTP /
