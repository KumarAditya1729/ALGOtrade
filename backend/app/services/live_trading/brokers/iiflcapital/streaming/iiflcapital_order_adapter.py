...
client ID), 05-orders.md ("Order Book" / "Trade Book", for the CalculatedRisk
...
from app.services.live_trading.brokers.iiflcapital.api.rate_limiter import apply_rate_limit
from app.services.live_trading.brokers.iiflcapital.baseurl import BASE_URL
from app.services.live_trading.brokers.iiflcapital.mapping.transform_data import (
    reverse_map_order_type,
    reverse_map_product_type,
)
from app.services.live_trading.brokers.iiflcapital.streaming.iiflcapital_mqtt import (
    CONNACK_ACCEPTED,
    IiflMqttClient,
)
...
from app.services.live_trading.calculatedrisk_compat import bus
...
from websocket_proxy.order_adapter import to_calculatedrisk_symbol
...
# orderStatus (order-update packets) -> CalculatedRisk's lowercase order_status
...
# `exchange` field, e.g. "NSEEQ") -> CalculatedRisk exchange codes. Mirrors
...
# (max 7 O(1) cache lookups); best-effort like to_calculatedrisk_symbol() itself.
...
# transactionType -> CalculatedRisk BUY/SELL. Order-update packets observed on a
...
def _resolve_order_exchange(token: str) -> str:
    """Best-effort resolve the CalculatedRisk exchange for an order-update packet's
...
class IiflCapitalOrderUpdateAdapter:
    ...
    def _run_forever(self) -> None:
        ...
        while not self._shutting_down:
            ...
            fresh_token = CredentialService.load_by_exchange(self.user_id, "iiflcapital").api_key
            if fresh_token:
    ...
    def _make_client_id(self) -> str:
        # Distinct prefix from the market-feed adapter's "calculatedrisk" + suffix
        ...
        return (
            "calculatedrisk-orderupdate" + datetime.now().strftime("%d%m%y%H%M%S%f") + os.urandom(4).hex()
        )
    ...
    def normalize(self, message_kind: str, data: dict) -> dict | None:
        ...
        _handle_message). Uses CalculatedRisk's common field-name vocabulary (see
    ...
    def _normalize_order(self, data: dict) -> dict:
        ...
        symbol = (
            to_calculatedrisk_symbol(trading_symbol, exchange, token=token)
            if exchange
            else trading_symbol
        )
...
def create_iiflcapital_order_adapter(user_id: str) -> IiflCapitalOrderUpdateAdapter | None:
    ...
    """
    auth_token = CredentialService.load_by_exchange(user_id, "iiflcapital").api_key
    if not auth_token:
        logger.warning(
