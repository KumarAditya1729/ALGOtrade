"""
Helpers for translating between CalculatedRisk's market-data conventions and the
...
already stores the IIFL segment in the `brexchange` column of `SymToken`:
    ...
    CalculatedRisk exchange  →  brexchange
    -----------------     ----------
    NSE                  NSEEQ
    BSE                  BSEEQ
    NFO                  NSEFO
...
# CalculatedRisk exchanges that are routed to the index MQTT topic prefix
...
# CalculatedRisk exchanges that support open-interest data. Anything outside this
