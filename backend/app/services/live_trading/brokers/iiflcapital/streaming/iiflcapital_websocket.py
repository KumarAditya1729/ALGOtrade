...
broker-feed surface that other CalculatedRisk adapters consume (Zerodha-style):
...
CalculatedRisk's WebSocket proxy still wants three distinct modes (LTP/Quote/Depth),
...
# CalculatedRisk's mode ints — the adapter layer translates 1/2/3 → "ltp"/"quote"/"full".
MODE_LTP = "ltp"
MODE_QUOTE = "quote"
MODE_FULL = "full"
...
class IiflcapitalWebSocket:
    ...
    def subscribe_instruments(
        ...
        Args:
            ...
            mode: "ltp" / "quote" / "full" — affects which CalculatedRisk modes the
    ...
    def _dispatch_oi(self, key: str, payload: bytes) -> None:
        ...
        # hours). This keeps the CalculatedRisk proxy's mode set simple.
