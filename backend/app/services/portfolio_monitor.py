...
def _monitor_report_texts(language: str = "") -> Dict[str, str]:
    texts = {
        ...
        "subtitle": "Generated by CalculatedRisk Fast AI Analysis Engine",
    ...
    overrides = {
        "zh-CN": {
            "title": "AI 定时分析报告", "subtitle": "由 CalculatedRisk 快速 AI 分析引擎生成", "kicker": "定时 AI 监控",
        ...
        "zh-TW": {
            "title": "AI 定時分析報告", "subtitle": "由 CalculatedRisk 快速 AI 分析引擎生成", "kicker": "定時 AI 監控",
        ...
        for channel in channels:
            try:
                ...
                if ch == 'browser':
                    ...
                    recipient = targets.get('whatsapp', '')
                    token_override = targets.get('whatsapp_token', '') or targets.get('whatsapp_bot_token', '')
                    phone_override = targets.get('whatsapp_phone_number_id', '')
                    if recipient:
                        import re
                        whatsapp_report = re.sub(r'<b>(.*?)</b>', r'*\1*', telegram_report)
                        whatsapp_report = re.sub(r'<i>(.*?)</i>', r'_\1_', whatsapp_report)
                        whatsapp_report = re.sub(r'<[^>]+>', '', whatsapp_report)
                        notifier._notify_whatsapp(
                            recipient=recipient, text=whatsapp_report,
                            token_override=token_override, phone_id_override=phone_override
                        )
                elif ch == 'email':
            ...
            for channel in channels:
                try:
                    ...
                    if ch == 'browser':
                        ...
                        phone_override = targets.get('whatsapp_phone_number_id', '')
                        if recipient:
                            notifier._notify_whatsapp(recipient=recipient, text=f"*{error_title}*\n\n{error_msg}", token_override=token_override, phone_id_override=phone_override)
                    elif ch == 'email':
                        to_email = targets.get('email', '')
                        if to_email:
                    ...
                    for channel in channels:
                        try:
                            ...
                            if ch == 'browser':
                                ...
                                if recipient:
                                    import re
                                    whatsapp_msg = re.sub(r'<b>(.*?)</b>', r'*\1*', alert_message)
                                    whatsapp_msg = re.sub(r'<i>(.*?)</i>', r'_\1_', whatsapp_msg)
                                    whatsapp_msg = re.sub(r'<[^>]+>', '', whatsapp_msg)
                                    notifier._notify_whatsapp(recipient=recipient, text=whatsapp_msg, token_override=token_override, phone_id_override=phone_override)
                            elif ch == 'email':
                                to_email = targets.get('email', '')
                                if to_email:
