...
def test_celery_queues_keep_trading_outside_task_system():
    ...
    assert routes["calculatedrisk.tasks.agent_job"]["queue"] == "jobs"
    assert routes["calculatedrisk.tasks.fast_analysis"]["queue"] == "ai"
...
def test_celery_beat_owns_periodic_maintenance():
    ...
    assert schedule["reflection-cycle"]["task"] == "calculatedrisk.tasks.reflection"
    assert schedule["ai-calibration-cycle"]["task"] == "calculatedrisk.tasks.ai_calibration"
    assert schedule["market-catalog-sync"]["task"] == "calculatedrisk.tasks.market_catalog_sync"
