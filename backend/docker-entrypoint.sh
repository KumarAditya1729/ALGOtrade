#!/bin/sh
# CalculatedRisk Docker Entrypoint Script
# Checks and validates SECRET_KEY before starting the application

set -e

echo "============================================"
echo "  CalculatedRisk Backend - Starting..."
echo "============================================"
...
if [ ! -f /app/.env ]; then
    ...
    if [ -f /app/env.example ]; then
        if cp /app/env.example /app/.env 2>/tmp/calculatedrisk-env-copy.err; then
            echo "[INFO] Created .env from env.example"
            echo "[IMPORTANT] Please edit /app/.env and set a secure SECRET_KEY before restarting!"
        else
            echo "[WARNING] Cannot create /app/.env: $(cat /tmp/calculatedrisk-env-copy.err)"
            ...
            rm -f /tmp/calculatedrisk-env-copy.err
        fi
    else
...
DEFAULT_SECRET="calculatedrisk-secret-key-change-me"
...
if [ -n "${PROMETHEUS_MULTIPROC_DIR:-}" ]; then
    ...
    chown -R calculatedrisk:calculatedrisk "$PROMETHEUS_MULTIPROC_DIR" 2>/dev/null || true
fi
...
if [ "$(id -u)" = "0" ] && id calculatedrisk >/dev/null 2>&1; then
    chown -R calculatedrisk:calculatedrisk /app/logs /app/data 2>/dev/null || true
    if [ -f /app/.env ]; then
        if chown calculatedrisk:calculatedrisk /app/.env 2>/dev/null; then
