...
def _csv_rows(url: str) -> list[dict]:
    response = requests.get(url, timeout=30, headers={"User-Agent": "CalculatedRisk/4.0"})
...
def hk_factsheet(factsheet_code: str) -> list[dict]:
    ...
    response = requests.get(url, timeout=45, headers={"User-Agent": "CalculatedRisk/4.0"})
    response.raise_for_status()
...
def crypto_top100() -> list[dict]:
    response = requests.get(
        ...
        timeout=30,
        headers={"User-Agent": "CalculatedRisk/4.0"},
    )
    response.raise_for_status()
    rows = []
    seen = set()
